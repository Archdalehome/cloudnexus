...
`;function J(t){return String(t??"").replace(/[&<>"']/g,e=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[e])}function ke(t){let e=String(t||"").trim();if(!e)return"";let a=J(e);return`<link rel="icon" href="${a}">
...
`}var Ye=`
  ...
  </div>`;function Ue(t,e,a,d){let m=t||"订单管理系统",p=e!==!1,r=String(a??"").trim();return`<!DOCTYPE html>
<html lang="zh-CN">
<head>
<meta charset="UTF-8">
...
</html>`}function Le(t,e,a){let d=e===!1,m=String(a??"").trim()||"订单管理系统";return`<!DOCTYPE html>
<html lang="zh-CN">
<head>
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width, initial-scale=1.0">
<title>我的待办 - ${J(m)}</title>
${ke(t)}
<style>
${ve}
  .topbar {
...
<body>
  ...
  <div class="container">
    <div class="add-row"${d?' style="display:none"':""}>
...
</html>`}function De(t,e){let a=String(e??"").trim()||"订单管理系统";return`<!DOCTYPE html>
<html lang="zh-CN">
<head>
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width, initial-scale=1.0">
...
</html>`}import{connect as Ze}from"cloudflare:sockets";var Y="default",Xe="__platform__",et=24*60*60*1e3,ye="订单管理系统",tt="该功能为专业版功能：请点击顶栏「订阅」升级为专业用户后使用（试用账号仅限本人使用与添加订单）",z=/^[A-Za-z0-9._%+-]+@[A-Za-z0-9-]+(\.[A-Za-z0-9-]+)*\.[A-Za-z]{2,}$/,j=10*60,$e=5,ue=60,rt=5,nt=1e4,Z=3,at=2e4,st=8e3;function R(){let t=new Uint8Array(24);return crypto.getRandomValues(t),Array.from(t,e=>e.toString(16).padStart(2,"0")).join("")}async function U(t){let e=new TextEncoder().encode(t+"::cf-todolist-salt"),a=await crypto.subtle.digest("SHA-256",e);return Array.from(new Uint8Array(a),d=>d.toString(16).padStart(2,"0")).join("")}function Q(t){let e=String(t??"").trim();return e?!/^https?:\/\//i.test(e)||/\s/.test(e)?null:e:""}var ot=["USD","CNY"];function it(t){let e=String(t??"").trim().toUpperCase();return ot.includes(e)?e:"USD"}function n(t,e=200){return new Response(JSON.stringify(t),{status:e,headers:{"Content-Type":"application/json; charset=utf-8"}})}async function S(t){try{return await t.json()}catch{return{}}}function Je(t){let a=(t.headers.get("Cookie")||"").match(/(?:^|;\s*)token=([^;]+)/);return a?a[1]:null}function x(t){return t?N(t.role)?Xe:t.teamId||Y:Y}function N(t){return t==="superadmin"}function E(t){return t==="team"}function ie(t){return!!t&&(t.plan==="pro"||t.plan==="paid")}function ae(t){if(!t)return"active";if(t.status==="disabled")return"disabled";if(ie(t)){let e=t.expiresAt||"";if(e&&new Date(e).getTime()<Date.now())return"expired"}return"active"}function C(t){return ie(t)&&ae(t)==="active"}function Qe(t){return t&&(t.subscribeRequest||t.renewRequest)||null}function M(){return n({error:tt},403)}async function $(t,e){if(!e||e===Y)return null;let a=await t.TODO_KV.get(`user:${e}`);if(!a)return null;let d=JSON.parse(a);return E(d.role)?d:null}async function dt(t,e){return e?E(e.role)?C(e):C(await $(t,x(e))):!1}async function h(t,e){let a=Je(t);if(!a)return null;let d=await e.TODO_KV.get(`session:${a}`);if(!d)return null;let m=await e.TODO_KV.get(`user:${d}`);if(!m)return null;let p=JSON.parse(m);if(!N(p.role)){let r=await $(e,x(p));if(r&&r.status==="disabled")return null}return p}function ct(){let t=new Uint32Array(1);return crypto.getRandomValues(t),String(t[0]%1e6).padStart(6,"0")}function K(t){return String(t??"").replace(/[&<>"']/g,e=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[e])}function te(t){let e=String(t||"").trim(),a=e.lastIndexOf("@");if(a<=0)return e;let d=e.slice(0,a),m=d.slice(0,Math.min(2,d.length));return m+"*".repeat(Math.max(1,d.length-m.length))+e.slice(a)}function be(t){try{let e=new URL(t.url).hostname;return e==="localhost"||e==="127.0.0.1"||e==="::1"||e==="[::1]"}catch{return!1}}function lt(t){let e=new Date(t||Date.now());if(Number.isNaN(e.getTime()))return String(t||"");let a=new Date(e.getTime()+8*60*60*1e3),d=m=>String(m).padStart(2,"0");return`${a.getUTCFullYear()}-${d(a.getUTCMonth()+1)}-${d(a.getUTCDate())} ${d(a.getUTCHours())}:${d(a.getUTCMinutes())}`}function he(t){let e=Array.isArray(t)?t.slice(0,Z):[],a=[];for(let d=0;d<Z;d++){let m=String(e[d]||"").trim();a.push(m&&m.length<=300&&!/\s/.test(m)&&/^https?:\/\//i.test(m)?m:"")}return a}function X(t){let e=String(t&&t.to||"").trim().toLowerCase(),a=t&&t.cc!==void 0&&t.cc!==null?t.cc:[],d=Array.isArray(a)?a:[a],m=[];for(let p of d){let r=String(p||"").trim();!r||!z.test(r)||r.toLowerCase()!==e&&(m.some(s=>s.toLowerCase()===r.toLowerCase())||m.push(r))}return m}function ut(t,e){let a=[t&&t.adminNotifyEmail,t&&t.from,e&&e.supportEmail];for(let d of a){let m=String(d||"").trim();if(m&&z.test(m))return m}return""}async function re(t){let e={};try{let w=await t.TODO_KV.get("emailSettings");w&&(e=JSON.parse(w)||{})}catch{e={}}let a=String(t.RESEND_API_KEY||e.apiKey||"").trim(),d=String(t.MAIL_FROM_NAME||e.fromName||"").trim(),m=String(t.SMTP_HOST||e.smtpHost||"smtp.qq.com").trim(),p=Number(t.SMTP_PORT||e.smtpPort||465)||465,r=String(t.SMTP_USER||t.QQ_MAIL_USER||e.smtpUser||"").trim(),s=String(t.SMTP_PASS||t.QQ_MAIL_PASS||e.smtpPass||"").trim(),o=String(t.MAIL_FROM||e.from||"").trim(),i=t.SEND_EMAIL&&typeof t.SEND_EMAIL.send=="function"?t.SEND_EMAIL:null,l=o||(r&&z.test(r)?r:""),c=a?"resend":r&&s?"smtp":i?"cloudflare":"",u=String(t.ADMIN_NOTIFY_EMAIL||e.adminNotifyEmail||"").trim(),f=he(e.subQrCodes),g=a?t.RESEND_API_KEY?"env":"kv":r&&s?t.SMTP_USER||t.SMTP_PASS||t.QQ_MAIL_USER||t.QQ_MAIL_PASS?"env":"kv":i?"binding":"none";return{provider:c,apiKey:a,from:l,fromName:d,smtpHost:m,smtpPort:p,smtpUser:r,smtpPass:s,binding:i,adminNotifyEmail:u,subExtraText:String(e.subExtraText||"").trim(),subQrCodes:f,devMode:e.devMode===!0,source:g}}function de(t){let e=new TextEncoder().encode(String(t??"")),a="";for(let d=0;d<e.length;d++)a+=String.fromCharCode(e[d]);return btoa(a)}function Re(t){let e=String(t||""),a=[];for(let d=0;d<e.length;d+=76)a.push(e.slice(d,d+76));return a.join(`\r
`)}function _e(t){let e=String(t??"");return/^[\x20-\x7e]*$/.test(e)?e:`=?UTF-8?B?${de(e)}?=`}var mt="support@cloudnexus.cn";async function G(t){let e={};try{let a=await t.TODO_KV.get("settings");a&&(e=JSON.parse(a)||{})}catch{e={}}return{siteName:e.siteName||ye,allowRegister:e.allowRegister!==!1,supportEmail:e.supportEmail===void 0?mt:String(e.supportEmail||"").trim(),favicon:String(e.favicon||"").trim()}}async function pt(t,e,a){let d="";if(e){let p=E(e.role)?e:await $(t,x(e));d=p?p.teamName||p.username:""}let m=a?String(a.siteName||"").trim():"";return d||m||ye}function Ve(t){let e=String(t||"");return e?e.slice(0,4)+"****"+e.slice(-4):""}function ze(t,e){return{provider:t.provider,source:t.source,from:t.from,fromName:t.fromName,hasApiKey:!!t.apiKey,apiKeyMasked:Ve(t.apiKey),smtpHost:t.smtpHost,smtpPort:t.smtpPort,smtpUser:t.smtpUser,hasSmtpPass:!!t.smtpPass,smtpPassMasked:Ve(t.smtpPass),devMode:t.devMode,adminNotifyEmail:t.adminNotifyEmail||"",subExtraText:t.subExtraText||"",subQrCodes:he(t.subQrCodes).concat("").slice(0,Z),envApiKey:!!e.RESEND_API_KEY,envFrom:!!e.MAIL_FROM,envSmtp:!!(e.SMTP_USER||e.SMTP_PASS||e.QQ_MAIL_USER||e.QQ_MAIL_PASS)}}function ft(t,e){let a="cfmail_"+R().slice(0,16),d=String(t.from||"localhost").split("@")[1]||"localhost",m=t.fromName?`${_e(t.fromName)} <${t.from}>`:`<${t.from}>`,p=X(e),r=[`From: ${m}`,`To: <${e.to}>`];return p.length&&r.push(`Cc: ${p.map(s=>`<${s}>`).join(", ")}`),r.push(`Subject: ${_e(e.subject||"")}`,`Date: ${new Date().toUTCString()}`,`Message-ID: <${R()}@${d}>`,"MIME-Version: 1.0",`Content-Type: multipart/alternative; boundary="${a}"`,""),r.concat([`--${a}`,"Content-Type: text/plain; charset=UTF-8","Content-Transfer-Encoding: base64","",Re(de(e.text||"")),`--${a}`,"Content-Type: text/html; charset=UTF-8","Content-Transfer-Encoding: base64","",Re(de(e.html||e.text||"")),`--${a}--`,""]).join(`\r
`)}function gt(t,e,a){let d=null;return Promise.race([t,new Promise((m,p)=>{d=setTimeout(()=>{try{a&&a()}catch{}p(new Error(`连接或响应超时（${Math.round(e/1e3)} 秒）`))},e)})]).finally(()=>clearTimeout(d))}function bt(t,e,a){let d=t&&t.message?t.message:String(t),m=(d.match(/\b(\d{3})\b/)||[])[1]||"",p="（提示：本地 `wrangler dev` 已支持 SMTP 的 TLS；如持续超时请检查网络 / 端口是否被拦截，或先用「调试模式」联调）",r="";return m==="535"||m==="534"?r="（提示：QQ 邮箱必须使用「SMTP 授权码」而不是 QQ 登录密码，并先在 QQ 邮箱「设置 → 账户」中开启 SMTP 服务）":m==="530"?r="（提示：服务器要求先建立加密连接 —— 465 用 SSL、587 用 STARTTLS）"+(a?p:""):m==="550"||m==="553"?r="（提示：QQ 邮箱要求「发件邮箱」与 SMTP 账号完全一致）":a&&/secureTransport|starttls|tls|ssl|certificate|handshake|超时|关闭/i.test(d)?r=p:/starttls must be set|secureTransport/i.test(d)?r="（提示：587 必须先以 STARTTLS 建立连接再升级；若持续失败请把端口改为 465（SSL）重试）":/certificate|tls|ssl|handshake/i.test(d)?r="（提示：端口 465 使用 SSL，端口 587 使用 STARTTLS，请检查服务器 / 端口设置）":/超时/.test(d)&&(r="（提示：连接超时通常是网络或端口被拦截所致；QQ 邮箱请用 smtp.qq.com 的 465（SSL）或 587（STARTTLS））"),`SMTP 发信失败（${e}）：${d}${r}`}async function wt(t,e,a){let d=Number(t.smtpPort)||465,m=[d];d===465?m.push(587):d===587&&m.push(465);let p=[];for(let r=0;r<m.length;r++){let s=await yt(m[r],t,e,a);if(s.ok)return s;p.push(m.length>1?`端口 ${m[r]}：${s.error}`:s.error)}return{ok:!1,error:p.join("；")}}async function yt(t,e,a,d){let m=e.smtpHost||"smtp.qq.com",p=!!(d&&d.local),r=Number(e.smtpTimeoutMs)||(p?st:at),s=new TextEncoder,o=new TextDecoder,i=null,l=null,c=null,u="",f=`连接 ${m}:${t}`;async function g(){for(;;){let k=u.indexOf(`
`);if(k!==-1){let y=u.slice(0,k);return u=u.slice(k+1),y.replace(/\r$/,"")}let{value:T,done:O}=await l.read();if(O)throw new Error("连接已被服务器关闭");u+=o.decode(T,{stream:!0})}}async function w(){let k=0,T="";for(;;){let O=await g();if(!(O.length<3)&&(k=parseInt(O.slice(0,3),10)||k,T+=(T?" ":"")+O.slice(4).trim(),O.charAt(3)!=="-"))break}return{code:k,text:T}}async function v(k,T){await c.write(s.encode(k+`\r
`));let O=await w();if(!T.includes(O.code))throw new Error(`服务器返回 ${O.code}${O.text?" "+O.text:""}`);return O}async function b(){f=`连接 ${m}:${t}`;try{await i.opened}catch(A){throw new Error(`建立连接失败：${A&&A.message?A.message:A}`)}let k=await w();if(k.code!==220)throw new Error(`服务器返回 ${k.code} ${k.text}`);let O=`EHLO ${e.smtpEhlo||String(e.from||"").split("@")[1]||"localhost"}`;if(await v(O,[250]),t===587){f="STARTTLS",await v("STARTTLS",[220]);try{l.releaseLock()}catch{}try{c.releaseLock()}catch{}i=i.startTls(),u="",l=i.readable.getReader(),c=i.writable.getWriter(),await v(O,[250])}f="身份认证（SMTP 授权码）",await v("AUTH LOGIN",[334]),await v(de(e.smtpUser),[334]),await v(de(e.smtpPass),[235]),f="发件地址",await v(`MAIL FROM:<${e.from}>`,[250]);let y=[a.to].concat(X(a));for(let A=0;A<y.length;A++)f=A===0?"收件地址":"抄送地址",await v(`RCPT TO:<${y[A]}>`,[250,251]);f="发送邮件正文",await v("DATA",[354]);let I=ft(e,a).replace(/\r\n\./g,`\r
..`);await c.write(s.encode(`${I}\r
.\r
`));let _=await w();if(_.code!==250)throw new Error(`服务器返回 ${_.code}${_.text?" "+_.text:""}`);try{await v("QUIT",[221])}catch{}return{ok:!0,id:""}}try{return i=Ze({hostname:m,port:t},{secureTransport:t===587?"starttls":"on"}),l=i.readable.getReader(),c=i.writable.getWriter(),await gt(b(),r,()=>{try{i.close()}catch{}})}catch(k){return{ok:!1,error:bt(k,f,p)}}finally{try{i&&i.close()}catch{}}}async function Te(t,e,a,d){if(!e.provider)return{ok:!1,error:"邮件服务未配置：请在控制台「邮件设置」中填写 QQ 邮箱 SMTP 账号与授权码（或 Resend API Key）"};if(!e.from)return{ok:!1,error:"邮件服务缺少「发件邮箱地址」：请在控制台「邮件设置」中补全后重试"};if(e.provider==="smtp")return wt(e,a,d);if(e.provider==="resend"){let m=new AbortController,p=setTimeout(()=>m.abort(),nt);try{let r=await fetch("https://api.resend.com/emails",{method:"POST",headers:{Authorization:`Bearer ${e.apiKey}`,"Content-Type":"application/json"},body:JSON.stringify(Object.assign({from:e.fromName?`${e.fromName} <${e.from}>`:e.from,to:[a.to],subject:a.subject,text:a.text,html:a.html},X(a).length?{cc:X(a)}:{})),signal:m.signal}),s=await r.json().catch(()=>({}));return r.ok?{ok:!0,id:s.id||""}:{ok:!1,error:`邮件服务返回错误（${r.status}）：${s.message||s.error||"请检查 API Key 与发件域名"}`}}catch(r){return{ok:!1,error:"邮件服务请求失败："+(r&&r.message?r.message:String(r))}}finally{clearTimeout(p)}}try{let m=X(a);return await e.binding.send({from:e.fromName?{name:e.fromName,email:e.from}:e.from,to:m.length?[a.to].concat(m):a.to,subject:a.subject,html:a.html,text:a.text}),{ok:!0,id:""}}catch(m){return{ok:!1,error:"邮件发送失败："+(m&&m.message?m.message:String(m))}}}async function ht(t,e,a){let d=ye;try{let o=await t.TODO_KV.get("settings");if(o){let i=JSON.parse(o);i&&i.siteName&&(d=i.siteName)}}catch{}let m=e.teamName||e.username,p=`【${d}】邮箱确认码 ${a}（10 分钟内有效）`,r=[`${m}，您好：`,"",`您正在注册「${d}」的团队账号（登录名：${e.username}）。`,`邮箱确认码：${a}`,"有效期：10 分钟。","","请在注册 / 登录页面输入上面的确认码完成验证，验证通过后即可正常登录。","若非本人操作，请忽略本邮件：验证通过前该账号无法登录。","",d].join(`
`),s=`<div style="font-family:-apple-system,'Microsoft YaHei',sans-serif;font-size:14px;color:#37352f;line-height:1.7"><p>${K(m)}，您好：</p><p>您正在注册「${K(d)}」的团队账号（登录名：<b>${K(e.username)}</b>）。</p><p>邮箱确认码：</p><p style="font-size:26px;font-weight:700;letter-spacing:6px;color:#2383e2;margin:8px 0">${a}</p><p>有效期 <b>10 分钟</b>。请在注册 / 登录页面输入该确认码完成验证，验证通过后即可正常登录。</p><p style="color:#9b9a97;font-size:12px">若非本人操作，请忽略本邮件：验证通过前该账号无法登录。</p></div>`;return{subject:p,text:r,html:s}}async function xt(t,e,a,d,m){let p=ye;try{let y=await t.TODO_KV.get("settings");if(y){let I=JSON.parse(y);I&&I.siteName&&(p=I.siteName)}}catch{}let s=m==="renew"?"续费申请":"订阅申请",o=e.teamName||e.username,i=a&&a.plan?a.plan:null,l=i?`${i.term}（￥${i.price} / ${i.days} 天）`:"未选择套餐",c=lt(a&&a.at?a.at:new Date().toISOString()),u=String(d&&d.subExtraText||"").trim(),f=he(d&&d.subQrCodes).filter(Boolean),g=`【${p}】${s}已收到：${o} · ${l}`,w=[`${o}，您好：`,"",`我们已收到您的「${s}」，信息如下（本邮件已同步抄送管理员）：`,"","【申请人信息】",`团队名称：${o}`,`登录账号：${e.username}`,`联系人：${e.contact||"（未填写）"}`,`联系邮箱：${e.email||"（未填写）"}`,`提交时间：${c}`,"","【订阅套餐】",`申请类型：${s}`,`套餐：${l}`,`留言：${a&&a.note||"（无）"}`,""];u&&w.push("【说明】",u,""),f.length&&(w.push("【收款二维码】"),f.forEach((y,I)=>w.push(`二维码 ${I+1}：${y}`)),w.push("")),w.push("提交后由超级管理员为您开通；开通成功后即可使用成员 / 生产方 / 客户管理等全部功能。","如需补充信息，可在系统内重新提交申请（会覆盖上一次）。","",p);let v=w.join(`
`),b=(y,I)=>`<tr><td style="padding:4px 12px 4px 0;color:#6b6b68;white-space:nowrap">${K(y)}</td><td style="padding:4px 0;color:#37352f">${K(I)}</td></tr>`,k=u?`<div style="margin:14px 0 6px;font-weight:600">说明</div><div style="background:#f7f7f5;border-radius:8px;padding:12px;white-space:pre-wrap">${K(u)}</div>`:"",T=f.length?'<div style="margin:16px 0 6px;font-weight:600">收款二维码</div>'+f.map((y,I)=>`<div style="display:inline-block;margin:0 12px 12px 0;text-align:center;vertical-align:top"><img src="${K(y)}" alt="收款二维码 ${I+1}" style="width:180px;height:180px;object-fit:contain;border:1px solid #e9e9e7;border-radius:8px;background:#fff"><div style="font-size:12px;color:#6b6b68;margin-top:4px">二维码 ${I+1}</div></div>`).join(""):"",O=`<div style="font-family:-apple-system,'Microsoft YaHei',sans-serif;font-size:14px;color:#37352f;line-height:1.7"><p>${K(o)}，您好：</p><p>我们已收到您的「<b>${K(s)}</b>」，信息如下（本邮件已同步抄送管理员）：</p><div style="margin:10px 0 6px;font-weight:600">申请人信息</div><table style="border-collapse:collapse;font-size:14px">`+b("团队名称",o)+b("登录账号",e.username)+b("联系人",e.contact||"（未填写）")+b("联系邮箱",e.email||"（未填写）")+b("提交时间",c)+'</table><div style="margin:14px 0 6px;font-weight:600">订阅套餐</div><table style="border-collapse:collapse;font-size:14px">'+b("申请类型",s)+b("套餐",l)+b("留言",a&&a.note||"（无）")+"</table>"+k+T+'<p style="margin-top:16px;color:#6b6b68;font-size:13px">提交后由超级管理员为您开通；开通成功后即可使用成员 / 生产方 / 客户管理等全部功能。<br>如需补充信息，可在系统内重新提交申请（会覆盖上一次）。</p></div>';return{subject:g,text:v,html:O}}async function Se(t,e,a){let d=await re(t),m=d.devMode||!d.provider&&be(a);if(!d.provider&&!m)return{ok:!1,error:"邮件服务未配置：请联系超级管理员在控制台「邮件设置」中配置发件服务后再试"};if(d.provider&&!d.from)return{ok:!1,error:"邮件服务缺少「发件邮箱地址」：请先联系超级管理员补全配置"};if(!e.email)return{ok:!1,error:"该账号没有可用的注册邮箱，请重新注册"};let p=Date.now(),r=`mailrate:${e.username}`,s=null;try{let c=await t.TODO_KV.get(r);s=c?JSON.parse(c):null}catch{s=null}let o=!!(s&&p-Number(s.windowStart||0)<60*60*1e3);if(o){let c=ue*1e3-(p-Number(s.lastSentAt||0));if(c>0)return{ok:!1,error:`确认码刚发送过，请 ${Math.ceil(c/1e3)} 秒后再点「重新发送确认码」`};if(Number(s.count||0)>=rt)return{ok:!1,error:"发送过于频繁（1 小时内最多 5 次），请稍后再试"}}let i=ct();if(m)console.log(`[邮箱确认码] ${e.username} <${e.email}> 确认码：${i}（调试模式，未真实发送邮件）`);else{let c=await ht(t,e,i),u=await Te(t,d,{to:e.email,...c},{local:be(a)});if(!u.ok)return{ok:!1,error:u.error}}await t.TODO_KV.put(`emailcode:${e.username}`,JSON.stringify({codeHash:await U(i),email:e.email||"",sentAt:p,attempts:0}),{expirationTtl:j});let l=o?{windowStart:s.windowStart,count:Number(s.count||0)+1,lastSentAt:p}:{windowStart:p,count:1,lastSentAt:p};return await t.TODO_KV.put(r,JSON.stringify(l),{expirationTtl:Math.max(60,Math.ceil((60*60*1e3-(p-Number(l.windowStart)))/1e3))}),{ok:!0,sentAt:p,devCode:m?i:""}}async function vt(t,e,a){try{let m=await t.TODO_KV.get(`emailcode:${e.username}`);if(m){let p=JSON.parse(m);if(j*1e3-(Date.now()-Number(p.sentAt||0))>0)return{ok:!0,resent:!1,sentAt:p.sentAt,devCode:""}}}catch{}let d=await Se(t,e,a);return d.ok?{ok:!0,resent:!0,sentAt:d.sentAt,devCode:d.devCode}:{ok:!1,error:d.error}}async function kt(t,e,a){let d=`emailcode:${e.username}`,m=String(a||"").trim();if(!m)return{ok:!1,error:"请输入邮箱确认码"};let p=await t.TODO_KV.get(d);if(!p)return{ok:!1,error:"确认码已失效，请点击「重新发送确认码」重新获取"};let r=null;try{r=JSON.parse(p)}catch{r=null}if(!r||!r.codeHash)return await t.TODO_KV.delete(d),{ok:!1,error:"确认码已失效，请点击「重新发送确认码」重新获取"};if(Date.now()-Number(r.sentAt||0)>j*1e3)return await t.TODO_KV.delete(d),{ok:!1,error:"确认码已过期（有效期 10 分钟），请点击「重新发送确认码」"};if(Number(r.attempts||0)>=$e)return await t.TODO_KV.delete(d),{ok:!1,error:"确认码错误次数过多，请点击「重新发送确认码」重新获取"};if(await U(m)!==r.codeHash){r.attempts=Number(r.attempts||0)+1;let s=$e-r.attempts;return s<=0?(await t.TODO_KV.delete(d),{ok:!1,error:"确认码错误次数过多，请点击「重新发送确认码」重新获取"}):(await t.TODO_KV.put(d,JSON.stringify(r),{expirationTtl:j}),{ok:!1,error:`确认码不正确，请检查后重试（还可尝试 ${s} 次）`})}return{ok:!0}}async function Ee(t,e){let a=R();return await t.TODO_KV.put(`session:${a}`,e.username,{expirationTtl:60*60*24*7}),new Response(JSON.stringify({ok:!0,username:e.username,role:e.role}),{status:200,headers:{"Content-Type":"application/json; charset=utf-8","Set-Cookie":`token=${a}; Path=/; HttpOnly; SameSite=Lax; Max-Age=${60*60*24*7}`}})}async function ce(t){let e=await t.TODO_KV.get("user:admin");if(!e){let d={username:"admin",password:await U("admin"),role:"superadmin",createdAt:new Date().toISOString()};await t.TODO_KV.put("user:admin",JSON.stringify(d));return}let a=JSON.parse(e);a.role==="admin"&&(a.role="superadmin",await t.TODO_KV.put("user:admin",JSON.stringify(a)))}async function qe(t,e){let a=await t.TODO_KV.list({prefix:"user:"}),d=[];for(let m of a.keys){let p=await t.TODO_KV.get(m.name);if(p){let r=JSON.parse(p);if(r.role==="producer"||r.role==="customer"||N(r.role)||E(r.role)||x(r)!==e)continue;let s={username:r.username,role:r.role,createdAt:r.createdAt,remark:r.remark||"",position:r.position||"",dept:r.dept||"",canPlaceOrder:xe(r),canPurchase:se(r),canViewCustomerOrder:Ne(r),canViewPurchaseOrder:Me(r),canUpdateStatus:pe(r),canViewAllOrders:oe(r),canPlaceMaskedOrder:fe(r),canAddShipDate:ge(r)};r.role==="restricted"&&(s.watched=await ne(t,r.username)),s.mentionsUnread=await Ot(t,r.username),d.push(s)}}return d}async function B(t,e,a){let d=await t.TODO_KV.get(`user:${e}`);if(!d)return null;let m=JSON.parse(d);return N(m.role)||x(m)!==a?null:m}async function Et(t){let e=await t.TODO_KV.list({prefix:"user:"}),a=[];for(let d of e.keys){let m=await t.TODO_KV.get(d.name);if(!m)continue;let p=JSON.parse(m);E(p.role)&&a.push({username:p.username,teamName:p.teamName||p.username,contact:p.contact||"",email:p.email||"",remark:p.remark||"",plan:ie(p)?"pro":"trial",pro:C(p),status:ae(p),rawStatus:p.status||"active",emailVerified:p.emailVerified!==!1,emailVerifiedAt:p.emailVerifiedAt||"",trialEndsAt:p.trialEndsAt||"",expiresAt:ie(p)&&p.expiresAt||"",createdAt:p.createdAt,renewedAt:p.renewedAt||"",subscribeRequest:Qe(p)})}return a.sort((d,m)=>{let p=d.subscribeRequest&&d.subscribeRequest.at?1:0,r=m.subscribeRequest&&m.subscribeRequest.at?1:0;return p!==r?r-p:d.createdAt<m.createdAt?1:-1}),a}async function V(t,e){let a=await t.TODO_KV.get(`todos:${e}`);return a?JSON.parse(a):[]}async function H(t,e,a){await t.TODO_KV.put(`todos:${e}`,JSON.stringify(a))}async function W(t,e){let a=await t.TODO_KV.get(`customers:${e}`);return a?JSON.parse(a):[]}async function Ie(t,e,a){await t.TODO_KV.put(`customers:${e}`,JSON.stringify(a))}async function q(t,e){let a=!e||e===Y?"customerList":`customerList:${e}`,d=await t.TODO_KV.get(a);return d?JSON.parse(d):[]}async function me(t,e,a){let d=!e||e===Y?"customerList":`customerList:${e}`;await t.TODO_KV.put(d,JSON.stringify(a))}async function Tt(t,e,a){let d=String(a||"").trim();if(!d)return;let m=await W(t,e.username);m.some(s=>s.name===d)||(m.push({id:R().slice(0,12),name:d,createdAt:new Date().toISOString()}),await Ie(t,e.username,m));let p=x(e),r=await q(t,p);r.some(s=>s.name===d)||(r.push({id:R().slice(0,12),name:d,description:"",createdAt:new Date().toISOString()}),await me(t,p,r))}var He=["self","purchased"],St="self";function Fe(t){let e=String(t??"").trim();return He.includes(e)?e:St}async function D(t,e){let a=!e||e===Y?"producers":`producers:${e}`,d=await t.TODO_KV.get(a);return(d?JSON.parse(d):[]).map(p=>({id:p.id,username:p.username||p.shortName||"",description:p.description||"",nature:Fe(p.nature),createdAt:p.createdAt}))}async function le(t,e,a){let d=!e||e===Y?"producers":`producers:${e}`;await t.TODO_KV.put(d,JSON.stringify(a))}async function ne(t,e){let a=await t.TODO_KV.get(`watch:${e}`);return a?JSON.parse(a):[]}async function Ke(t,e,a){await t.TODO_KV.put(`watch:${e}`,JSON.stringify(a))}var It=50;function Ct(t){let e=new Set,a=/@([A-Za-z0-9_.-]{3,20})/g,d=String(t||""),m;for(;(m=a.exec(d))!==null;)e.add(m[1]);return[...e]}async function je(t,e){let a=[],d=await qe(t,e);for(let m of d)a.push({username:m.username,role:m.role,position:m.position||"",dept:m.dept||"",isTeamAdmin:!1,label:m.username});return a}async function we(t,e){let a=await t.TODO_KV.get(`mentions:${e}`);return a?JSON.parse(a):[]}async function We(t,e,a){await t.TODO_KV.put(`mentions:${e}`,JSON.stringify(a.slice(0,It)))}async function Ge(t,e){let a=await we(t,e);return{unread:a.filter(d=>!d.read).length,total:a.length}}async function Ot(t,e){return(await Ge(t,e)).unread}async function Pt(t,e,a,d){if(!a||!a.length)return;let m=new Date().toISOString();for(let p of a){let r=await we(t,p);r.unshift({id:R().slice(0,12),from:e.username,todoId:d.todoId,todoTitle:d.todoTitle||"",todoOwner:d.owner||"",text:d.text||"",at:m,read:!1}),await We(t,p,r)}}var Nt={superadmin:"超级管理员",team:"团队管理员",editor:"成员",viewer:"业务主管",restricted:"生产部",producer:"生产方",customer:"客户",superviewer:"总经理",deptmanager:"部门主管"};function L(t){return t&&(t.position||t.dept||Nt[t.role]||t.role)||""}function Ce(t){return t==="viewer"||t==="restricted"||t==="producer"||t==="customer"}var Mt=["品质部","财务部"];function Ae(t){return!!t&&t.role==="restricted"&&Mt.includes(t.dept||"")}function P(t){return t==="editor"||t==="member"}function At(t){let e=t&&t.role;return P(e)?"添加订单":"生产单下单权限"}function xe(t){return t?E(t.role)||P(t.role)?!0:t.role==="superviewer"||ee(t.role)||Ae(t)?!1:typeof t.canPlaceOrder=="boolean"?t.canPlaceOrder:!1:!1}function se(t){return t?E(t.role)?!0:Ae(t)?!1:P(t.role)?t.canUpdateStatus===!0||t.canPurchase===!0:typeof t.canPurchase=="boolean"?t.canPurchase:xe(t):!1}async function Bt(t,e){let a=await W(t,e);return Array.isArray(a)&&a.length>0}async function Oe(t,e){return e?E(e.role)?!C(e):P(e.role)?Bt(t,e.username):xe(e):!1}function Be(t){return E(t)||t==="superviewer"||ee(t)}function pe(t){return t?E(t.role)||t.role==="superviewer"?!0:P(t.role)?t.canUpdateStatus===!0:!1:!1}function oe(t){return t?P(t.role)?t.canViewAllOrders!==!1:Be(t.role):!1}function Ut(t){return t?E(t.role)||t.role==="superviewer"?!0:P(t.role)&&t.canUpdateStatus===!0:!1}function Pe(t){return t?Be(t.role)?!0:P(t.role)&&t.canUpdateStatus===!0:!1}function Lt(t){return t?Ce(t.role)||Pe(t)?!0:P(t.role)?oe(t):!1:!1}function ee(t){return t==="deptmanager"}function Ne(t){return t?E(t.role)?!0:P(t.role)?t.canUpdateStatus===!0||t.canViewCustomerOrder===!0:ee(t.role)?t.canViewCustomerOrder!==!1:t.role==="producer"||t.role==="customer":!1}function Me(t){return t?E(t.role)?!0:P(t.role)?t.canUpdateStatus===!0||t.canViewPurchaseOrder===!0:ee(t.role)?t.canViewPurchaseOrder!==!1:t.role==="superviewer"?!0:se(t):!1}function fe(t){return t?E(t.role)?!0:P(t.role)?t.canPlaceMaskedOrder===!0:!1:!1}function ge(t){return t?E(t.role)?!0:P(t.role)?t.canAddShipDate===!0:!1:!1}async function F(t,e,a=!1,d=null){let m=await t.TODO_KV.list({prefix:"todos:"}),p=m.keys.map(o=>o.name.replace("todos:","")),r=await Promise.all(p.map(async o=>{let i=await t.TODO_KV.get(`user:${o}`);return i?JSON.parse(i):null})),s=[];for(let o=0;o<p.length;o++){let i=r[o];if(!i||x(i)!==e)continue;let l=await t.TODO_KV.get(m.keys[o].name),c=l?JSON.parse(l):[];for(let u of c){let f=u.status||(u.done?"done":"pending");a&&f==="pending"||d&&!d.includes(u.producerId)||s.push({...u,status:f,owner:p[o]})}}return s.sort((o,i)=>o.createdAt<i.createdAt?1:-1),s}async function Dt(t,e){let a=await t.TODO_KV.list({prefix:"user:"}),d=[];for(let m of a.keys){let p=await t.TODO_KV.get(m.name);if(!p)continue;let r=null;try{r=JSON.parse(p)}catch{continue}!r||N(r.role)||x(r)===e&&d.push({username:r.username||m.name.replace(/^user:/,""),role:r.role})}return d}async function $t(t,e){let a=0,d=await t.TODO_KV.get(`todos:${e}`);if(d)try{let m=JSON.parse(d);a=Array.isArray(m)?m.length:0}catch{a=0}return await t.TODO_KV.delete(`user:${e}`),await t.TODO_KV.delete(`todos:${e}`),await t.TODO_KV.delete(`customers:${e}`),await t.TODO_KV.delete(`watch:${e}`),await t.TODO_KV.delete(`mentions:${e}`),await t.TODO_KV.delete(`emailcode:${e}`),await t.TODO_KV.delete(`mailrate:${e}`),{todos:a}}async function Rt(t,e,a){let d=t.method;if(a==="/api/login"&&d==="POST"){await ce(e);let{username:r,password:s}=await S(t);if(!r||!s)return n({error:"请输入用户名和密码"},400);let o=await e.TODO_KV.get(`user:${r}`);if(!o)return n({error:"用户名或密码错误"},401);let i=JSON.parse(o);if(await U(s)!==i.password)return n({error:"用户名或密码错误"},401);if(!N(i.role)){let c=await $(e,x(i));if(c&&ae(c)==="disabled")return n({error:"该账户已被停用，请联系超级管理员开通"},403)}if(i.role==="team"&&i.emailVerified===!1){let c=await vt(e,i,t);return n({ok:!1,needVerify:!0,username:i.username,email:te(i.email||""),codeTtlSec:j,resendAfterSec:ue,devCode:c.devCode||void 0,sendError:c.ok?void 0:c.error,message:c.ok?`确认码${c.resent?"已重新发送":"已发送"}至注册邮箱，请在 ${Math.floor(j/60)} 分钟内输入完成确认`:"确认码发送失败，请稍后点「重新发送确认码」重试"})}return Ee(e,i)}if(a==="/api/register"&&d==="POST"){if(await ce(e),!(await G(e)).allowRegister)return n({error:"系统当前已关闭新用户注册，如需开通请联系超级管理员"},403);let{teamName:s,username:o,email:i,password:l,confirmPassword:c,contact:u,remark:f}=await S(t),g=(o||"").trim(),w=(s||"").trim(),v=String(i||"").trim(),b=String(l||"");if(!w)return n({error:"请输入团队名称"},400);if(!g)return n({error:"请输入登录用户名"},400);if(!/^[A-Za-z0-9_.-]{3,20}$/.test(g))return n({error:"用户名为 3~20 位字母、数字、下划线、点或短横线"},400);if(g==="admin")return n({error:"该用户名不可用，请更换"},400);if(!v)return n({error:"请输入邮箱"},400);if(v.length>60)return n({error:"邮箱长度不能超过 60 个字符"},400);if(!z.test(v))return n({error:"邮箱格式不正确，请检查后重试"},400);if(!b)return n({error:"请输入密码"},400);if(b.length<6)return n({error:"密码至少 6 位"},400);if(c!==void 0&&b!==String(c))return n({error:"两次输入的密码不一致"},400);if(await e.TODO_KV.get(`user:${g}`))return n({error:"该用户名已被占用，请更换"},400);let T=new Date,O={username:g,password:await U(b),role:"team",teamId:g,teamName:w,contact:String(u||"").trim(),email:v,remark:String(f||"").trim(),emailVerified:!1,plan:"trial",status:"active",trialEndsAt:"",expiresAt:"",createdAt:T.toISOString()};await e.TODO_KV.put(`user:${g}`,JSON.stringify(O));let y=await Se(e,O,t);return y.ok?n({ok:!0,needVerify:!0,username:g,role:"team",teamName:w,email:te(v),codeTtlSec:j,resendAfterSec:ue,devCode:y.devCode||void 0,unlimitedTrial:!0,message:`确认码已发送至 ${te(v)}，请在 ${Math.floor(j/60)} 分钟内输入完成邮箱确认`}):(await e.TODO_KV.delete(`user:${g}`),n({error:"注册未完成："+y.error},503))}if(a==="/api/register/verify"&&d==="POST"){await ce(e);let{username:r,code:s}=await S(t),o=String(r||"").trim();if(!o)return n({error:"请输入用户名"},400);let i=await e.TODO_KV.get(`user:${o}`);if(!i)return n({error:"用户名不存在，请先注册"},404);let l=JSON.parse(i);if(l.role!=="team")return n({error:"该账号无需邮箱确认"},400);if(l.emailVerified!==!1)return Ee(e,l);let c=await kt(e,l,s);return c.ok?(l.emailVerified=!0,l.emailVerifiedAt=new Date().toISOString(),await e.TODO_KV.put(`user:${o}`,JSON.stringify(l)),await e.TODO_KV.delete(`emailcode:${o}`),Ee(e,l)):n({error:c.error},400)}if(a==="/api/register/resend"&&d==="POST"){await ce(e);let{username:r,password:s}=await S(t),o=String(r||"").trim();if(!o)return n({error:"请输入用户名"},400);let i=await e.TODO_KV.get(`user:${o}`);if(!i)return n({error:"用户名不存在，请先注册"},404);let l=JSON.parse(i);if(l.role!=="team")return n({error:"该账号无需邮箱确认"},400);if(l.emailVerified!==!1)return n({error:"该账号已完成邮箱确认，可直接登录"},400);if(!s||await U(String(s))!==l.password)return n({error:"密码不正确，无法重新发送确认码"},401);let c=await Se(e,l,t);return c.ok?n({ok:!0,email:te(l.email||""),codeTtlSec:j,resendAfterSec:ue,devCode:c.devCode||void 0,message:`确认码已重新发送至 ${te(l.email||"")}`}):n({error:c.error},429)}if(a==="/api/logout"&&d==="POST"){let r=Je(t);return r&&await e.TODO_KV.delete(`session:${r}`),new Response(JSON.stringify({ok:!0}),{status:200,headers:{"Content-Type":"application/json; charset=utf-8","Set-Cookie":"token=; Path=/; HttpOnly; SameSite=Lax; Max-Age=0"}})}if(a==="/api/me"&&d==="GET"){let r=await h(t,e);if(!r)return n({error:"未登录"},401);let s={username:r.username,role:r.role};if(s.position=r.position||"",s.canPlaceOrder=await Oe(e,r),s.canPurchase=se(r),s.canViewCustomerOrder=Ne(r),s.canViewPurchaseOrder=Me(r),s.canUpdateStatus=pe(r),s.canViewAllOrders=oe(r),s.canPlaceMaskedOrder=fe(r),s.canAddShipDate=ge(r),E(r.role)){s.teamId=r.teamId||r.username,s.teamName=r.teamName||r.username,s.contact=r.contact||"",s.plan=ie(r)?"pro":"trial",s.pro=C(r),s.trialEndsAt=r.trialEndsAt||"",s.expiresAt=r.expiresAt||"",s.status=ae(r);let i=Qe(r);s.subscribeRequest=i,s.renewRequest=i}else{let i=await $(e,x(r));s.teamId=x(r),s.teamName=i?i.teamName||i.username:"",s.teamPro=C(i)}let o=await Ge(e,r.username);return s.mentionsUnread=o.unread,s.mentionsTotal=o.total,n(s)}if(a==="/api/change-password"&&d==="POST"){let r=await h(t,e);if(!r)return n({error:"未登录"},401);let{oldPassword:s,newPassword:o}=await S(t);return!s||!o?n({error:"请填写完整"},400):await U(s)!==r.password?n({error:"原密码错误"},400):(r.password=await U(o),await e.TODO_KV.put(`user:${r.username}`,JSON.stringify(r)),n({ok:!0}))}if(a==="/api/admin/password"&&d==="POST"){let r=await h(t,e);if(!r)return n({error:"未登录"},401);if(!N(r.role))return n({error:"无权限"},403);let{newPassword:s,confirmPassword:o}=await S(t),i=String(s||"");if(!i.trim())return n({error:"请输入新密码"},400);if(i.length<6)return n({error:"密码至少 6 位"},400);if(o!==void 0&&i!==String(o))return n({error:"两次输入的密码不一致"},400);let l=await e.TODO_KV.get(`user:${r.username}`);if(!l)return n({error:"账号不存在"},404);let c=JSON.parse(l);return c.password=await U(i),c.passwordUpdatedAt=new Date().toISOString(),await e.TODO_KV.put(`user:${r.username}`,JSON.stringify(c)),n({ok:!0,username:r.username})}if(a==="/api/settings"&&d==="GET"){let r=await h(t,e);if(!r)return n({error:"未登录"},401);let s=await G(e),o=E(r.role)?r:await $(e,x(r));return n({settings:{siteName:s.siteName,allowRegister:s.allowRegister,supportEmail:s.supportEmail,favicon:s.favicon,teamName:o?o.teamName||o.username:""}})}if(a==="/api/settings"&&d==="POST"){let r=await h(t,e);if(!r)return n({error:"未登录"},401);let{siteName:s,teamName:o,allowRegister:i,supportEmail:l,favicon:c}=await S(t);if(o!==void 0){if(!E(r.role))return n({error:"无权限"},403);let w=String(o).trim();return w?(r.teamName=w,await e.TODO_KV.put(`user:${r.username}`,JSON.stringify(r)),n({ok:!0,settings:{teamName:w}})):n({error:"请输入团队名称"},400)}if(s===void 0&&i===void 0&&l===void 0&&c===void 0)return n({error:"请填写要保存的内容"},400);if(!N(r.role))return n({error:"无权限"},403);let u=await e.TODO_KV.get("settings"),f=u?JSON.parse(u):{};if(s!==void 0){if(!s||!s.trim())return n({error:"请输入网站名称"},400);f.siteName=s.trim()}if(i!==void 0&&(f.allowRegister=i===!0||i==="true"),l!==void 0){let w=String(l||"").trim();if(w.length>60)return n({error:"联系邮箱长度不能超过 60 个字符"},400);if(w&&!z.test(w))return n({error:"联系邮箱格式不正确，请检查后重试（留空则不显示该提示）"},400);f.supportEmail=w}if(c!==void 0){let w=String(c||"").trim();if(w.length>300)return n({error:"网站图标链接不能超过 300 个字符"},400);let v=Q(w);if(v===null)return n({error:"网站图标需填写以 http:// 或 https:// 开头的图片链接（留空则使用默认图标）"},400);f.favicon=v}await e.TODO_KV.put("settings",JSON.stringify(f));let g=await G(e);return n({ok:!0,settings:{siteName:g.siteName,allowRegister:g.allowRegister,supportEmail:g.supportEmail,favicon:g.favicon}})}if((a==="/api/subscribe-request"||a==="/api/renew-request")&&d==="POST"){let r=await h(t,e);if(!r)return n({error:"未登录"},401);if(!E(r.role))return n({error:"无权限"},403);let{note:s,contact:o,plan:i}=await S(t);o!==void 0&&(r.contact=String(o||"").trim().slice(0,50));let l=i&&typeof i=="object"?i:null,c=l&&Number(l.days)>0?Math.floor(Number(l.days)):0,u={at:new Date().toISOString(),kind:C(r)?"renew":"subscribe",plan:l&&c?{id:String(l.id||"").slice(0,20),term:String(l.term||"").slice(0,20),price:Number(l.price)>=0?Number(l.price):0,days:c}:null,note:String(s||"").trim().slice(0,200),contact:r.contact||""};r.subscribeRequest=u,delete r.renewRequest,await e.TODO_KV.put(`user:${r.username}`,JSON.stringify(r));let f={ok:!1,to:r.email||"",cc:[],error:""};try{if(!r.email)f.error="该账号没有可用邮箱，无法发送申请邮件";else{let g=await re(e),w=await G(e),v=ut(g,w);if(f.cc=X({to:r.email,cc:v?[v]:[]}),g.devMode)f.ok=!0,f.devMode=!0;else{let b=await xt(e,r,u,g,u.kind),k=await Te(e,g,{to:r.email,cc:f.cc,subject:b.subject,text:b.text,html:b.html},{local:be(t)});f.ok=!!k.ok,k.ok||(f.error=k.error||"邮件发送失败")}}}catch(g){f.ok=!1,f.error="邮件发送异常："+(g&&g.message?g.message:String(g))}return console.log(`[订阅申请邮件] ${r.username} -> ${f.to||"-"}`+(f.cc&&f.cc.length?`（抄送 ${f.cc.join(", ")}）`:"")+` ${f.ok?"成功":"失败："+f.error}`),n({ok:!0,subscribeRequest:u,mail:f})}let m="/api/teams";if(a===m&&d==="GET"){let r=await h(t,e);return r?N(r.role)?n({teams:await Et(e)}):n({error:"无权限"},403):n({error:"未登录"},401)}if(a.startsWith(`${m}/`)&&a.endsWith("/status")&&d==="POST"){let r=await h(t,e);if(!r)return n({error:"未登录"},401);if(!N(r.role))return n({error:"无权限"},403);let s=decodeURIComponent(a.replace(`${m}/`,"").replace("/status","")),{status:o}=await S(t);if(!["active","disabled"].includes(o))return n({error:"无效的状态"},400);let i=await $(e,s);return i?(i.status=o,await e.TODO_KV.put(`user:${s}`,JSON.stringify(i)),n({ok:!0,status:ae(i),expiresAt:i.expiresAt||i.trialEndsAt||""})):n({error:"团队用户不存在"},404)}if(a.startsWith(`${m}/`)&&a.endsWith("/renew")&&d==="POST"){let r=await h(t,e);if(!r)return n({error:"未登录"},401);if(!N(r.role))return n({error:"无权限"},403);let s=decodeURIComponent(a.replace(`${m}/`,"").replace("/renew","")),o=await S(t),i=Number(o.days)>0?Math.floor(Number(o.days)):30;if(i>3650)return n({error:"续费天数过大"},400);let l=await $(e,s);if(!l)return n({error:"团队用户不存在"},404);let c=Date.now(),u=new Date(l.expiresAt||"").getTime(),f=Number.isNaN(u)||u<c?c:u,g=new Date(f+i*et).toISOString();return l.expiresAt=g,l.status="active",l.plan="pro",l.renewedAt=new Date().toISOString(),delete l.subscribeRequest,delete l.renewRequest,await e.TODO_KV.put(`user:${s}`,JSON.stringify(l)),n({ok:!0,days:i,expiresAt:g,status:"active",pro:!0})}if(a.startsWith(`${m}/`)&&a.endsWith("/reset-password")&&d==="POST"){let r=await h(t,e);if(!r)return n({error:"未登录"},401);if(!N(r.role))return n({error:"无权限"},403);let s=decodeURIComponent(a.replace(`${m}/`,"").replace("/reset-password","")),{newPassword:o}=await S(t);if(!o||!String(o).trim())return n({error:"请输入新密码"},400);let i=await $(e,s);return i?(i.password=await U(String(o).trim()),await e.TODO_KV.put(`user:${s}`,JSON.stringify(i)),n({ok:!0})):n({error:"团队用户不存在"},404)}if(a.startsWith(`${m}/`)&&a.endsWith("/remark")&&d==="POST"){let r=await h(t,e);if(!r)return n({error:"未登录"},401);if(!N(r.role))return n({error:"无权限"},403);let s=decodeURIComponent(a.replace(`${m}/`,"").replace("/remark","")),{remark:o}=await S(t),i=await $(e,s);return i?(i.remark=String(o||"").trim(),await e.TODO_KV.put(`user:${s}`,JSON.stringify(i)),n({ok:!0,remark:i.remark})):n({error:"团队用户不存在"},404)}if(a.startsWith(`${m}/`)&&d==="DELETE"){let r=await h(t,e);if(!r)return n({error:"未登录"},401);if(!N(r.role))return n({error:"无权限"},403);let s=decodeURIComponent(a.slice(m.length+1));if(!s||s.indexOf("/")!==-1)return n({error:"接口不存在"},404);let o=await $(e,s);if(!o)return n({error:"团队用户不存在"},404);if(o.status!=="disabled")return n({error:"请先「停用」该团队用户，确认无误后再删除"},400);let i=await Dt(e,s),l=i.reduce((g,w)=>(g[w.role]=(g[w.role]||0)+1,g),{}),c=0;for(let g of i){let w=await $t(e,g.username);c+=w.todos}let u=await D(e,s),f=await q(e,s);return await e.TODO_KV.delete(`producers:${s}`),await e.TODO_KV.delete(`customerList:${s}`),n({ok:!0,deleted:{team:s,teamName:o.teamName||s,accounts:i.length,members:i.filter(g=>!["team","producer","customer"].includes(g.role)).length,producers:u.length,customers:f.length,todos:c,roles:l}})}if(a==="/api/admin/email-settings"&&d==="GET"){let r=await h(t,e);if(!r)return n({error:"未登录"},401);if(!N(r.role))return n({error:"无权限"},403);let s=await re(e);return n({settings:ze(s,e)})}if(a==="/api/admin/email-settings"&&d==="POST"){let r=await h(t,e);if(!r)return n({error:"未登录"},401);if(!N(r.role))return n({error:"无权限"},403);let{from:s,fromName:o,apiKey:i,devMode:l,smtpHost:c,smtpPort:u,smtpUser:f,smtpPass:g,adminNotifyEmail:w,subExtraText:v,subQrCodes:b}=await S(t),k=await e.TODO_KV.get("emailSettings"),T=k?JSON.parse(k):{};if(s!==void 0){let y=String(s||"").trim();if(y&&!z.test(y))return n({error:"发件邮箱格式不正确，请检查后重试"},400);T.from=y}if(o!==void 0&&(T.fromName=String(o||"").trim().slice(0,30)),i!==void 0){let y=String(i||"").trim();y==="-"?delete T.apiKey:y&&(T.apiKey=y)}if(c!==void 0){let y=String(c||"").trim();if(y&&!/^[A-Za-z0-9.-]{1,60}$/.test(y))return n({error:"SMTP 服务器地址格式不正确（例如 smtp.qq.com）"},400);T.smtpHost=y||"smtp.qq.com"}if(u!==void 0){let y=String(u??"").trim(),I=Number(y)||0;if(y&&(I<1||I>65535))return n({error:"SMTP 端口需为 1~65535 的数字（QQ 邮箱用 465，或 587 + STARTTLS）"},400);T.smtpPort=I||465}if(f!==void 0){let y=String(f||"").trim();if(y&&!z.test(y))return n({error:"SMTP 账号需填写完整邮箱地址（例如 xxx@qq.com）"},400);T.smtpUser=y}if(g!==void 0){let y=String(g||"").trim();y==="-"?delete T.smtpPass:y&&(T.smtpPass=y)}if(l!==void 0&&(T.devMode=!!l),w!==void 0){let y=String(w||"").trim();if(y&&!z.test(y))return n({error:"管理员提醒邮箱格式不正确（留空则默认抄送到发件邮箱）"},400);T.adminNotifyEmail=y}if(v!==void 0){let y=String(v||"").replace(/\r\n/g,`
`).trim();if(y.length>1e3)return n({error:"自定义说明不能超过 1000 个字符"},400);T.subExtraText=y}if(b!==void 0){let y=Array.isArray(b)?b.slice(0,Z):[];if(y.length&&b.length>Z)return n({error:`收款二维码最多 ${Z} 个`},400);for(let _=0;_<y.length;_++){let A=String(y[_]||"").trim();if(A){if(A.length>300)return n({error:`收款二维码链接太长（第 ${_+1} 个，最多 300 个字符）`},400);if(Q(A)===null)return n({error:`收款二维码需填写以 http:// 或 https:// 开头的图片链接（第 ${_+1} 个，留空则不显示）`},400)}}let I=he(y);for(;I.length&&!I[I.length-1];)I.pop();T.subQrCodes=I}await e.TODO_KV.put("emailSettings",JSON.stringify(T));let O=await re(e);return n({ok:!0,settings:ze(O,e)})}if(a==="/api/admin/email-test"&&d==="POST"){let r=await h(t,e);if(!r)return n({error:"未登录"},401);if(!N(r.role))return n({error:"无权限"},403);let{to:s}=await S(t),o=String(s||"").trim();if(!o)return n({error:"请输入测试收件邮箱"},400);if(!z.test(o))return n({error:"测试收件邮箱格式不正确"},400);let i=await re(e);if(i.devMode)return n({ok:!0,devMode:!0,message:"当前为调试模式（不真实发送邮件），无需测试发信"});if(!i.provider)return n({error:"邮件服务未配置：请填写 QQ 邮箱 SMTP 账号与授权码（或 Resend API Key）并保存后，再发送测试邮件"},400);let l=i.provider==="smtp"?"QQ 邮箱 SMTP":i.provider==="resend"?"Resend":"Cloudflare 邮件绑定",c=await Te(e,i,{to:o,subject:"【测试】邮件服务配置正常",text:"这是一封测试邮件：收到本邮件说明「注册邮箱确认码」的发件服务已配置成功。",html:"<p>这是一封测试邮件：收到本邮件说明「注册邮箱确认码」的发件服务已配置成功。</p>"},{local:be(t)});return c.ok?n({ok:!0,message:`测试邮件已通过 ${l} 发送至 ${o}，请查收（若没收到，请检查垃圾邮件箱）`}):n({error:c.error},400)}if(a==="/api/users"&&d==="GET"){let r=await h(t,e);return r?E(r.role)?C(r)?n({users:await qe(e,x(r))}):M():n({error:"无权限"},403):n({error:"未登录"},401)}if(a==="/api/users"&&d==="POST"){let r=await h(t,e);if(!r)return n({error:"未登录"},401);if(!E(r.role))return n({error:"无权限"},403);if(!C(r))return M();let{username:s,password:o,position:i}=await S(t);if(!s||!o)return n({error:"请填写用户名和密码"},400);if(await e.TODO_KV.get(`user:${s}`))return n({error:"用户名已存在"},400);let c={username:s,password:await U(o),role:"editor",teamId:x(r),createdAt:new Date().toISOString()},u=String(i??"").trim();return u&&(c.position=u.slice(0,20)),await e.TODO_KV.put(`user:${s}`,JSON.stringify(c)),n({ok:!0})}if(a.startsWith("/api/users/")&&d==="DELETE"){let r=await h(t,e);if(!r)return n({error:"未登录"},401);if(!E(r.role))return n({error:"无权限"},403);if(!C(r))return M();let s=decodeURIComponent(a.replace("/api/users/",""));return await B(e,s,x(r))?(await e.TODO_KV.delete(`user:${s}`),await e.TODO_KV.delete(`todos:${s}`),await e.TODO_KV.delete(`customers:${s}`),await e.TODO_KV.delete(`watch:${s}`),await e.TODO_KV.delete(`mentions:${s}`),n({ok:!0})):n({error:"成员不存在"},404)}if(a.startsWith("/api/users/")&&a.endsWith("/reset-password")&&d==="POST"){let r=await h(t,e);if(!r)return n({error:"未登录"},401);if(!E(r.role))return n({error:"无权限"},403);if(!C(r))return M();let s=x(r),o=decodeURIComponent(a.replace("/api/users/","").replace("/reset-password","")),{newPassword:i}=await S(t);if(!i||!i.trim())return n({error:"请输入新密码"},400);let l=await e.TODO_KV.get(`user:${o}`);if(!l){let f=(await D(e,s)).find(v=>v.username===o);if(f)return await e.TODO_KV.put(`user:${o}`,JSON.stringify({username:o,password:await U(i),role:"producer",producerId:f.id,teamId:s,createdAt:new Date().toISOString()})),n({ok:!0,created:!0});let w=(await q(e,s)).find(v=>v.name===o);return w?(await e.TODO_KV.put(`user:${o}`,JSON.stringify({username:o,password:await U(i),role:"customer",customerId:w.id,customerName:w.name,teamId:s,createdAt:new Date().toISOString()})),n({ok:!0,created:!0})):n({error:"成员不存在"},404)}let c=JSON.parse(l);return N(c.role)||x(c)!==s?n({error:"成员不存在"},404):(c.password=await U(i),await e.TODO_KV.put(`user:${o}`,JSON.stringify(c)),n({ok:!0}))}if(a.startsWith("/api/users/")&&a.endsWith("/remark")&&d==="POST"){let r=await h(t,e);if(!r)return n({error:"未登录"},401);if(!E(r.role))return n({error:"无权限"},403);if(!C(r))return M();let s=decodeURIComponent(a.replace("/api/users/","").replace("/remark","")),{remark:o}=await S(t),i=await B(e,s,x(r));return i?(i.remark=String(o||"").trim(),await e.TODO_KV.put(`user:${s}`,JSON.stringify(i)),n({ok:!0,remark:i.remark})):n({error:"成员不存在"},404)}if(a.startsWith("/api/users/")&&a.endsWith("/position")&&d==="POST"){let r=await h(t,e);if(!r)return n({error:"未登录"},401);if(!E(r.role))return n({error:"无权限"},403);if(!C(r))return M();let s=decodeURIComponent(a.replace("/api/users/","").replace("/position","")),{position:o}=await S(t),i=await B(e,s,x(r));return i?(i.position=String(o||"").trim().slice(0,20),await e.TODO_KV.put(`user:${s}`,JSON.stringify(i)),n({ok:!0,position:i.position})):n({error:"成员不存在"},404)}if(a.startsWith("/api/users/")&&a.endsWith("/order-permission")&&d==="POST"){let r=await h(t,e);if(!r)return n({error:"未登录"},401);if(!E(r.role))return n({error:"无权限"},403);if(!C(r))return M();let s=decodeURIComponent(a.replace("/api/users/","").replace("/order-permission","")),o=await S(t),i=typeof o.canPlaceOrder=="boolean",l=typeof o.canPurchase=="boolean",c=typeof o.canViewCustomerOrder=="boolean",u=typeof o.canViewPurchaseOrder=="boolean",f=typeof o.canUpdateStatus=="boolean",g=typeof o.canViewAllOrders=="boolean",w=typeof o.canPlaceMaskedOrder=="boolean",v=typeof o.canAddShipDate=="boolean";if(!i&&!l&&!c&&!u&&!f&&!g&&!w&&!v)return n({error:"请传入 true（有）或 false（无）"},400);let b=await B(e,s,x(r));return b?i&&P(b.role)?n({error:"该成员的「添加订单」权限由「客户列表」自动决定（有客户即可添加订单），无需在此设置"},400):i&&(b.role==="superviewer"||ee(b.role))?n({error:L(b)+"成员不需要「生产单下单权限」（固定不录入订单），无需设置"},400):g&&!P(b.role)?n({error:"「是否可查看全部订单」仅适用于普通成员（原业务部）（当前成员："+L(b)+"）"},400):f&&!P(b.role)?n({error:"「是否可以更新订单状态」仅适用于普通成员（原业务部）（当前成员："+L(b)+"）"},400):w&&!P(b.role)?n({error:"「是否可以下脱敏订单」仅适用于普通成员（原业务部）（当前成员："+L(b)+"）"},400):v&&!P(b.role)?n({error:"「是否可以添加出货日期」仅适用于普通成员（原业务部）（当前成员："+L(b)+"）"},400):(c||u)&&!P(b.role)&&!ee(b.role)?n({error:"「是否可以查看客户订单 / 查看生产订单」仅适用于普通成员（原业务部）与「部门主管」成员（当前成员："+L(b)+"）"},400):Ae(b)&&(i||l)?n({error:L(b)+"成员不需要「"+(l?"下生产订单":"生产单下单权限")+"」（该部门无录入/下单需求），无需设置"},400):(i&&(b.canPlaceOrder=o.canPlaceOrder),l&&(b.canPurchase=o.canPurchase),c&&(b.canViewCustomerOrder=o.canViewCustomerOrder),u&&(b.canViewPurchaseOrder=o.canViewPurchaseOrder),f&&(b.canUpdateStatus=o.canUpdateStatus),g&&(b.canViewAllOrders=o.canViewAllOrders),w&&(b.canPlaceMaskedOrder=o.canPlaceMaskedOrder),v&&(b.canAddShipDate=o.canAddShipDate),await e.TODO_KV.put(`user:${s}`,JSON.stringify(b)),n({ok:!0,canPlaceOrder:xe(b),canPurchase:se(b),canViewCustomerOrder:Ne(b),canViewPurchaseOrder:Me(b),canUpdateStatus:pe(b),canViewAllOrders:oe(b),canPlaceMaskedOrder:fe(b),canAddShipDate:ge(b)})):n({error:"成员不存在"},404)}if(a.startsWith("/api/customers/")&&d==="GET"){let r=await h(t,e);if(!r)return n({error:"未登录"},401);let s=decodeURIComponent(a.replace("/api/customers/",""));return s!==r.username&&!(E(r.role)?await B(e,s,x(r)):null)?n({error:"无权限"},403):n({customers:await W(e,s)})}if(a.startsWith("/api/customers/")&&d==="POST"){let r=await h(t,e);if(!r)return n({error:"未登录"},401);if(!E(r.role))return n({error:"无权限"},403);if(!C(r))return M();let s=decodeURIComponent(a.replace("/api/customers/",""));if(!await B(e,s,x(r)))return n({error:"成员不存在"},404);let{name:i,globalId:l}=await S(t);if(!i||!i.trim())return n({error:"请输入客户名称"},400);let c=await W(e,s),u=i.trim(),f=l?String(l).trim():"";if(c.some(w=>w.name===u||f&&w.id===f))return n({error:"客户已存在"},400);let g={id:f||R().slice(0,12),name:u,createdAt:new Date().toISOString()};return c.push(g),await Ie(e,s,c),n({ok:!0,customer:g})}if(a.startsWith("/api/customers/")&&d==="DELETE"){let r=await h(t,e);if(!r)return n({error:"未登录"},401);if(!E(r.role))return n({error:"无权限"},403);if(!C(r))return M();let s=decodeURIComponent(a.replace("/api/customers/","")),o=s.lastIndexOf("/");if(o===-1)return n({error:"参数错误"},400);let i=s.slice(0,o),l=s.slice(o+1);if(!await B(e,i,x(r)))return n({error:"成员不存在"},404);let u=await W(e,i);return u=u.filter(f=>f.id!==l),await Ie(e,i,u),n({ok:!0})}if(a==="/api/customer-list"&&d==="GET"){let r=await h(t,e);return r?!E(r.role)||!C(r)?M():n({customers:await q(e,x(r))}):n({error:"未登录"},401)}if(a==="/api/customer-list"&&d==="POST"){let r=await h(t,e);if(!r)return n({error:"未登录"},401);if(!E(r.role))return n({error:"无权限"},403);let s=x(r),{name:o,password:i,description:l}=await S(t);if(!o||!o.trim())return n({error:"请输入用户名"},400);if(!i||!String(i).trim())return n({error:"请输入密码"},400);let c=await q(e,s),u=o.trim();if(c.some(w=>w.name===u))return n({error:"该用户名已存在"},400);if(await e.TODO_KV.get(`user:${u}`))return n({error:"该用户名已被占用"},400);let g={id:R().slice(0,12),name:u,description:(l||"").trim(),createdAt:new Date().toISOString()};return c.push(g),await me(e,s,c),await e.TODO_KV.put(`user:${u}`,JSON.stringify({username:u,password:await U(String(i).trim()),role:"customer",customerId:g.id,customerName:u,teamId:s,createdAt:g.createdAt})),n({ok:!0,customer:g})}if(a.startsWith("/api/customer-list/")&&d==="DELETE"){let r=await h(t,e);if(!r)return n({error:"未登录"},401);if(!E(r.role))return n({error:"无权限"},403);if(!C(r))return M();let s=x(r),o=decodeURIComponent(a.replace("/api/customer-list/","")),i=await q(e,s),l=i.find(c=>c.id===o);return i=i.filter(c=>c.id!==o),await me(e,s,i),l&&l.name&&await e.TODO_KV.delete(`user:${l.name}`),n({ok:!0})}if(a.startsWith("/api/customer-list/")&&a.endsWith("/description")&&d==="POST"){let r=await h(t,e);if(!r)return n({error:"未登录"},401);if(!E(r.role))return n({error:"无权限"},403);if(!C(r))return M();let s=x(r),o=decodeURIComponent(a.replace("/api/customer-list/","").replace("/description","")),{description:i}=await S(t),l=await q(e,s),c=l.find(u=>u.id===o);return c?(c.description=String(i||"").trim(),await me(e,s,l),n({ok:!0,description:c.description})):n({error:"客户不存在"},404)}if(a==="/api/producers"&&d==="GET"){let r=await h(t,e);return r?n({producers:await D(e,x(r))}):n({error:"未登录"},401)}if(a==="/api/producers"&&d==="POST"){let r=await h(t,e);if(!r)return n({error:"未登录"},401);if(!E(r.role))return n({error:"无权限"},403);if(!C(r))return M();let s=x(r),{username:o,password:i,description:l,nature:c}=await S(t),u=(o||"").trim();if(!u)return n({error:"请输入用户名"},400);if(!i||!String(i).trim())return n({error:"请输入密码"},400);let f=await D(e,s);if(f.some(v=>v.username===u))return n({error:"该用户名已存在"},400);if(await e.TODO_KV.get(`user:${u}`))return n({error:"该用户名已被成员占用"},400);let w={id:R().slice(0,12),username:u,description:(l||"").trim(),nature:Fe(c),createdAt:new Date().toISOString()};return f.push(w),await le(e,s,f),await e.TODO_KV.put(`user:${u}`,JSON.stringify({username:u,password:await U(String(i).trim()),role:"producer",producerId:w.id,teamId:s,createdAt:w.createdAt})),n({ok:!0,producer:w})}if(a.startsWith("/api/producers/")&&d==="DELETE"){let r=await h(t,e);if(!r)return n({error:"未登录"},401);if(!E(r.role))return n({error:"无权限"},403);if(!C(r))return M();let s=x(r),o=decodeURIComponent(a.replace("/api/producers/","")),i=await D(e,s),l=i.find(c=>c.id===o);return i=i.filter(c=>c.id!==o),await le(e,s,i),l&&l.username&&await e.TODO_KV.delete(`user:${l.username}`),n({ok:!0})}if(a.startsWith("/api/producers/")&&a.endsWith("/description")&&d==="POST"){let r=await h(t,e);if(!r)return n({error:"未登录"},401);if(!E(r.role))return n({error:"无权限"},403);if(!C(r))return M();let s=x(r),o=decodeURIComponent(a.replace("/api/producers/","").replace("/description","")),{description:i}=await S(t),l=await D(e,s),c=l.find(u=>u.id===o);return c?(c.description=String(i||"").trim(),await le(e,s,l),n({ok:!0,description:c.description})):n({error:"生产方不存在"},404)}if(a.startsWith("/api/producers/")&&a.endsWith("/nature")&&d==="POST"){let r=await h(t,e);if(!r)return n({error:"未登录"},401);if(!E(r.role))return n({error:"无权限"},403);if(!C(r))return M();let s=x(r),o=decodeURIComponent(a.replace("/api/producers/","").replace("/nature","")),{nature:i}=await S(t),l=String(i??"").trim();if(!He.includes(l))return n({error:"生产方性质只能是「自产」或「外购」"},400);let c=await D(e,s),u=c.find(f=>f.id===o);return u?(u.nature=l,await le(e,s,c),n({ok:!0,nature:u.nature})):n({error:"生产方不存在"},404)}let p="/api/watch/";if(a.startsWith(p)&&d==="GET"){let r=await h(t,e);if(!r)return n({error:"未登录"},401);let s=x(r),o=decodeURIComponent(a.replace(p,""));if(o!==r.username&&!(E(r.role)?await B(e,o,s):null))return n({error:"无权限"},403);let i=await ne(e,o),l=await D(e,s),c=i.map(u=>{let f=l.find(g=>g.id===u);return f||{id:u,username:"（已删除的生产方）",description:"",deleted:!0}});return n({producers:c})}if(a.startsWith(p)&&d==="POST"){let r=await h(t,e);if(!r)return n({error:"未登录"},401);if(!E(r.role))return n({error:"无权限"},403);if(!C(r))return M();let s=x(r),o=decodeURIComponent(a.replace(p,"")),i=await B(e,o,s);if(!i)return n({error:"成员不存在"},404);let{producerId:l}=await S(t);if(!l||!String(l).trim())return n({error:"请选择生产方"},400);let c=String(l).trim();if(!(await D(e,s)).some(g=>g.id===c))return n({error:"生产方不存在，请先在「生产方管理」中添加"},400);if(i.role!=="restricted")return n({error:"该成员不是「生产部」成员，无法授权可观察的生产方"},400);let f=await ne(e,o);return f.includes(c)?n({error:"已授权该生产方"},400):(f.push(c),await Ke(e,o,f),n({ok:!0,producerIds:f}))}if(a.startsWith(p)&&d==="DELETE"){let r=await h(t,e);if(!r)return n({error:"未登录"},401);if(!E(r.role))return n({error:"无权限"},403);if(!C(r))return M();let s=decodeURIComponent(a.replace(p,"")),o=s.lastIndexOf("/");if(o===-1)return n({error:"参数错误"},400);let i=s.slice(0,o),l=s.slice(o+1);if(!await B(e,i,x(r)))return n({error:"成员不存在"},404);let u=await ne(e,i);return u=u.filter(f=>f!==l),await Ke(e,i,u),n({ok:!0,producerIds:u})}if(a==="/api/team-members"&&d==="GET"){let r=await h(t,e);return r?n({members:await je(e,x(r))}):n({error:"未登录"},401)}if(a==="/api/mentions"&&d==="GET"){let r=await h(t,e);if(!r)return n({error:"未登录"},401);let s=await we(e,r.username);return n({items:s,unread:s.filter(o=>!o.read).length,total:s.length})}if(a==="/api/mentions/read"&&d==="POST"){let r=await h(t,e);if(!r)return n({error:"未登录"},401);let s=await S(t),o=s&&s.id?String(s.id).trim():"",i=!(s&&s.read===!1),l=await we(e,r.username),c=0;for(let u of l)u.read!==i&&(o&&u.id!==o||(u.read=i,c++));return c&&await We(e,r.username,l),n({ok:!0,changed:c,read:i,unread:l.filter(u=>!u.read).length,total:l.length})}if(a==="/api/todos"&&d==="GET"){let r=await h(t,e);if(!r)return n({error:"未登录"},401);let s=x(r);if(P(r.role)&&pe(r))return n({todos:await F(e,s,!1),readonly:!1,allUsers:!0});if(Be(r.role))return n({todos:await F(e,s,!1),readonly:!1,allUsers:!0});if(r.role==="viewer")return n({todos:await F(e,s,!0),readonly:!0,allUsers:!0});if(r.role==="restricted"){let i=await ne(e,r.username);return n({todos:await F(e,s,!0,i),readonly:!0,allUsers:!0})}if(r.role==="producer"){let l=(await D(e,s)).find(c=>c.id===r.producerId);return n({todos:await F(e,s,!0,l?[l.id]:[]),readonly:!0,allUsers:!0})}if(r.role==="customer"){let i=r.customerName||"",l=i?await F(e,s,!0):[];return n({todos:l.filter(c=>c.customer===i),readonly:!0,allUsers:!0})}if(P(r.role)&&!oe(r)){let i=await V(e,r.username);return n({todos:i.map(l=>Object.assign({},l,{owner:r.username})),readonly:!1})}if(P(r.role))return n({todos:await F(e,s,!1),readonly:!0,allUsers:!0});let o=await V(e,r.username);return n({todos:o.map(i=>Object.assign({},i,{owner:r.username})),readonly:!1})}if(a==="/api/todos"&&d==="POST"){let r=await h(t,e);if(!r)return n({error:"未登录"},401);if(!await Oe(e,r))return E(r.role)?n({error:"专业版团队账号不录入订单：请在「成员管理」中添加成员，由成员登录后录入订单"},403):P(r.role)?n({error:"暂无客户，请联系团队管理员在「成员管理」的「客户列表」中为你添加客户（有客户即可添加订单）"},403):n({error:L(r)+"无「"+At(r)+"」，请联系团队管理员在「成员管理」中开通"},403);let{title:s,customer:o,dueDate:i,amount:l,orderUrl:c,purchaseUrl:u,currency:f}=await S(t);if(!o||!o.trim())return n({error:"选择客户"},400);if(!s||!s.trim())return n({error:"请输入主题"},400);if(!i||!String(i).trim())return n({error:"选择交期"},400);let g=Q(c);if(g===null)return n({error:"订单文件链接需以 http:// 或 https:// 开头"},400);let w=Q(u);if(w===null)return n({error:"采购文件链接需以 http:// 或 https:// 开头"},400);if(l==null||String(l).trim()==="")return n({error:"输入金额"},400);let v=Number(l);if(Number.isNaN(v)||v<0)return n({error:"金额必须为非负数字"},400);let b=o.trim();if(E(r.role)&&!C(r))await Tt(e,r,b);else if(E(r.role)){let O=await q(e,x(r)),y=await W(e,r.username);if(!O.some(I=>I.name===b)&&!y.some(I=>I.name===b))return n({error:"客户不存在，请先在「客户管理」中添加"},400)}else if(!(await W(e,r.username)).some(y=>y.name===b))return n({error:"客户不存在，请联系管理员添加"},400);let k=await V(e,r.username),T={id:R().slice(0,12),customer:o.trim(),title:s.trim(),dueDate:String(i).trim(),amount:v,currency:it(f),orderUrl:g,purchaseUrl:w,notes:[],status:"pending",done:!1,createdAt:new Date().toISOString()};return k.unshift(T),await H(e,r.username,k),n({ok:!0,todo:T})}if(a.startsWith("/api/todos/")&&a.endsWith("/notes")&&d==="POST"){let r=await h(t,e);if(!r)return n({error:"未登录"},401);let s=decodeURIComponent(a.replace("/api/todos/","").replace("/notes","")),o=await S(t),i=o.text;if(!i||!i.trim())return n({error:"请输入备注内容"},400);let l=r.username;if(Lt(r)){let k=o.owner||r.username;if(k!==r.username&&!await B(e,k,x(r)))return n({error:"无权操作该用户的待办"},403);l=k}else if(o.owner&&o.owner!==r.username)return n({error:"无权操作该用户的待办"},403);let c=await V(e,l),u=c.findIndex(k=>k.id===s);if(u===-1)return n({error:"未找到"},404);Array.isArray(c[u].notes)||(c[u].notes=[]);let f=await je(e,x(r)),g=new Set(f.map(k=>k.username)),w=new Set;if(Array.isArray(o.mentions))for(let k of o.mentions){let T=String(k||"").trim();T&&g.has(T)&&w.add(T)}for(let k of Ct(i))g.has(k)&&w.add(k);w.delete(r.username);let v=[...w],b={id:R().slice(0,12),text:i.trim(),author:r.username,createdAt:new Date().toISOString(),mentions:v};return c[u].notes.push(b),await H(e,l,c),await Pt(e,r,v,{todoId:c[u].id,todoTitle:c[u].title,owner:l,text:b.text}),n({ok:!0,note:b})}if(a.startsWith("/api/todos/")&&d==="PUT"){let r=await h(t,e);if(!r)return n({error:"未登录"},401);if(Ce(r.role))return n({error:L(r)+"无编辑权限"},403);let s=decodeURIComponent(a.replace("/api/todos/","")),o=await S(t);if((typeof o.status=="string"||typeof o.done=="boolean")&&!Ut(r))return n({error:"只有团队管理员、总经理或拥有「更新订单状态」权限的成员可以改变订单状态"},403);let l=r.username;if(Pe(r)&&o.owner&&o.owner!==r.username){if(!await B(e,o.owner,x(r)))return n({error:"无权操作该用户的待办"},403);l=o.owner}let c=await V(e,l),u=c.findIndex(f=>f.id===s);if(u===-1)return n({error:"未找到"},404);if(typeof o.title=="string"){let f=o.title.trim();if(!f)return n({error:"请输入 PO# / 主题"},400);c[u].title=f}if(typeof o.dueDate=="string"||o.amount!==void 0||o.orderUrl!==void 0||o.purchaseUrl!==void 0){if((c[u].status||(c[u].done?"done":"pending"))!=="pending")return n({error:"仅「待确认」的待办可修改交期/金额/文件链接"},403);if(typeof o.dueDate=="string"){let g=o.dueDate.trim();if(!/^\d{4}-\d{2}-\d{2}$/.test(g))return n({error:"请输入正确的交期"},400);c[u].dueDate=g}if(o.amount!==void 0&&o.amount!==null&&String(o.amount).trim()!==""){let g=Number(o.amount);if(Number.isNaN(g)||g<0)return n({error:"金额必须为非负数字"},400);c[u].amount=g}if(o.orderUrl!==void 0){let g=Q(o.orderUrl);if(g===null)return n({error:"订单文件链接需以 http:// 或 https:// 开头"},400);c[u].orderUrl=g}if(o.purchaseUrl!==void 0){let g=Q(o.purchaseUrl);if(g===null)return n({error:"采购文件链接需以 http:// 或 https:// 开头"},400);c[u].purchaseUrl=g}}if(typeof o.producerId=="string"&&o.producerId.trim()){let g=(await D(e,x(r))).find(w=>w.id===o.producerId.trim());if(!g)return n({error:"生产方不存在，请先在「生产方管理」中添加"},400);c[u].producerId=g.id,c[u].producerName=g.username,c[u].producerAssignedAt=new Date().toISOString()}if(typeof o.status=="string"){if(!["pending","doing","done"].includes(o.status))return n({error:"无效的状态"},400);c[u].status=o.status,c[u].done=o.status==="done"}else typeof o.done=="boolean"&&(c[u].done=o.done,c[u].status=o.done?"done":"doing");return c[u].status==="doing"&&!c[u].producerId&&await dt(e,r)?n({error:"请为该待办指定生产方（生产方来自「生产方管理」）"},400):(await H(e,l,c),n({ok:!0,todo:c[u]}))}if(a.startsWith("/api/todos/")&&a.endsWith("/purchase-link")&&d==="POST"){let r=await h(t,e);if(!r)return n({error:"未登录"},401);if(!se(r))return n({error:L(r)+"无「下生产订单」权限，请联系团队管理员在「成员管理」中开通"},403);let s=decodeURIComponent(a.replace("/api/todos/","").replace("/purchase-link","")),o=await S(t),i=r.username;if(o.owner&&o.owner!==r.username){if(!await B(e,o.owner,x(r)))return n({error:"无权操作该用户的待办"},403);i=o.owner}let l=await V(e,i),c=l.findIndex(f=>f.id===s);if(c===-1)return n({error:"未找到"},404);if(l[c].purchaseUrl)return n({error:"该订单已有采购文件链接，如需修改请联系团队管理员"},403);let u=Q(o.purchaseUrl);return u===null?n({error:"采购文件链接需以 http:// 或 https:// 开头"},400):u?(l[c].purchaseUrl=u,await H(e,i,l),n({ok:!0,purchaseUrl:u})):n({error:"请输入采购文件链接"},400)}if(a.startsWith("/api/todos/")&&a.endsWith("/masked-link")&&d==="POST"){let r=await h(t,e);if(!r)return n({error:"未登录"},401);if(!fe(r))return n({error:L(r)+"无「下脱敏订单」权限，请联系团队管理员在「成员管理」中开通"},403);let s=decodeURIComponent(a.replace("/api/todos/","").replace("/masked-link","")),o=await S(t),i=r.username;if(o.owner&&o.owner!==r.username){if(!await B(e,o.owner,x(r)))return n({error:"无权操作该用户的待办"},403);i=o.owner}let l=await V(e,i),c=l.findIndex(f=>f.id===s);if(c===-1)return n({error:"未找到"},404);if(l[c].maskedUrl)return n({error:"该订单已有脱敏订单文件链接，填写后不可再修改"},403);let u=Q(o.maskedUrl);return u===null?n({error:"脱敏订单文件链接需以 http:// 或 https:// 开头"},400):u?(l[c].maskedUrl=u,await H(e,i,l),n({ok:!0,maskedUrl:u})):n({error:"请输入脱敏订单文件链接"},400)}if(a.startsWith("/api/todos/")&&a.endsWith("/ship-date")&&d==="POST"){let r=await h(t,e);if(!r)return n({error:"未登录"},401);if(!ge(r))return n({error:L(r)+"无「添加出货日期」权限，请联系团队管理员在「成员管理」中开通"},403);let s=decodeURIComponent(a.replace("/api/todos/","").replace("/ship-date","")),o=await S(t),i=r.username;if(o.owner&&o.owner!==r.username){if(!await B(e,o.owner,x(r)))return n({error:"无权操作该用户的待办"},403);i=o.owner}let l=await V(e,i),c=l.findIndex(f=>f.id===s);if(c===-1)return n({error:"未找到"},404);if(l[c].shipDate)return n({error:"该订单已添加出货日期，不能重复添加"},403);let u=String(o.shipDate||"").trim();return u?/^\d{4}-\d{2}-\d{2}$/.test(u)?(l[c].shipDate=u,await H(e,i,l),n({ok:!0,shipDate:u})):n({error:"请输入正确的出货日期"},400):n({error:"请选择出货日期"},400)}if(a.startsWith("/api/todos/")&&d==="DELETE"){let r=await h(t,e);if(!r)return n({error:"未登录"},401);if(Ce(r.role))return n({error:L(r)+"无删除权限"},403);let s=decodeURIComponent(a.replace("/api/todos/","")),o=r.username;if(Pe(r)){let u=new URL(t.url).searchParams.get("owner");if(u&&u!==r.username){if(!await B(e,u,x(r)))return n({error:"无权操作该用户的待办"},403);o=u}}let i=await V(e,o),l=i.find(u=>u.id===s);return l?(l.status||(l.done?"done":"pending"))!=="pending"?n({error:"该事件已进入「进行中/已完成」状态，无法删除"},403):(i=i.filter(u=>u.id!==s),await H(e,o,i),n({ok:!0})):n({error:"未找到"},404)}return n({error:"接口不存在"},404)}var Kt={async fetch(t,e,a){let d=new URL(t.url),m=d.pathname;if(m.startsWith("/api/"))try{return await Rt(t,e,m)}catch(p){return n({error:"服务器错误: "+p.message},500)}if(m==="/"||m==="/login"){let p=await G(e);return new Response(Ue(p.siteName,p.allowRegister,p.supportEmail,p.favicon),{headers:{"Content-Type":"text/html; charset=utf-8"}})}if(m==="/todos"){let p=await G(e),r=await h(t,e),s=r?await Oe(e,r):!1,o=await pt(e,r,p);return new Response(Le(p.favicon,s,o),{headers:{"Content-Type":"text/html; charset=utf-8"}})}if(m==="/admin"){let p=await h(t,e);if(!p)return Response.redirect(d.origin+"/",302);if(!N(p.role))return Response.redirect(d.origin+"/todos",302);let r=await G(e);return new Response(De(r.favicon,r.siteName),{headers:{"Content-Type":"text/html; charset=utf-8"}})}return new Response("Not Found",{status:404})}};export{Kt as default};
