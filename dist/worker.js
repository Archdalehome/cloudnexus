...
</html>`}function je(t,e,a,d){let m=e===!1,f=String(a??"").trim()||"订单管理系统",p=String(d??"").trim(),r=p?`
  <!-- 广告位（试用版）：广告代码由超级管理员在控制台「系统设置」中维护；订阅专业版后不再显示 -->
  <div class="ad-slot" id="adSlot">${p}</div>
`:"";return`<!DOCTYPE html>
<html lang="zh-CN">
<head>
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width, initial-scale=1.0">
<title>我的待办 - ${j(f)}</title>
${Ce(t)}
<style>
${Ie}
  .topbar {
...
</html>`}function Je(t,e){let a=String(e??"").trim()||"订单管理系统";return`<!DOCTYPE html>
<html lang="zh-CN">
<head>
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width, initial-scale=1.0">
...
</html>`}import{connect as lt}from"cloudflare:sockets";var Z="default",ut="__platform__",mt=24*60*60*1e3,ke="订单管理系统",W={members:2,producers:1,customers:1},pt={members:"成员",producers:"生产方",customers:"客户"};function Oe(t){let e=W[t]||0,a=pt[t]||t;return`试用版最多可添加 ${e} 个${a}：如需添加更多，请点击顶栏「订阅」升级为专业用户（订阅后解除数量限制）`}var V=/^[A-Za-z0-9._%+-]+@[A-Za-z0-9-]+(\.[A-Za-z0-9-]+)*\.[A-Za-z]{2,}$/,K=10*60,He=5,ge=60,ft=5,gt=1e4,ee=3,bt=2e4,wt=8e3;function _(){let t=new Uint8Array(24);return crypto.getRandomValues(t),Array.from(t,e=>e.toString(16).padStart(2,"0")).join("")}async function U(t){let e=new TextEncoder().encode(t+"::cf-todolist-salt"),a=await crypto.subtle.digest("SHA-256",e);return Array.from(new Uint8Array(a),d=>d.toString(16).padStart(2,"0")).join("")}function J(t){let e=String(t??"").trim();return e?!/^https?:\/\//i.test(e)||/\s/.test(e)?null:e:""}var yt=["USD","CNY"];function ht(t){let e=String(t??"").trim().toUpperCase();return yt.includes(e)?e:"USD"}function n(t,e=200){return new Response(JSON.stringify(t),{status:e,headers:{"Content-Type":"application/json; charset=utf-8"}})}async function I(t){try{return await t.json()}catch{return{}}}function tt(t){let a=(t.headers.get("Cookie")||"").match(/(?:^|;\s*)token=([^;]+)/);return a?a[1]:null}function v(t){return t?A(t.role)?ut:t.teamId||Z:Z}function A(t){return t==="superadmin"}function T(t){return t==="team"}function ue(t){return!!t&&(t.plan==="pro"||t.plan==="paid")}function ce(t){if(!t)return"active";if(t.status==="disabled")return"disabled";if(ue(t)){let e=t.expiresAt||"";if(e&&new Date(e).getTime()<Date.now())return"expired"}return"active"}function $(t){return ue(t)&&ce(t)==="active"}function rt(t){return t&&(t.subscribeRequest||t.renewRequest)||null}async function D(t,e){if(!e||e===Z)return null;let a=await t.TODO_KV.get(`user:${e}`);if(!a)return null;let d=JSON.parse(a);return T(d.role)?d:null}async function nt(t,e){return e?T(e.role)?$(e):$(await D(t,v(e))):!1}async function x(t,e){let a=tt(t);if(!a)return null;let d=await e.TODO_KV.get(`session:${a}`);if(!d)return null;let m=await e.TODO_KV.get(`user:${d}`);if(!m)return null;let f=JSON.parse(m);if(!A(f.role)){let p=await D(e,v(f));if(p&&p.status==="disabled")return null}return f}function xt(){let t=new Uint32Array(1);return crypto.getRandomValues(t),String(t[0]%1e6).padStart(6,"0")}function z(t){return String(t??"").replace(/[&<>"']/g,e=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[e])}function ae(t){let e=String(t||"").trim(),a=e.lastIndexOf("@");if(a<=0)return e;let d=e.slice(0,a),m=d.slice(0,Math.min(2,d.length));return m+"*".repeat(Math.max(1,d.length-m.length))+e.slice(a)}function xe(t){try{let e=new URL(t.url).hostname;return e==="localhost"||e==="127.0.0.1"||e==="::1"||e==="[::1]"}catch{return!1}}function vt(t){let e=new Date(t||Date.now());if(Number.isNaN(e.getTime()))return String(t||"");let a=new Date(e.getTime()+8*60*60*1e3),d=m=>String(m).padStart(2,"0");return`${a.getUTCFullYear()}-${d(a.getUTCMonth()+1)}-${d(a.getUTCDate())} ${d(a.getUTCHours())}:${d(a.getUTCMinutes())}`}function Te(t){let e=Array.isArray(t)?t.slice(0,ee):[],a=[];for(let d=0;d<ee;d++){let m=String(e[d]||"").trim();a.push(m&&m.length<=300&&!/\s/.test(m)&&/^https?:\/\//i.test(m)?m:"")}return a}function te(t){let e=String(t&&t.to||"").trim().toLowerCase(),a=t&&t.cc!==void 0&&t.cc!==null?t.cc:[],d=Array.isArray(a)?a:[a],m=[];for(let f of d){let p=String(f||"").trim();!p||!V.test(p)||p.toLowerCase()!==e&&(m.some(r=>r.toLowerCase()===p.toLowerCase())||m.push(p))}return m}function kt(t,e){let a=[t&&t.adminNotifyEmail,t&&t.from,e&&e.supportEmail];for(let d of a){let m=String(d||"").trim();if(m&&V.test(m))return m}return""}async function se(t){let e={};try{let b=await t.TODO_KV.get("emailSettings");b&&(e=JSON.parse(b)||{})}catch{e={}}let a=String(t.RESEND_API_KEY||e.apiKey||"").trim(),d=String(t.MAIL_FROM_NAME||e.fromName||"").trim(),m=String(t.SMTP_HOST||e.smtpHost||"smtp.qq.com").trim(),f=Number(t.SMTP_PORT||e.smtpPort||465)||465,p=String(t.SMTP_USER||t.QQ_MAIL_USER||e.smtpUser||"").trim(),r=String(t.SMTP_PASS||t.QQ_MAIL_PASS||e.smtpPass||"").trim(),s=String(t.MAIL_FROM||e.from||"").trim(),o=t.SEND_EMAIL&&typeof t.SEND_EMAIL.send=="function"?t.SEND_EMAIL:null,i=s||(p&&V.test(p)?p:""),l=a?"resend":p&&r?"smtp":o?"cloudflare":"",c=String(t.ADMIN_NOTIFY_EMAIL||e.adminNotifyEmail||"").trim(),u=Te(e.subQrCodes),g=a?t.RESEND_API_KEY?"env":"kv":p&&r?t.SMTP_USER||t.SMTP_PASS||t.QQ_MAIL_USER||t.QQ_MAIL_PASS?"env":"kv":o?"binding":"none";return{provider:l,apiKey:a,from:i,fromName:d,smtpHost:m,smtpPort:f,smtpUser:p,smtpPass:r,binding:o,adminNotifyEmail:c,subExtraText:String(e.subExtraText||"").trim(),subQrCodes:u,devMode:e.devMode===!0,source:g}}function me(t){let e=new TextEncoder().encode(String(t??"")),a="";for(let d=0;d<e.length;d++)a+=String.fromCharCode(e[d]);return btoa(a)}function qe(t){let e=String(t||""),a=[];for(let d=0;d<e.length;d+=76)a.push(e.slice(d,d+76));return a.join(`\r
`)}function Qe(t){let e=String(t??"");return/^[\x20-\x7e]*$/.test(e)?e:`=?UTF-8?B?${me(e)}?=`}var Tt="support@cloudnexus.cn";async function G(t){let e={};try{let a=await t.TODO_KV.get("settings");a&&(e=JSON.parse(a)||{})}catch{e={}}return{siteName:e.siteName||ke,allowRegister:e.allowRegister!==!1,supportEmail:e.supportEmail===void 0?Tt:String(e.supportEmail||"").trim(),favicon:String(e.favicon||"").trim(),adCode:String(e.adCode||"").trim()}}async function Et(t,e,a){let d="";if(e){let f=T(e.role)?e:await D(t,v(e));d=f?f.teamName||f.username:""}let m=a?String(a.siteName||"").trim():"";return d||m||ke}function Fe(t){let e=String(t||"");return e?e.slice(0,4)+"****"+e.slice(-4):""}function We(t,e){return{provider:t.provider,source:t.source,from:t.from,fromName:t.fromName,hasApiKey:!!t.apiKey,apiKeyMasked:Fe(t.apiKey),smtpHost:t.smtpHost,smtpPort:t.smtpPort,smtpUser:t.smtpUser,hasSmtpPass:!!t.smtpPass,smtpPassMasked:Fe(t.smtpPass),devMode:t.devMode,adminNotifyEmail:t.adminNotifyEmail||"",subExtraText:t.subExtraText||"",subQrCodes:Te(t.subQrCodes).concat("").slice(0,ee),envApiKey:!!e.RESEND_API_KEY,envFrom:!!e.MAIL_FROM,envSmtp:!!(e.SMTP_USER||e.SMTP_PASS||e.QQ_MAIL_USER||e.QQ_MAIL_PASS)}}function St(t,e){let a="cfmail_"+_().slice(0,16),d=String(t.from||"localhost").split("@")[1]||"localhost",m=t.fromName?`${Qe(t.fromName)} <${t.from}>`:`<${t.from}>`,f=te(e),p=[`From: ${m}`,`To: <${e.to}>`];return f.length&&p.push(`Cc: ${f.map(r=>`<${r}>`).join(", ")}`),p.push(`Subject: ${Qe(e.subject||"")}`,`Date: ${new Date().toUTCString()}`,`Message-ID: <${_()}@${d}>`,"MIME-Version: 1.0",`Content-Type: multipart/alternative; boundary="${a}"`,""),p.concat([`--${a}`,"Content-Type: text/plain; charset=UTF-8","Content-Transfer-Encoding: base64","",qe(me(e.text||"")),`--${a}`,"Content-Type: text/html; charset=UTF-8","Content-Transfer-Encoding: base64","",qe(me(e.html||e.text||"")),`--${a}--`,""]).join(`\r
`)}function It(t,e,a){let d=null;return Promise.race([t,new Promise((m,f)=>{d=setTimeout(()=>{try{a&&a()}catch{}f(new Error(`连接或响应超时（${Math.round(e/1e3)} 秒）`))},e)})]).finally(()=>clearTimeout(d))}function Ct(t,e,a){let d=t&&t.message?t.message:String(t),m=(d.match(/\b(\d{3})\b/)||[])[1]||"",f="（提示：本地 `wrangler dev` 已支持 SMTP 的 TLS；如持续超时请检查网络 / 端口是否被拦截，或先用「调试模式」联调）",p="";return m==="535"||m==="534"?p="（提示：QQ 邮箱必须使用「SMTP 授权码」而不是 QQ 登录密码，并先在 QQ 邮箱「设置 → 账户」中开启 SMTP 服务）":m==="530"?p="（提示：服务器要求先建立加密连接 —— 465 用 SSL、587 用 STARTTLS）"+(a?f:""):m==="550"||m==="553"?p="（提示：QQ 邮箱要求「发件邮箱」与 SMTP 账号完全一致）":a&&/secureTransport|starttls|tls|ssl|certificate|handshake|超时|关闭/i.test(d)?p=f:/starttls must be set|secureTransport/i.test(d)?p="（提示：587 必须先以 STARTTLS 建立连接再升级；若持续失败请把端口改为 465（SSL）重试）":/certificate|tls|ssl|handshake/i.test(d)?p="（提示：端口 465 使用 SSL，端口 587 使用 STARTTLS，请检查服务器 / 端口设置）":/超时/.test(d)&&(p="（提示：连接超时通常是网络或端口被拦截所致；QQ 邮箱请用 smtp.qq.com 的 465（SSL）或 587（STARTTLS））"),`SMTP 发信失败（${e}）：${d}${p}`}async function Ot(t,e,a){let d=Number(t.smtpPort)||465,m=[d];d===465?m.push(587):d===587&&m.push(465);let f=[];for(let p=0;p<m.length;p++){let r=await Pt(m[p],t,e,a);if(r.ok)return r;f.push(m.length>1?`端口 ${m[p]}：${r.error}`:r.error)}return{ok:!1,error:f.join("；")}}async function Pt(t,e,a,d){let m=e.smtpHost||"smtp.qq.com",f=!!(d&&d.local),p=Number(e.smtpTimeoutMs)||(f?wt:bt),r=new TextEncoder,s=new TextDecoder,o=null,i=null,l=null,c="",u=`连接 ${m}:${t}`;async function g(){for(;;){let w=c.indexOf(`
`);if(w!==-1){let N=c.slice(0,w);return c=c.slice(w+1),N.replace(/\r$/,"")}let{value:S,done:E}=await i.read();if(E)throw new Error("连接已被服务器关闭");c+=s.decode(S,{stream:!0})}}async function b(){let w=0,S="";for(;;){let E=await g();if(!(E.length<3)&&(w=parseInt(E.slice(0,3),10)||w,S+=(S?" ":"")+E.slice(4).trim(),E.charAt(3)!=="-"))break}return{code:w,text:S}}async function h(w,S){await l.write(r.encode(w+`\r
`));let E=await b();if(!S.includes(E.code))throw new Error(`服务器返回 ${E.code}${E.text?" "+E.text:""}`);return E}async function y(){u=`连接 ${m}:${t}`;try{await o.opened}catch(O){throw new Error(`建立连接失败：${O&&O.message?O.message:O}`)}let w=await b();if(w.code!==220)throw new Error(`服务器返回 ${w.code} ${w.text}`);let E=`EHLO ${e.smtpEhlo||String(e.from||"").split("@")[1]||"localhost"}`;if(await h(E,[250]),t===587){u="STARTTLS",await h("STARTTLS",[220]);try{i.releaseLock()}catch{}try{l.releaseLock()}catch{}o=o.startTls(),c="",i=o.readable.getReader(),l=o.writable.getWriter(),await h(E,[250])}u="身份认证（SMTP 授权码）",await h("AUTH LOGIN",[334]),await h(me(e.smtpUser),[334]),await h(me(e.smtpPass),[235]),u="发件地址",await h(`MAIL FROM:<${e.from}>`,[250]);let N=[a.to].concat(te(a));for(let O=0;O<N.length;O++)u=O===0?"收件地址":"抄送地址",await h(`RCPT TO:<${N[O]}>`,[250,251]);u="发送邮件正文",await h("DATA",[354]);let k=St(e,a).replace(/\r\n\./g,`\r
..`);await l.write(r.encode(`${k}\r
.\r
`));let M=await b();if(M.code!==250)throw new Error(`服务器返回 ${M.code}${M.text?" "+M.text:""}`);try{await h("QUIT",[221])}catch{}return{ok:!0,id:""}}try{return o=lt({hostname:m,port:t},{secureTransport:t===587?"starttls":"on"}),i=o.readable.getReader(),l=o.writable.getWriter(),await It(y(),p,()=>{try{o.close()}catch{}})}catch(w){return{ok:!1,error:Ct(w,u,f)}}finally{try{o&&o.close()}catch{}}}async function Ae(t,e,a,d){if(!e.provider)return{ok:!1,error:"邮件服务未配置：请在控制台「邮件设置」中填写 QQ 邮箱 SMTP 账号与授权码（或 Resend API Key）"};if(!e.from)return{ok:!1,error:"邮件服务缺少「发件邮箱地址」：请在控制台「邮件设置」中补全后重试"};if(e.provider==="smtp")return Ot(e,a,d);if(e.provider==="resend"){let m=new AbortController,f=setTimeout(()=>m.abort(),gt);try{let p=await fetch("https://api.resend.com/emails",{method:"POST",headers:{Authorization:`Bearer ${e.apiKey}`,"Content-Type":"application/json"},body:JSON.stringify(Object.assign({from:e.fromName?`${e.fromName} <${e.from}>`:e.from,to:[a.to],subject:a.subject,text:a.text,html:a.html},te(a).length?{cc:te(a)}:{})),signal:m.signal}),r=await p.json().catch(()=>({}));return p.ok?{ok:!0,id:r.id||""}:{ok:!1,error:`邮件服务返回错误（${p.status}）：${r.message||r.error||"请检查 API Key 与发件域名"}`}}catch(p){return{ok:!1,error:"邮件服务请求失败："+(p&&p.message?p.message:String(p))}}finally{clearTimeout(f)}}try{let m=te(a);return await e.binding.send({from:e.fromName?{name:e.fromName,email:e.from}:e.from,to:m.length?[a.to].concat(m):a.to,subject:a.subject,html:a.html,text:a.text}),{ok:!0,id:""}}catch(m){return{ok:!1,error:"邮件发送失败："+(m&&m.message?m.message:String(m))}}}async function Nt(t,e,a){let d=ke;try{let s=await t.TODO_KV.get("settings");if(s){let o=JSON.parse(s);o&&o.siteName&&(d=o.siteName)}}catch{}let m=e.teamName||e.username,f=`【${d}】邮箱确认码 ${a}（10 分钟内有效）`,p=[`${m}，您好：`,"",`您正在注册「${d}」的团队账号（登录名：${e.username}）。`,`邮箱确认码：${a}`,"有效期：10 分钟。","","请在注册 / 登录页面输入上面的确认码完成验证，验证通过后即可正常登录。","若非本人操作，请忽略本邮件：验证通过前该账号无法登录。","",d].join(`
`),r=`<div style="font-family:-apple-system,'Microsoft YaHei',sans-serif;font-size:14px;color:#37352f;line-height:1.7"><p>${z(m)}，您好：</p><p>您正在注册「${z(d)}」的团队账号（登录名：<b>${z(e.username)}</b>）。</p><p>邮箱确认码：</p><p style="font-size:26px;font-weight:700;letter-spacing:6px;color:#2383e2;margin:8px 0">${a}</p><p>有效期 <b>10 分钟</b>。请在注册 / 登录页面输入该确认码完成验证，验证通过后即可正常登录。</p><p style="color:#9b9a97;font-size:12px">若非本人操作，请忽略本邮件：验证通过前该账号无法登录。</p></div>`;return{subject:f,text:p,html:r}}async function At(t,e,a,d,m){let f=ke;try{let N=await t.TODO_KV.get("settings");if(N){let k=JSON.parse(N);k&&k.siteName&&(f=k.siteName)}}catch{}let r=m==="renew"?"续费申请":"订阅申请",s=e.teamName||e.username,o=a&&a.plan?a.plan:null,i=o?`${o.term}（￥${o.price} / ${o.days} 天）`:"未选择套餐",l=vt(a&&a.at?a.at:new Date().toISOString()),c=String(d&&d.subExtraText||"").trim(),u=Te(d&&d.subQrCodes).filter(Boolean),g=`【${f}】${r}已收到：${s} · ${i}`,b=[`${s}，您好：`,"",`我们已收到您的「${r}」，信息如下（本邮件已同步抄送管理员）：`,"","【申请人信息】",`团队名称：${s}`,`登录账号：${e.username}`,`联系人：${e.contact||"（未填写）"}`,`联系邮箱：${e.email||"（未填写）"}`,`提交时间：${l}`,"","【订阅套餐】",`申请类型：${r}`,`套餐：${i}`,`留言：${a&&a.note||"（无）"}`,""];c&&b.push("【说明】",c,""),u.length&&(b.push("【收款二维码】"),u.forEach((N,k)=>b.push(`二维码 ${k+1}：${N}`)),b.push("")),b.push("提交后由超级管理员为您开通；开通成功后即可使用成员 / 生产方 / 客户管理等全部功能。","如需补充信息，可在系统内重新提交申请（会覆盖上一次）。","",f);let h=b.join(`
`),y=(N,k)=>`<tr><td style="padding:4px 12px 4px 0;color:#6b6b68;white-space:nowrap">${z(N)}</td><td style="padding:4px 0;color:#37352f">${z(k)}</td></tr>`,w=c?`<div style="margin:14px 0 6px;font-weight:600">说明</div><div style="background:#f7f7f5;border-radius:8px;padding:12px;white-space:pre-wrap">${z(c)}</div>`:"",S=u.length?'<div style="margin:16px 0 6px;font-weight:600">收款二维码</div>'+u.map((N,k)=>`<div style="display:inline-block;margin:0 12px 12px 0;text-align:center;vertical-align:top"><img src="${z(N)}" alt="收款二维码 ${k+1}" style="width:180px;height:180px;object-fit:contain;border:1px solid #e9e9e7;border-radius:8px;background:#fff"><div style="font-size:12px;color:#6b6b68;margin-top:4px">二维码 ${k+1}</div></div>`).join(""):"",E=`<div style="font-family:-apple-system,'Microsoft YaHei',sans-serif;font-size:14px;color:#37352f;line-height:1.7"><p>${z(s)}，您好：</p><p>我们已收到您的「<b>${z(r)}</b>」，信息如下（本邮件已同步抄送管理员）：</p><div style="margin:10px 0 6px;font-weight:600">申请人信息</div><table style="border-collapse:collapse;font-size:14px">`+y("团队名称",s)+y("登录账号",e.username)+y("联系人",e.contact||"（未填写）")+y("联系邮箱",e.email||"（未填写）")+y("提交时间",l)+'</table><div style="margin:14px 0 6px;font-weight:600">订阅套餐</div><table style="border-collapse:collapse;font-size:14px">'+y("申请类型",r)+y("套餐",i)+y("留言",a&&a.note||"（无）")+"</table>"+w+S+'<p style="margin-top:16px;color:#6b6b68;font-size:13px">提交后由超级管理员为您开通；开通成功后即可使用成员 / 生产方 / 客户管理等全部功能。<br>如需补充信息，可在系统内重新提交申请（会覆盖上一次）。</p></div>';return{subject:g,text:h,html:E}}async function Me(t,e,a){let d=await se(t),m=d.devMode||!d.provider&&xe(a);if(!d.provider&&!m)return{ok:!1,error:"邮件服务未配置：请联系超级管理员在控制台「邮件设置」中配置发件服务后再试"};if(d.provider&&!d.from)return{ok:!1,error:"邮件服务缺少「发件邮箱地址」：请先联系超级管理员补全配置"};if(!e.email)return{ok:!1,error:"该账号没有可用的注册邮箱，请重新注册"};let f=Date.now(),p=`mailrate:${e.username}`,r=null;try{let l=await t.TODO_KV.get(p);r=l?JSON.parse(l):null}catch{r=null}let s=!!(r&&f-Number(r.windowStart||0)<60*60*1e3);if(s){let l=ge*1e3-(f-Number(r.lastSentAt||0));if(l>0)return{ok:!1,error:`确认码刚发送过，请 ${Math.ceil(l/1e3)} 秒后再点「重新发送确认码」`};if(Number(r.count||0)>=ft)return{ok:!1,error:"发送过于频繁（1 小时内最多 5 次），请稍后再试"}}let o=xt();if(m)console.log(`[邮箱确认码] ${e.username} <${e.email}> 确认码：${o}（调试模式，未真实发送邮件）`);else{let l=await Nt(t,e,o),c=await Ae(t,d,{to:e.email,...l},{local:xe(a)});if(!c.ok)return{ok:!1,error:c.error}}await t.TODO_KV.put(`emailcode:${e.username}`,JSON.stringify({codeHash:await U(o),email:e.email||"",sentAt:f,attempts:0}),{expirationTtl:K});let i=s?{windowStart:r.windowStart,count:Number(r.count||0)+1,lastSentAt:f}:{windowStart:f,count:1,lastSentAt:f};return await t.TODO_KV.put(p,JSON.stringify(i),{expirationTtl:Math.max(60,Math.ceil((60*60*1e3-(f-Number(i.windowStart)))/1e3))}),{ok:!0,sentAt:f,devCode:m?o:""}}async function Mt(t,e,a){try{let m=await t.TODO_KV.get(`emailcode:${e.username}`);if(m){let f=JSON.parse(m);if(K*1e3-(Date.now()-Number(f.sentAt||0))>0)return{ok:!0,resent:!1,sentAt:f.sentAt,devCode:""}}}catch{}let d=await Me(t,e,a);return d.ok?{ok:!0,resent:!0,sentAt:d.sentAt,devCode:d.devCode}:{ok:!1,error:d.error}}async function Bt(t,e,a){let d=`emailcode:${e.username}`,m=String(a||"").trim();if(!m)return{ok:!1,error:"请输入邮箱确认码"};let f=await t.TODO_KV.get(d);if(!f)return{ok:!1,error:"确认码已失效，请点击「重新发送确认码」重新获取"};let p=null;try{p=JSON.parse(f)}catch{p=null}if(!p||!p.codeHash)return await t.TODO_KV.delete(d),{ok:!1,error:"确认码已失效，请点击「重新发送确认码」重新获取"};if(Date.now()-Number(p.sentAt||0)>K*1e3)return await t.TODO_KV.delete(d),{ok:!1,error:"确认码已过期（有效期 10 分钟），请点击「重新发送确认码」"};if(Number(p.attempts||0)>=He)return await t.TODO_KV.delete(d),{ok:!1,error:"确认码错误次数过多，请点击「重新发送确认码」重新获取"};if(await U(m)!==p.codeHash){p.attempts=Number(p.attempts||0)+1;let r=He-p.attempts;return r<=0?(await t.TODO_KV.delete(d),{ok:!1,error:"确认码错误次数过多，请点击「重新发送确认码」重新获取"}):(await t.TODO_KV.put(d,JSON.stringify(p),{expirationTtl:K}),{ok:!1,error:`确认码不正确，请检查后重试（还可尝试 ${r} 次）`})}return{ok:!0}}async function Pe(t,e){let a=_();return await t.TODO_KV.put(`session:${a}`,e.username,{expirationTtl:60*60*24*7}),new Response(JSON.stringify({ok:!0,username:e.username,role:e.role}),{status:200,headers:{"Content-Type":"application/json; charset=utf-8","Set-Cookie":`token=${a}; Path=/; HttpOnly; SameSite=Lax; Max-Age=${60*60*24*7}`}})}async function pe(t){let e=await t.TODO_KV.get("user:admin");if(!e){let d={username:"admin",password:await U("admin"),role:"superadmin",createdAt:new Date().toISOString()};await t.TODO_KV.put("user:admin",JSON.stringify(d));return}let a=JSON.parse(e);a.role==="admin"&&(a.role="superadmin",await t.TODO_KV.put("user:admin",JSON.stringify(a)))}async function be(t,e){let a=await t.TODO_KV.list({prefix:"user:"}),d=[];for(let m of a.keys){let f=await t.TODO_KV.get(m.name);if(f){let p=JSON.parse(f);if(p.role==="producer"||p.role==="customer"||A(p.role)||T(p.role)||v(p)!==e)continue;await ie(t,p);let r=await re(t,p.username),s={username:p.username,role:p.role,createdAt:p.createdAt,remark:p.remark||"",position:p.position||"",dept:p.dept||"",canPlaceOrder:Ee(p),canPurchase:le(p),canViewCustomerOrder:Re(p),canViewPurchaseOrder:$e(p),canUpdateStatus:De(p),canViewAllOrders:Y(p),viewCustomerCount:r.length,orderProducerIds:await X(t,p.username),canPlaceMaskedOrder:ye(p),canAddShipDate:he(p)};p.role==="restricted"&&(s.watched=await de(t,p.username)),s.mentionsUnread=await $t(t,p.username),d.push(s)}}return d}async function P(t,e,a){let d=await t.TODO_KV.get(`user:${e}`);if(!d)return null;let m=JSON.parse(d);return A(m.role)||v(m)!==a?null:m}async function Ut(t){let e=await t.TODO_KV.list({prefix:"user:"}),a=[];for(let d of e.keys){let m=await t.TODO_KV.get(d.name);if(!m)continue;let f=JSON.parse(m);T(f.role)&&a.push({username:f.username,teamName:f.teamName||f.username,contact:f.contact||"",email:f.email||"",remark:f.remark||"",plan:ue(f)?"pro":"trial",pro:$(f),status:ce(f),rawStatus:f.status||"active",emailVerified:f.emailVerified!==!1,emailVerifiedAt:f.emailVerifiedAt||"",trialEndsAt:f.trialEndsAt||"",expiresAt:ue(f)&&f.expiresAt||"",createdAt:f.createdAt,renewedAt:f.renewedAt||"",subscribeRequest:rt(f)})}return a.sort((d,m)=>{let f=d.subscribeRequest&&d.subscribeRequest.at?1:0,p=m.subscribeRequest&&m.subscribeRequest.at?1:0;return f!==p?p-f:d.createdAt<m.createdAt?1:-1}),a}async function R(t,e){let a=await t.TODO_KV.get(`todos:${e}`);return a?JSON.parse(a):[]}async function Q(t,e,a){await t.TODO_KV.put(`todos:${e}`,JSON.stringify(a))}async function oe(t,e){let a=await t.TODO_KV.get(`customers:${e}`);return a?JSON.parse(a):[]}async function Ge(t,e,a){await t.TODO_KV.put(`customers:${e}`,JSON.stringify(a))}async function re(t,e){let a=await t.TODO_KV.get(`viewCustomers:${e}`);return a?JSON.parse(a):[]}async function Ye(t,e,a){await t.TODO_KV.put(`viewCustomers:${e}`,JSON.stringify(a))}async function ie(t,e){if(!e||!C(e.role))return e;let a=await re(t,e.username),d=Array.isArray(a)&&a.length>0;return e.canViewAllOrders!==!1===d&&(e.canViewAllOrders=!d,await t.TODO_KV.put(`user:${e.username}`,JSON.stringify(e))),e}async function H(t,e){let a=!e||e===Z?"customerList":`customerList:${e}`,d=await t.TODO_KV.get(a);return d?JSON.parse(d):[]}async function Ne(t,e,a){let d=!e||e===Z?"customerList":`customerList:${e}`;await t.TODO_KV.put(d,JSON.stringify(a))}var at=["self","purchased"],Lt="self";function Be(t){let e=String(t??"").trim();return at.includes(e)?e:Lt}async function B(t,e){let a=!e||e===Z?"producers":`producers:${e}`,d=await t.TODO_KV.get(a);return(d?JSON.parse(d):[]).map(f=>({id:f.id,username:f.username||f.shortName||"",description:f.description||"",nature:Be(f.nature),createdAt:f.createdAt}))}async function fe(t,e,a){let d=!e||e===Z?"producers":`producers:${e}`;await t.TODO_KV.put(d,JSON.stringify(a))}async function de(t,e){let a=await t.TODO_KV.get(`watch:${e}`);return a?JSON.parse(a):[]}async function Ze(t,e,a){await t.TODO_KV.put(`watch:${e}`,JSON.stringify(a))}async function X(t,e){let a=await t.TODO_KV.get(`orderProducers:${e}`);return a?JSON.parse(a):[]}async function Xe(t,e,a){await t.TODO_KV.put(`orderProducers:${e}`,JSON.stringify(a))}var Dt=50;function Rt(t){let e=new Set,a=/@([A-Za-z0-9_.-]{3,20})/g,d=String(t||""),m;for(;(m=a.exec(d))!==null;)e.add(m[1]);return[...e]}async function et(t,e){let a=[],d=await be(t,e);for(let m of d)a.push({username:m.username,role:m.role,position:m.position||"",dept:m.dept||"",isTeamAdmin:!1,label:m.username});return a}async function ve(t,e){let a=await t.TODO_KV.get(`mentions:${e}`);return a?JSON.parse(a):[]}async function st(t,e,a){await t.TODO_KV.put(`mentions:${e}`,JSON.stringify(a.slice(0,Dt)))}async function ot(t,e){let a=await ve(t,e);return{unread:a.filter(d=>!d.read).length,total:a.length}}async function $t(t,e){return(await ot(t,e)).unread}async function _t(t,e,a,d){if(!a||!a.length)return;let m=new Date().toISOString();for(let f of a){let p=await ve(t,f);p.unshift({id:_().slice(0,12),from:e.username,todoId:d.todoId,todoTitle:d.todoTitle||"",todoOwner:d.owner||"",text:d.text||"",at:m,read:!1}),await st(t,f,p)}}var Vt={superadmin:"超级管理员",team:"团队管理员",editor:"成员",viewer:"业务主管",restricted:"生产部",producer:"生产方",customer:"客户",superviewer:"总经理",deptmanager:"部门主管"};function L(t){return t&&(t.position||t.dept||Vt[t.role]||t.role)||""}function Ue(t){return t==="viewer"||t==="restricted"||t==="producer"||t==="customer"}var zt=["品质部","财务部"];function _e(t){return!!t&&t.role==="restricted"&&zt.includes(t.dept||"")}function C(t){return t==="editor"||t==="member"}function Kt(t){let e=t&&t.role;return C(e)?"添加订单":"生产单下单权限"}function Ee(t){return t?T(t.role)||C(t.role)?!0:t.role==="superviewer"||ne(t.role)||_e(t)?!1:typeof t.canPlaceOrder=="boolean"?t.canPlaceOrder:!1:!1}function le(t){return t?T(t.role)?!0:_e(t)?!1:C(t.role)?t.canUpdateStatus===!0||t.canPurchase===!0:typeof t.canPurchase=="boolean"?t.canPurchase:Ee(t):!1}async function jt(t,e){let a=await oe(t,e);return Array.isArray(a)&&a.length>0}async function Le(t,e){return!e||T(e.role)?!1:C(e.role)?jt(t,e.username):Ee(e)}function Ve(t){return T(t)||t==="superviewer"||ne(t)}function De(t){return t?T(t.role)||t.role==="superviewer"?!0:C(t.role)?t.canUpdateStatus===!0:!1:!1}function Y(t){return t?C(t.role)?t.canViewAllOrders!==!1:Ve(t.role):!1}function Jt(t){return t?T(t.role)||t.role==="superviewer"?!0:C(t.role)&&t.canUpdateStatus===!0:!1}function we(t){return t?Ve(t.role)?!0:C(t.role)&&t.canUpdateStatus===!0:!1}function Ht(t){return t?Ue(t.role)||we(t)?!0:C(t.role)?Y(t):!1:!1}async function it(t,e){if(!e||!C(e.role)||Y(e))return[];let a=await re(t,e.username),d=[];for(let m of Array.isArray(a)?a:[]){let f=String(m&&m.name||"").trim();f&&d.indexOf(f)===-1&&d.push(f)}return d}async function qt(t,e,a,d){if(!e||!C(e.role)||Y(e)||!a||a===e.username)return!1;let m=await it(t,e);if(!m.length||!await P(t,a,v(e)))return!1;let r=(await R(t,a)).find(s=>s.id===d);return!!r&&m.indexOf(String(r.customer||"").trim())!==-1}function ne(t){return t==="deptmanager"}function Re(t){return t?T(t.role)?!0:C(t.role)?t.canUpdateStatus===!0||t.canViewCustomerOrder===!0:ne(t.role)?t.canViewCustomerOrder!==!1:t.role==="producer"||t.role==="customer":!1}function $e(t){return t?T(t.role)?!0:C(t.role)?t.canUpdateStatus===!0||t.canViewPurchaseOrder===!0:ne(t.role)?t.canViewPurchaseOrder!==!1:t.role==="superviewer"?!0:le(t):!1}function ye(t){return t?T(t.role)?!0:C(t.role)?t.canPlaceMaskedOrder===!0:!1:!1}function he(t){return t?T(t.role)?!0:C(t.role)?t.canAddShipDate===!0:!1:!1}async function F(t,e,a=!1,d=null){let m=await t.TODO_KV.list({prefix:"todos:"}),f=m.keys.map(s=>s.name.replace("todos:","")),p=await Promise.all(f.map(async s=>{let o=await t.TODO_KV.get(`user:${s}`);return o?JSON.parse(o):null})),r=[];for(let s=0;s<f.length;s++){let o=p[s];if(!o||v(o)!==e)continue;let i=await t.TODO_KV.get(m.keys[s].name),l=i?JSON.parse(i):[];for(let c of l){let u=c.status||(c.done?"done":"pending");a&&u==="pending"||d&&!d.includes(c.producerId)||r.push({...c,status:u,owner:f[s]})}}return r.sort((s,o)=>s.createdAt<o.createdAt?1:-1),r}async function Qt(t,e){let a=await t.TODO_KV.list({prefix:"user:"}),d=[];for(let m of a.keys){let f=await t.TODO_KV.get(m.name);if(!f)continue;let p=null;try{p=JSON.parse(f)}catch{continue}!p||A(p.role)||v(p)===e&&d.push({username:p.username||m.name.replace(/^user:/,""),role:p.role})}return d}async function Ft(t,e){let a=0,d=await t.TODO_KV.get(`todos:${e}`);if(d)try{let m=JSON.parse(d);a=Array.isArray(m)?m.length:0}catch{a=0}return await t.TODO_KV.delete(`user:${e}`),await t.TODO_KV.delete(`todos:${e}`),await t.TODO_KV.delete(`customers:${e}`),await t.TODO_KV.delete(`viewCustomers:${e}`),await t.TODO_KV.delete(`orderProducers:${e}`),await t.TODO_KV.delete(`watch:${e}`),await t.TODO_KV.delete(`mentions:${e}`),await t.TODO_KV.delete(`emailcode:${e}`),await t.TODO_KV.delete(`mailrate:${e}`),{todos:a}}async function Wt(t,e,a){let d=t.method;if(a==="/api/login"&&d==="POST"){await pe(e);let{username:r,password:s}=await I(t);if(!r||!s)return n({error:"请输入用户名和密码"},400);let o=await e.TODO_KV.get(`user:${r}`);if(!o)return n({error:"用户名或密码错误"},401);let i=JSON.parse(o);if(await U(s)!==i.password)return n({error:"用户名或密码错误"},401);if(!A(i.role)){let c=await D(e,v(i));if(c&&ce(c)==="disabled")return n({error:"该账户已被停用，请联系超级管理员开通"},403)}if(i.role==="team"&&i.emailVerified===!1){let c=await Mt(e,i,t);return n({ok:!1,needVerify:!0,username:i.username,email:ae(i.email||""),codeTtlSec:K,resendAfterSec:ge,devCode:c.devCode||void 0,sendError:c.ok?void 0:c.error,message:c.ok?`确认码${c.resent?"已重新发送":"已发送"}至注册邮箱，请在 ${Math.floor(K/60)} 分钟内输入完成确认`:"确认码发送失败，请稍后点「重新发送确认码」重试"})}return Pe(e,i)}if(a==="/api/register"&&d==="POST"){if(await pe(e),!(await G(e)).allowRegister)return n({error:"系统当前已关闭新用户注册，如需开通请联系超级管理员"},403);let{teamName:s,username:o,email:i,password:l,confirmPassword:c,contact:u,remark:g}=await I(t),b=(o||"").trim(),h=(s||"").trim(),y=String(i||"").trim(),w=String(l||"");if(!h)return n({error:"请输入团队名称"},400);if(!b)return n({error:"请输入登录用户名"},400);if(!/^[A-Za-z0-9_.-]{3,20}$/.test(b))return n({error:"用户名为 3~20 位字母、数字、下划线、点或短横线"},400);if(b==="admin")return n({error:"该用户名不可用，请更换"},400);if(!y)return n({error:"请输入邮箱"},400);if(y.length>60)return n({error:"邮箱长度不能超过 60 个字符"},400);if(!V.test(y))return n({error:"邮箱格式不正确，请检查后重试"},400);if(!w)return n({error:"请输入密码"},400);if(w.length<6)return n({error:"密码至少 6 位"},400);if(c!==void 0&&w!==String(c))return n({error:"两次输入的密码不一致"},400);if(await e.TODO_KV.get(`user:${b}`))return n({error:"该用户名已被占用，请更换"},400);let E=new Date,N={username:b,password:await U(w),role:"team",teamId:b,teamName:h,contact:String(u||"").trim(),email:y,remark:String(g||"").trim(),emailVerified:!1,plan:"trial",status:"active",trialEndsAt:"",expiresAt:"",createdAt:E.toISOString()};await e.TODO_KV.put(`user:${b}`,JSON.stringify(N));let k=await Me(e,N,t);return k.ok?n({ok:!0,needVerify:!0,username:b,role:"team",teamName:h,email:ae(y),codeTtlSec:K,resendAfterSec:ge,devCode:k.devCode||void 0,unlimitedTrial:!0,message:`确认码已发送至 ${ae(y)}，请在 ${Math.floor(K/60)} 分钟内输入完成邮箱确认`}):(await e.TODO_KV.delete(`user:${b}`),n({error:"注册未完成："+k.error},503))}if(a==="/api/register/verify"&&d==="POST"){await pe(e);let{username:r,code:s}=await I(t),o=String(r||"").trim();if(!o)return n({error:"请输入用户名"},400);let i=await e.TODO_KV.get(`user:${o}`);if(!i)return n({error:"用户名不存在，请先注册"},404);let l=JSON.parse(i);if(l.role!=="team")return n({error:"该账号无需邮箱确认"},400);if(l.emailVerified!==!1)return Pe(e,l);let c=await Bt(e,l,s);return c.ok?(l.emailVerified=!0,l.emailVerifiedAt=new Date().toISOString(),await e.TODO_KV.put(`user:${o}`,JSON.stringify(l)),await e.TODO_KV.delete(`emailcode:${o}`),Pe(e,l)):n({error:c.error},400)}if(a==="/api/register/resend"&&d==="POST"){await pe(e);let{username:r,password:s}=await I(t),o=String(r||"").trim();if(!o)return n({error:"请输入用户名"},400);let i=await e.TODO_KV.get(`user:${o}`);if(!i)return n({error:"用户名不存在，请先注册"},404);let l=JSON.parse(i);if(l.role!=="team")return n({error:"该账号无需邮箱确认"},400);if(l.emailVerified!==!1)return n({error:"该账号已完成邮箱确认，可直接登录"},400);if(!s||await U(String(s))!==l.password)return n({error:"密码不正确，无法重新发送确认码"},401);let c=await Me(e,l,t);return c.ok?n({ok:!0,email:ae(l.email||""),codeTtlSec:K,resendAfterSec:ge,devCode:c.devCode||void 0,message:`确认码已重新发送至 ${ae(l.email||"")}`}):n({error:c.error},429)}if(a==="/api/logout"&&d==="POST"){let r=tt(t);return r&&await e.TODO_KV.delete(`session:${r}`),new Response(JSON.stringify({ok:!0}),{status:200,headers:{"Content-Type":"application/json; charset=utf-8","Set-Cookie":"token=; Path=/; HttpOnly; SameSite=Lax; Max-Age=0"}})}if(a==="/api/me"&&d==="GET"){let r=await x(t,e);if(!r)return n({error:"未登录"},401);let s={username:r.username,role:r.role};if(s.position=r.position||"",s.canPlaceOrder=await Le(e,r),s.canPurchase=le(r),s.canViewCustomerOrder=Re(r),s.canViewPurchaseOrder=$e(r),s.canUpdateStatus=De(r),await ie(e,r),s.canViewAllOrders=Y(r),s.canPlaceMaskedOrder=ye(r),s.canAddShipDate=he(r),s.orderProducers=[],C(r.role)){let i=await X(e,r.username);if(Array.isArray(i)&&i.length){let l=await B(e,v(r));s.orderProducers=l.filter(c=>i.indexOf(c.id)!==-1).map(c=>({id:c.id,username:c.username,nature:Be(c.nature)}))}}if(T(r.role)){if(s.teamId=r.teamId||r.username,s.teamName=r.teamName||r.username,s.contact=r.contact||"",s.plan=ue(r)?"pro":"trial",s.pro=$(r),s.trialEndsAt=r.trialEndsAt||"",s.expiresAt=r.expiresAt||"",s.status=ce(r),!$(r)){let l=v(r),[c,u,g]=await Promise.all([be(e,l),B(e,l),H(e,l)]);s.trialLimits={members:{used:c.length,max:W.members},producers:{used:u.length,max:W.producers},customers:{used:g.length,max:W.customers}}}let i=rt(r);s.subscribeRequest=i,s.renewRequest=i}else{let i=await D(e,v(r));s.teamId=v(r),s.teamName=i?i.teamName||i.username:"",s.teamPro=$(i)}let o=await ot(e,r.username);return s.mentionsUnread=o.unread,s.mentionsTotal=o.total,n(s)}if(a==="/api/change-password"&&d==="POST"){let r=await x(t,e);if(!r)return n({error:"未登录"},401);let{oldPassword:s,newPassword:o}=await I(t);return!s||!o?n({error:"请填写完整"},400):await U(s)!==r.password?n({error:"原密码错误"},400):(r.password=await U(o),await e.TODO_KV.put(`user:${r.username}`,JSON.stringify(r)),n({ok:!0}))}if(a==="/api/admin/password"&&d==="POST"){let r=await x(t,e);if(!r)return n({error:"未登录"},401);if(!A(r.role))return n({error:"无权限"},403);let{newPassword:s,confirmPassword:o}=await I(t),i=String(s||"");if(!i.trim())return n({error:"请输入新密码"},400);if(i.length<6)return n({error:"密码至少 6 位"},400);if(o!==void 0&&i!==String(o))return n({error:"两次输入的密码不一致"},400);let l=await e.TODO_KV.get(`user:${r.username}`);if(!l)return n({error:"账号不存在"},404);let c=JSON.parse(l);return c.password=await U(i),c.passwordUpdatedAt=new Date().toISOString(),await e.TODO_KV.put(`user:${r.username}`,JSON.stringify(c)),n({ok:!0,username:r.username})}if(a==="/api/settings"&&d==="GET"){let r=await x(t,e);if(!r)return n({error:"未登录"},401);let s=await G(e),o=T(r.role)?r:await D(e,v(r)),i={siteName:s.siteName,allowRegister:s.allowRegister,supportEmail:s.supportEmail,favicon:s.favicon,teamName:o?o.teamName||o.username:""};return A(r.role)&&(i.adCode=s.adCode),n({settings:i})}if(a==="/api/settings"&&d==="POST"){let r=await x(t,e);if(!r)return n({error:"未登录"},401);let{siteName:s,teamName:o,allowRegister:i,supportEmail:l,favicon:c,adCode:u}=await I(t);if(o!==void 0){if(!T(r.role))return n({error:"无权限"},403);let y=String(o).trim();return y?(r.teamName=y,await e.TODO_KV.put(`user:${r.username}`,JSON.stringify(r)),n({ok:!0,settings:{teamName:y}})):n({error:"请输入团队名称"},400)}if(s===void 0&&i===void 0&&l===void 0&&c===void 0&&u===void 0)return n({error:"请填写要保存的内容"},400);if(!A(r.role))return n({error:"无权限"},403);let g=await e.TODO_KV.get("settings"),b=g?JSON.parse(g):{};if(s!==void 0){if(!s||!s.trim())return n({error:"请输入网站名称"},400);b.siteName=s.trim()}if(i!==void 0&&(b.allowRegister=i===!0||i==="true"),l!==void 0){let y=String(l||"").trim();if(y.length>60)return n({error:"联系邮箱长度不能超过 60 个字符"},400);if(y&&!V.test(y))return n({error:"联系邮箱格式不正确，请检查后重试（留空则不显示该提示）"},400);b.supportEmail=y}if(c!==void 0){let y=String(c||"").trim();if(y.length>300)return n({error:"网站图标链接不能超过 300 个字符"},400);let w=J(y);if(w===null)return n({error:"网站图标需填写以 http:// 或 https:// 开头的图片链接（留空则使用默认图标）"},400);b.favicon=w}if(u!==void 0){let y=String(u===null?"":u).trim();if(y.length>3e3)return n({error:"广告代码不能超过 3000 个字符"},400);b.adCode=y}await e.TODO_KV.put("settings",JSON.stringify(b));let h=await G(e);return n({ok:!0,settings:{siteName:h.siteName,allowRegister:h.allowRegister,supportEmail:h.supportEmail,favicon:h.favicon,adCode:h.adCode}})}if((a==="/api/subscribe-request"||a==="/api/renew-request")&&d==="POST"){let r=await x(t,e);if(!r)return n({error:"未登录"},401);if(!T(r.role))return n({error:"无权限"},403);let{note:s,contact:o,plan:i}=await I(t);o!==void 0&&(r.contact=String(o||"").trim().slice(0,50));let l=i&&typeof i=="object"?i:null,c=l&&Number(l.days)>0?Math.floor(Number(l.days)):0,u={at:new Date().toISOString(),kind:$(r)?"renew":"subscribe",plan:l&&c?{id:String(l.id||"").slice(0,20),term:String(l.term||"").slice(0,20),price:Number(l.price)>=0?Number(l.price):0,days:c}:null,note:String(s||"").trim().slice(0,200),contact:r.contact||""};r.subscribeRequest=u,delete r.renewRequest,await e.TODO_KV.put(`user:${r.username}`,JSON.stringify(r));let g={ok:!1,to:r.email||"",cc:[],error:""};try{if(!r.email)g.error="该账号没有可用邮箱，无法发送申请邮件";else{let b=await se(e),h=await G(e),y=kt(b,h);if(g.cc=te({to:r.email,cc:y?[y]:[]}),b.devMode)g.ok=!0,g.devMode=!0;else{let w=await At(e,r,u,b,u.kind),S=await Ae(e,b,{to:r.email,cc:g.cc,subject:w.subject,text:w.text,html:w.html},{local:xe(t)});g.ok=!!S.ok,S.ok||(g.error=S.error||"邮件发送失败")}}}catch(b){g.ok=!1,g.error="邮件发送异常："+(b&&b.message?b.message:String(b))}return console.log(`[订阅申请邮件] ${r.username} -> ${g.to||"-"}`+(g.cc&&g.cc.length?`（抄送 ${g.cc.join(", ")}）`:"")+` ${g.ok?"成功":"失败："+g.error}`),n({ok:!0,subscribeRequest:u,mail:g})}let m="/api/teams";if(a===m&&d==="GET"){let r=await x(t,e);return r?A(r.role)?n({teams:await Ut(e)}):n({error:"无权限"},403):n({error:"未登录"},401)}if(a.startsWith(`${m}/`)&&a.endsWith("/status")&&d==="POST"){let r=await x(t,e);if(!r)return n({error:"未登录"},401);if(!A(r.role))return n({error:"无权限"},403);let s=decodeURIComponent(a.replace(`${m}/`,"").replace("/status","")),{status:o}=await I(t);if(!["active","disabled"].includes(o))return n({error:"无效的状态"},400);let i=await D(e,s);return i?(i.status=o,await e.TODO_KV.put(`user:${s}`,JSON.stringify(i)),n({ok:!0,status:ce(i),expiresAt:i.expiresAt||i.trialEndsAt||""})):n({error:"团队用户不存在"},404)}if(a.startsWith(`${m}/`)&&a.endsWith("/renew")&&d==="POST"){let r=await x(t,e);if(!r)return n({error:"未登录"},401);if(!A(r.role))return n({error:"无权限"},403);let s=decodeURIComponent(a.replace(`${m}/`,"").replace("/renew","")),o=await I(t),i=Number(o.days)>0?Math.floor(Number(o.days)):30;if(i>3650)return n({error:"续费天数过大"},400);let l=await D(e,s);if(!l)return n({error:"团队用户不存在"},404);let c=Date.now(),u=new Date(l.expiresAt||"").getTime(),g=Number.isNaN(u)||u<c?c:u,b=new Date(g+i*mt).toISOString();return l.expiresAt=b,l.status="active",l.plan="pro",l.renewedAt=new Date().toISOString(),delete l.subscribeRequest,delete l.renewRequest,await e.TODO_KV.put(`user:${s}`,JSON.stringify(l)),n({ok:!0,days:i,expiresAt:b,status:"active",pro:!0})}if(a.startsWith(`${m}/`)&&a.endsWith("/reset-password")&&d==="POST"){let r=await x(t,e);if(!r)return n({error:"未登录"},401);if(!A(r.role))return n({error:"无权限"},403);let s=decodeURIComponent(a.replace(`${m}/`,"").replace("/reset-password","")),{newPassword:o}=await I(t);if(!o||!String(o).trim())return n({error:"请输入新密码"},400);let i=await D(e,s);return i?(i.password=await U(String(o).trim()),await e.TODO_KV.put(`user:${s}`,JSON.stringify(i)),n({ok:!0})):n({error:"团队用户不存在"},404)}if(a.startsWith(`${m}/`)&&a.endsWith("/remark")&&d==="POST"){let r=await x(t,e);if(!r)return n({error:"未登录"},401);if(!A(r.role))return n({error:"无权限"},403);let s=decodeURIComponent(a.replace(`${m}/`,"").replace("/remark","")),{remark:o}=await I(t),i=await D(e,s);return i?(i.remark=String(o||"").trim(),await e.TODO_KV.put(`user:${s}`,JSON.stringify(i)),n({ok:!0,remark:i.remark})):n({error:"团队用户不存在"},404)}if(a.startsWith(`${m}/`)&&d==="DELETE"){let r=await x(t,e);if(!r)return n({error:"未登录"},401);if(!A(r.role))return n({error:"无权限"},403);let s=decodeURIComponent(a.slice(m.length+1));if(!s||s.indexOf("/")!==-1)return n({error:"接口不存在"},404);let o=await D(e,s);if(!o)return n({error:"团队用户不存在"},404);if(o.status!=="disabled")return n({error:"请先「停用」该团队用户，确认无误后再删除"},400);let i=await Qt(e,s),l=i.reduce((b,h)=>(b[h.role]=(b[h.role]||0)+1,b),{}),c=0;for(let b of i){let h=await Ft(e,b.username);c+=h.todos}let u=await B(e,s),g=await H(e,s);return await e.TODO_KV.delete(`producers:${s}`),await e.TODO_KV.delete(`customerList:${s}`),n({ok:!0,deleted:{team:s,teamName:o.teamName||s,accounts:i.length,members:i.filter(b=>!["team","producer","customer"].includes(b.role)).length,producers:u.length,customers:g.length,todos:c,roles:l}})}if(a==="/api/admin/email-settings"&&d==="GET"){let r=await x(t,e);if(!r)return n({error:"未登录"},401);if(!A(r.role))return n({error:"无权限"},403);let s=await se(e);return n({settings:We(s,e)})}if(a==="/api/admin/email-settings"&&d==="POST"){let r=await x(t,e);if(!r)return n({error:"未登录"},401);if(!A(r.role))return n({error:"无权限"},403);let{from:s,fromName:o,apiKey:i,devMode:l,smtpHost:c,smtpPort:u,smtpUser:g,smtpPass:b,adminNotifyEmail:h,subExtraText:y,subQrCodes:w}=await I(t),S=await e.TODO_KV.get("emailSettings"),E=S?JSON.parse(S):{};if(s!==void 0){let k=String(s||"").trim();if(k&&!V.test(k))return n({error:"发件邮箱格式不正确，请检查后重试"},400);E.from=k}if(o!==void 0&&(E.fromName=String(o||"").trim().slice(0,30)),i!==void 0){let k=String(i||"").trim();k==="-"?delete E.apiKey:k&&(E.apiKey=k)}if(c!==void 0){let k=String(c||"").trim();if(k&&!/^[A-Za-z0-9.-]{1,60}$/.test(k))return n({error:"SMTP 服务器地址格式不正确（例如 smtp.qq.com）"},400);E.smtpHost=k||"smtp.qq.com"}if(u!==void 0){let k=String(u??"").trim(),M=Number(k)||0;if(k&&(M<1||M>65535))return n({error:"SMTP 端口需为 1~65535 的数字（QQ 邮箱用 465，或 587 + STARTTLS）"},400);E.smtpPort=M||465}if(g!==void 0){let k=String(g||"").trim();if(k&&!V.test(k))return n({error:"SMTP 账号需填写完整邮箱地址（例如 xxx@qq.com）"},400);E.smtpUser=k}if(b!==void 0){let k=String(b||"").trim();k==="-"?delete E.smtpPass:k&&(E.smtpPass=k)}if(l!==void 0&&(E.devMode=!!l),h!==void 0){let k=String(h||"").trim();if(k&&!V.test(k))return n({error:"管理员提醒邮箱格式不正确（留空则默认抄送到发件邮箱）"},400);E.adminNotifyEmail=k}if(y!==void 0){let k=String(y||"").replace(/\r\n/g,`
`).trim();if(k.length>1e3)return n({error:"自定义说明不能超过 1000 个字符"},400);E.subExtraText=k}if(w!==void 0){let k=Array.isArray(w)?w.slice(0,ee):[];if(k.length&&w.length>ee)return n({error:`收款二维码最多 ${ee} 个`},400);for(let O=0;O<k.length;O++){let q=String(k[O]||"").trim();if(q){if(q.length>300)return n({error:`收款二维码链接太长（第 ${O+1} 个，最多 300 个字符）`},400);if(J(q)===null)return n({error:`收款二维码需填写以 http:// 或 https:// 开头的图片链接（第 ${O+1} 个，留空则不显示）`},400)}}let M=Te(k);for(;M.length&&!M[M.length-1];)M.pop();E.subQrCodes=M}await e.TODO_KV.put("emailSettings",JSON.stringify(E));let N=await se(e);return n({ok:!0,settings:We(N,e)})}if(a==="/api/admin/email-test"&&d==="POST"){let r=await x(t,e);if(!r)return n({error:"未登录"},401);if(!A(r.role))return n({error:"无权限"},403);let{to:s}=await I(t),o=String(s||"").trim();if(!o)return n({error:"请输入测试收件邮箱"},400);if(!V.test(o))return n({error:"测试收件邮箱格式不正确"},400);let i=await se(e);if(i.devMode)return n({ok:!0,devMode:!0,message:"当前为调试模式（不真实发送邮件），无需测试发信"});if(!i.provider)return n({error:"邮件服务未配置：请填写 QQ 邮箱 SMTP 账号与授权码（或 Resend API Key）并保存后，再发送测试邮件"},400);let l=i.provider==="smtp"?"QQ 邮箱 SMTP":i.provider==="resend"?"Resend":"Cloudflare 邮件绑定",c=await Ae(e,i,{to:o,subject:"【测试】邮件服务配置正常",text:"这是一封测试邮件：收到本邮件说明「注册邮箱确认码」的发件服务已配置成功。",html:"<p>这是一封测试邮件：收到本邮件说明「注册邮箱确认码」的发件服务已配置成功。</p>"},{local:xe(t)});return c.ok?n({ok:!0,message:`测试邮件已通过 ${l} 发送至 ${o}，请查收（若没收到，请检查垃圾邮件箱）`}):n({error:c.error},400)}if(a==="/api/users"&&d==="GET"){let r=await x(t,e);return r?T(r.role)?n({users:await be(e,v(r))}):n({error:"无权限"},403):n({error:"未登录"},401)}if(a==="/api/users"&&d==="POST"){let r=await x(t,e);if(!r)return n({error:"未登录"},401);if(!T(r.role))return n({error:"无权限"},403);if(!$(r)&&(await be(e,v(r))).length>=W.members)return n({error:Oe("members")},403);let{username:s,password:o,position:i}=await I(t);if(!s||!o)return n({error:"请填写用户名和密码"},400);if(await e.TODO_KV.get(`user:${s}`))return n({error:"用户名已存在"},400);let c={username:s,password:await U(o),role:"editor",teamId:v(r),createdAt:new Date().toISOString()},u=String(i??"").trim();return u&&(c.position=u.slice(0,20)),await e.TODO_KV.put(`user:${s}`,JSON.stringify(c)),n({ok:!0})}if(a.startsWith("/api/users/")&&d==="DELETE"){let r=await x(t,e);if(!r)return n({error:"未登录"},401);if(!T(r.role))return n({error:"无权限"},403);let s=decodeURIComponent(a.replace("/api/users/",""));return await P(e,s,v(r))?(await e.TODO_KV.delete(`user:${s}`),await e.TODO_KV.delete(`todos:${s}`),await e.TODO_KV.delete(`customers:${s}`),await e.TODO_KV.delete(`viewCustomers:${s}`),await e.TODO_KV.delete(`orderProducers:${s}`),await e.TODO_KV.delete(`watch:${s}`),await e.TODO_KV.delete(`mentions:${s}`),n({ok:!0})):n({error:"成员不存在"},404)}if(a.startsWith("/api/users/")&&a.endsWith("/reset-password")&&d==="POST"){let r=await x(t,e);if(!r)return n({error:"未登录"},401);if(!T(r.role))return n({error:"无权限"},403);let s=v(r),o=decodeURIComponent(a.replace("/api/users/","").replace("/reset-password","")),{newPassword:i}=await I(t);if(!i||!i.trim())return n({error:"请输入新密码"},400);let l=await e.TODO_KV.get(`user:${o}`);if(!l){let g=(await B(e,s)).find(y=>y.username===o);if(g)return await e.TODO_KV.put(`user:${o}`,JSON.stringify({username:o,password:await U(i),role:"producer",producerId:g.id,teamId:s,createdAt:new Date().toISOString()})),n({ok:!0,created:!0});let h=(await H(e,s)).find(y=>y.name===o);return h?(await e.TODO_KV.put(`user:${o}`,JSON.stringify({username:o,password:await U(i),role:"customer",customerId:h.id,customerName:h.name,teamId:s,createdAt:new Date().toISOString()})),n({ok:!0,created:!0})):n({error:"成员不存在"},404)}let c=JSON.parse(l);return A(c.role)||v(c)!==s?n({error:"成员不存在"},404):(c.password=await U(i),await e.TODO_KV.put(`user:${o}`,JSON.stringify(c)),n({ok:!0}))}if(a.startsWith("/api/users/")&&a.endsWith("/remark")&&d==="POST"){let r=await x(t,e);if(!r)return n({error:"未登录"},401);if(!T(r.role))return n({error:"无权限"},403);let s=decodeURIComponent(a.replace("/api/users/","").replace("/remark","")),{remark:o}=await I(t),i=await P(e,s,v(r));return i?(i.remark=String(o||"").trim(),await e.TODO_KV.put(`user:${s}`,JSON.stringify(i)),n({ok:!0,remark:i.remark})):n({error:"成员不存在"},404)}if(a.startsWith("/api/users/")&&a.endsWith("/position")&&d==="POST"){let r=await x(t,e);if(!r)return n({error:"未登录"},401);if(!T(r.role))return n({error:"无权限"},403);let s=decodeURIComponent(a.replace("/api/users/","").replace("/position","")),{position:o}=await I(t),i=await P(e,s,v(r));return i?(i.position=String(o||"").trim().slice(0,20),await e.TODO_KV.put(`user:${s}`,JSON.stringify(i)),n({ok:!0,position:i.position})):n({error:"成员不存在"},404)}if(a.startsWith("/api/users/")&&a.endsWith("/order-permission")&&d==="POST"){let r=await x(t,e);if(!r)return n({error:"未登录"},401);if(!T(r.role))return n({error:"无权限"},403);let s=decodeURIComponent(a.replace("/api/users/","").replace("/order-permission","")),o=await I(t),i=typeof o.canPlaceOrder=="boolean",l=typeof o.canPurchase=="boolean",c=typeof o.canViewCustomerOrder=="boolean",u=typeof o.canViewPurchaseOrder=="boolean",g=typeof o.canUpdateStatus=="boolean",b=typeof o.canViewAllOrders=="boolean",h=typeof o.canPlaceMaskedOrder=="boolean",y=typeof o.canAddShipDate=="boolean";if(!i&&!l&&!c&&!u&&!g&&!b&&!h&&!y)return n({error:"请传入 true（有）或 false（无）"},400);let w=await P(e,s,v(r));return w?i&&C(w.role)?n({error:"该成员的「添加订单」权限由「可录入订单客户列表」自动决定（有客户即可添加订单），无需在此设置"},400):i&&(w.role==="superviewer"||ne(w.role))?n({error:L(w)+"成员不需要「生产单下单权限」（固定不录入订单），无需设置"},400):b&&C(w.role)?n({error:"「是否可查看全部订单」由「可查看客户列表」自动决定（列表为空 = 是，可查看本团队全部订单；在下方「可查看客户列表」中选择客户后 = 否，只能查看这些客户的订单 + 自己录入的订单），无需在此设置"},400):b&&!C(w.role)?n({error:"「是否可查看全部订单」仅适用于普通成员（原业务部）（当前成员："+L(w)+"）"},400):g&&!C(w.role)?n({error:"「是否可以更新订单状态」仅适用于普通成员（原业务部）（当前成员："+L(w)+"）"},400):h&&!C(w.role)?n({error:"「是否可以下脱敏订单」仅适用于普通成员（原业务部）（当前成员："+L(w)+"）"},400):y&&!C(w.role)?n({error:"「是否可以添加出货日期」仅适用于普通成员（原业务部）（当前成员："+L(w)+"）"},400):(c||u)&&!C(w.role)&&!ne(w.role)?n({error:"「是否可以查看客户订单 / 查看生产订单」仅适用于普通成员（原业务部）与「部门主管」成员（当前成员："+L(w)+"）"},400):_e(w)&&(i||l)?n({error:L(w)+"成员不需要「"+(l?"下生产订单":"生产单下单权限")+"」（该部门无录入/下单需求），无需设置"},400):(i&&(w.canPlaceOrder=o.canPlaceOrder),l&&(w.canPurchase=o.canPurchase),c&&(w.canViewCustomerOrder=o.canViewCustomerOrder),u&&(w.canViewPurchaseOrder=o.canViewPurchaseOrder),g&&(w.canUpdateStatus=o.canUpdateStatus),h&&(w.canPlaceMaskedOrder=o.canPlaceMaskedOrder),y&&(w.canAddShipDate=o.canAddShipDate),await e.TODO_KV.put(`user:${s}`,JSON.stringify(w)),n({ok:!0,canPlaceOrder:Ee(w),canPurchase:le(w),canViewCustomerOrder:Re(w),canViewPurchaseOrder:$e(w),canUpdateStatus:De(w),canViewAllOrders:Y(w),canPlaceMaskedOrder:ye(w),canAddShipDate:he(w)})):n({error:"成员不存在"},404)}if(a.startsWith("/api/customers/")&&d==="GET"){let r=await x(t,e);if(!r)return n({error:"未登录"},401);let s=decodeURIComponent(a.replace("/api/customers/",""));return s!==r.username&&!(T(r.role)?await P(e,s,v(r)):null)?n({error:"无权限"},403):n({customers:await oe(e,s)})}if(a.startsWith("/api/customers/")&&d==="POST"){let r=await x(t,e);if(!r)return n({error:"未登录"},401);if(!T(r.role))return n({error:"无权限"},403);let s=decodeURIComponent(a.replace("/api/customers/",""));if(!await P(e,s,v(r)))return n({error:"成员不存在"},404);let{name:i,globalId:l}=await I(t);if(!i||!i.trim())return n({error:"请输入客户名称"},400);let c=await oe(e,s),u=i.trim(),g=l?String(l).trim():"";if(c.some(h=>h.name===u||g&&h.id===g))return n({error:"客户已存在"},400);let b={id:g||_().slice(0,12),name:u,createdAt:new Date().toISOString()};return c.push(b),await Ge(e,s,c),n({ok:!0,customer:b})}if(a.startsWith("/api/customers/")&&d==="DELETE"){let r=await x(t,e);if(!r)return n({error:"未登录"},401);if(!T(r.role))return n({error:"无权限"},403);let s=decodeURIComponent(a.replace("/api/customers/","")),o=s.lastIndexOf("/");if(o===-1)return n({error:"参数错误"},400);let i=s.slice(0,o),l=s.slice(o+1);if(!await P(e,i,v(r)))return n({error:"成员不存在"},404);let u=await oe(e,i);return u=u.filter(g=>g.id!==l),await Ge(e,i,u),n({ok:!0})}if(a.startsWith("/api/view-customers/")&&d==="GET"){let r=await x(t,e);if(!r)return n({error:"未登录"},401);let s=decodeURIComponent(a.replace("/api/view-customers/",""));return s!==r.username&&!(T(r.role)?await P(e,s,v(r)):null)?n({error:"无权限"},403):n({customers:await re(e,s)})}if(a.startsWith("/api/view-customers/")&&d==="POST"){let r=await x(t,e);if(!r)return n({error:"未登录"},401);if(!T(r.role))return n({error:"无权限"},403);let s=decodeURIComponent(a.replace("/api/view-customers/","")),o=await P(e,s,v(r));if(!o)return n({error:"成员不存在"},404);let{name:i,globalId:l}=await I(t);if(!i||!i.trim())return n({error:"请输入客户名称"},400);let c=i.trim(),u=l?String(l).trim():"",g=await re(e,s);if(g.some(y=>y.name===c||u&&y.id===u))return n({error:"该客户已在「可查看客户列表」中"},400);let b={id:u||_().slice(0,12),name:c,createdAt:new Date().toISOString()};g.push(b),await Ye(e,s,g);let h=await ie(e,o);return n({ok:!0,customer:b,canViewAllOrders:h.canViewAllOrders!==!1,viewCustomerCount:g.length})}if(a.startsWith("/api/view-customers/")&&d==="DELETE"){let r=await x(t,e);if(!r)return n({error:"未登录"},401);if(!T(r.role))return n({error:"无权限"},403);let s=decodeURIComponent(a.replace("/api/view-customers/","")),o=s.lastIndexOf("/");if(o===-1)return n({error:"参数错误"},400);let i=s.slice(0,o),l=s.slice(o+1),c=await P(e,i,v(r));if(!c)return n({error:"成员不存在"},404);let u=await re(e,i);u=u.filter(b=>b.id!==l),await Ye(e,i,u);let g=await ie(e,c);return n({ok:!0,canViewAllOrders:g.canViewAllOrders!==!1,viewCustomerCount:u.length})}if(a==="/api/customer-list"&&d==="GET"){let r=await x(t,e);return r?T(r.role)?n({customers:await H(e,v(r))}):n({error:"无权限"},403):n({error:"未登录"},401)}if(a==="/api/customer-list"&&d==="POST"){let r=await x(t,e);if(!r)return n({error:"未登录"},401);if(!T(r.role))return n({error:"无权限"},403);let s=v(r);if(!$(r)&&(await H(e,s)).length>=W.customers)return n({error:Oe("customers")},403);let{name:o,password:i,description:l}=await I(t);if(!o||!o.trim())return n({error:"请输入用户名"},400);if(!i||!String(i).trim())return n({error:"请输入密码"},400);let c=await H(e,s),u=o.trim();if(c.some(h=>h.name===u))return n({error:"该用户名已存在"},400);if(await e.TODO_KV.get(`user:${u}`))return n({error:"该用户名已被占用"},400);let b={id:_().slice(0,12),name:u,description:(l||"").trim(),createdAt:new Date().toISOString()};return c.push(b),await Ne(e,s,c),await e.TODO_KV.put(`user:${u}`,JSON.stringify({username:u,password:await U(String(i).trim()),role:"customer",customerId:b.id,customerName:u,teamId:s,createdAt:b.createdAt})),n({ok:!0,customer:b})}if(a.startsWith("/api/customer-list/")&&d==="DELETE"){let r=await x(t,e);if(!r)return n({error:"未登录"},401);if(!T(r.role))return n({error:"无权限"},403);let s=v(r),o=decodeURIComponent(a.replace("/api/customer-list/","")),i=await H(e,s),l=i.find(c=>c.id===o);return i=i.filter(c=>c.id!==o),await Ne(e,s,i),l&&l.name&&await e.TODO_KV.delete(`user:${l.name}`),n({ok:!0})}if(a.startsWith("/api/customer-list/")&&a.endsWith("/description")&&d==="POST"){let r=await x(t,e);if(!r)return n({error:"未登录"},401);if(!T(r.role))return n({error:"无权限"},403);let s=v(r),o=decodeURIComponent(a.replace("/api/customer-list/","").replace("/description","")),{description:i}=await I(t),l=await H(e,s),c=l.find(u=>u.id===o);return c?(c.description=String(i||"").trim(),await Ne(e,s,l),n({ok:!0,description:c.description})):n({error:"客户不存在"},404)}if(a==="/api/producers"&&d==="GET"){let r=await x(t,e);return r?n({producers:await B(e,v(r))}):n({error:"未登录"},401)}if(a==="/api/producers"&&d==="POST"){let r=await x(t,e);if(!r)return n({error:"未登录"},401);if(!T(r.role))return n({error:"无权限"},403);let s=v(r);if(!$(r)&&(await B(e,s)).length>=W.producers)return n({error:Oe("producers")},403);let{username:o,password:i,description:l,nature:c}=await I(t),u=(o||"").trim();if(!u)return n({error:"请输入用户名"},400);if(!i||!String(i).trim())return n({error:"请输入密码"},400);let g=await B(e,s);if(g.some(y=>y.username===u))return n({error:"该用户名已存在"},400);if(await e.TODO_KV.get(`user:${u}`))return n({error:"该用户名已被成员占用"},400);let h={id:_().slice(0,12),username:u,description:(l||"").trim(),nature:Be(c),createdAt:new Date().toISOString()};return g.push(h),await fe(e,s,g),await e.TODO_KV.put(`user:${u}`,JSON.stringify({username:u,password:await U(String(i).trim()),role:"producer",producerId:h.id,teamId:s,createdAt:h.createdAt})),n({ok:!0,producer:h})}if(a.startsWith("/api/producers/")&&d==="DELETE"){let r=await x(t,e);if(!r)return n({error:"未登录"},401);if(!T(r.role))return n({error:"无权限"},403);let s=v(r),o=decodeURIComponent(a.replace("/api/producers/","")),i=await B(e,s),l=i.find(c=>c.id===o);return i=i.filter(c=>c.id!==o),await fe(e,s,i),l&&l.username&&await e.TODO_KV.delete(`user:${l.username}`),n({ok:!0})}if(a.startsWith("/api/producers/")&&a.endsWith("/description")&&d==="POST"){let r=await x(t,e);if(!r)return n({error:"未登录"},401);if(!T(r.role))return n({error:"无权限"},403);let s=v(r),o=decodeURIComponent(a.replace("/api/producers/","").replace("/description","")),{description:i}=await I(t),l=await B(e,s),c=l.find(u=>u.id===o);return c?(c.description=String(i||"").trim(),await fe(e,s,l),n({ok:!0,description:c.description})):n({error:"生产方不存在"},404)}if(a.startsWith("/api/producers/")&&a.endsWith("/nature")&&d==="POST"){let r=await x(t,e);if(!r)return n({error:"未登录"},401);if(!T(r.role))return n({error:"无权限"},403);let s=v(r),o=decodeURIComponent(a.replace("/api/producers/","").replace("/nature","")),{nature:i}=await I(t),l=String(i??"").trim();if(!at.includes(l))return n({error:"生产方性质只能是「自产」或「外购」"},400);let c=await B(e,s),u=c.find(g=>g.id===o);return u?(u.nature=l,await fe(e,s,c),n({ok:!0,nature:u.nature})):n({error:"生产方不存在"},404)}let f="/api/watch/";if(a.startsWith(f)&&d==="GET"){let r=await x(t,e);if(!r)return n({error:"未登录"},401);let s=v(r),o=decodeURIComponent(a.replace(f,""));if(o!==r.username&&!(T(r.role)?await P(e,o,s):null))return n({error:"无权限"},403);let i=await de(e,o),l=await B(e,s),c=i.map(u=>{let g=l.find(b=>b.id===u);return g||{id:u,username:"（已删除的生产方）",description:"",deleted:!0}});return n({producers:c})}if(a.startsWith(f)&&d==="POST"){let r=await x(t,e);if(!r)return n({error:"未登录"},401);if(!T(r.role))return n({error:"无权限"},403);let s=v(r),o=decodeURIComponent(a.replace(f,"")),i=await P(e,o,s);if(!i)return n({error:"成员不存在"},404);let{producerId:l}=await I(t);if(!l||!String(l).trim())return n({error:"请选择生产方"},400);let c=String(l).trim();if(!(await B(e,s)).some(b=>b.id===c))return n({error:"生产方不存在，请先在「生产方管理」中添加"},400);if(i.role!=="restricted")return n({error:"该成员不是「生产部」成员，无法授权可观察的生产方"},400);let g=await de(e,o);return g.includes(c)?n({error:"已授权该生产方"},400):(g.push(c),await Ze(e,o,g),n({ok:!0,producerIds:g}))}if(a.startsWith(f)&&d==="DELETE"){let r=await x(t,e);if(!r)return n({error:"未登录"},401);if(!T(r.role))return n({error:"无权限"},403);let s=decodeURIComponent(a.replace(f,"")),o=s.lastIndexOf("/");if(o===-1)return n({error:"参数错误"},400);let i=s.slice(0,o),l=s.slice(o+1);if(!await P(e,i,v(r)))return n({error:"成员不存在"},404);let u=await de(e,i);return u=u.filter(g=>g!==l),await Ze(e,i,u),n({ok:!0,producerIds:u})}let p="/api/order-producers/";if(a.startsWith(p)&&d==="GET"){let r=await x(t,e);if(!r)return n({error:"未登录"},401);let s=decodeURIComponent(a.replace(p,""));if(s!==r.username&&!(T(r.role)?await P(e,s,v(r)):null))return n({error:"无权限"},403);let o=await X(e,s),i=await B(e,v(r)),l=o.map(c=>{let u=i.find(g=>g.id===c);return u||{id:c,username:"（已删除的生产方）",description:"",deleted:!0}});return n({producers:l,producerIds:o})}if(a.startsWith(p)&&d==="POST"){let r=await x(t,e);if(!r)return n({error:"未登录"},401);if(!T(r.role))return n({error:"无权限"},403);let s=v(r),o=decodeURIComponent(a.replace(p,"")),i=await P(e,o,s);if(!i)return n({error:"成员不存在"},404);if(!C(i.role))return n({error:"「可选生产方」仅适用于普通成员（原业务部）"},400);let{producerId:l}=await I(t);if(!l||!String(l).trim())return n({error:"请选择生产方"},400);let c=String(l).trim();if(!(await B(e,s)).some(b=>b.id===c))return n({error:"生产方不存在，请先在「生产方管理」中添加"},400);let g=await X(e,o);return g.includes(c)?n({error:"已授权该生产方"},400):(g.push(c),await Xe(e,o,g),n({ok:!0,producerIds:g}))}if(a.startsWith(p)&&d==="DELETE"){let r=await x(t,e);if(!r)return n({error:"未登录"},401);if(!T(r.role))return n({error:"无权限"},403);let s=decodeURIComponent(a.replace(p,"")),o=s.lastIndexOf("/");if(o===-1)return n({error:"参数错误"},400);let i=s.slice(0,o),l=s.slice(o+1);if(!await P(e,i,v(r)))return n({error:"成员不存在"},404);let u=await X(e,i);return u=u.filter(g=>g!==l),await Xe(e,i,u),n({ok:!0,producerIds:u})}if(a==="/api/team-members"&&d==="GET"){let r=await x(t,e);return r?n({members:await et(e,v(r))}):n({error:"未登录"},401)}if(a==="/api/mentions"&&d==="GET"){let r=await x(t,e);if(!r)return n({error:"未登录"},401);let s=await ve(e,r.username);return n({items:s,unread:s.filter(o=>!o.read).length,total:s.length})}if(a==="/api/mentions/read"&&d==="POST"){let r=await x(t,e);if(!r)return n({error:"未登录"},401);let s=await I(t),o=s&&s.id?String(s.id).trim():"",i=!(s&&s.read===!1),l=await ve(e,r.username),c=0;for(let u of l)u.read!==i&&(o&&u.id!==o||(u.read=i,c++));return c&&await st(e,r.username,l),n({ok:!0,changed:c,read:i,unread:l.filter(u=>!u.read).length,total:l.length})}if(a==="/api/todos"&&d==="GET"){let r=await x(t,e);if(!r)return n({error:"未登录"},401);let s=v(r);if(Ve(r.role))return n({todos:await F(e,s,!1),readonly:!1,allUsers:!0});if(r.role==="viewer")return n({todos:await F(e,s,!0),readonly:!0,allUsers:!0});if(r.role==="restricted"){let i=await de(e,r.username);return n({todos:await F(e,s,!0,i),readonly:!0,allUsers:!0})}if(r.role==="producer"){let l=(await B(e,s)).find(c=>c.id===r.producerId);return n({todos:await F(e,s,!0,l?[l.id]:[]),readonly:!0,allUsers:!0})}if(r.role==="customer"){let i=r.customerName||"",l=i?await F(e,s,!0):[];return n({todos:l.filter(c=>c.customer===i),readonly:!0,allUsers:!0})}if(C(r.role)&&await ie(e,r),C(r.role)&&!Y(r)){let i=we(r),c=(await R(e,r.username)).map(y=>Object.assign({},y,{owner:r.username})),u=await it(e,r);if(!u.length)return n({todos:c,readonly:!1});let b=(await F(e,s,!1)).filter(y=>y.owner!==r.username&&u.indexOf(String(y.customer||"").trim())!==-1).map(y=>i?Object.assign({},y):Object.assign({},y,{viewOnly:!0})),h=c.concat(b);return h.sort((y,w)=>y.createdAt<w.createdAt?1:-1),n({todos:h,readonly:!1,allUsers:!0})}if(C(r.role))return n({todos:await F(e,s,!1),readonly:!0,allUsers:!0});let o=await R(e,r.username);return n({todos:o.map(i=>Object.assign({},i,{owner:r.username})),readonly:!1})}if(a==="/api/todos"&&d==="POST"){let r=await x(t,e);if(!r)return n({error:"未登录"},401);if(!await Le(e,r))return T(r.role)?n({error:"团队账号不录入订单（试用版与专业版一致）：请在「成员管理」中添加成员、在「可录入订单客户列表」中为其分配客户后，由成员登录录入订单"},403):C(r.role)?n({error:"暂无客户，请联系团队管理员在「成员管理」的「可录入订单客户列表」中为你添加客户（有客户即可添加订单）"},403):n({error:L(r)+"无「"+Kt(r)+"」，请联系团队管理员在「成员管理」中开通"},403);let{title:s,customer:o,dueDate:i,amount:l,orderUrl:c,purchaseUrl:u,currency:g,producerId:b}=await I(t);if(!o||!o.trim())return n({error:"选择客户"},400);if(!s||!s.trim())return n({error:"请输入主题"},400);if(!i||!String(i).trim())return n({error:"选择交期"},400);let h=J(c);if(h===null)return n({error:"订单文件链接需以 http:// 或 https:// 开头"},400);let y=J(u);if(y===null)return n({error:"采购文件链接需以 http:// 或 https:// 开头"},400);if(l==null||String(l).trim()==="")return n({error:"输入金额"},400);let w=Number(l);if(Number.isNaN(w)||w<0)return n({error:"金额必须为非负数字"},400);let S=o.trim();if(!(await oe(e,r.username)).some(ze=>ze.name===S))return n({error:"客户不存在，请联系管理员添加"},400);let E="",N="",k=b==null?"":String(b).trim();if(k){let q=await X(e,r.username);if(!Array.isArray(q)||q.indexOf(k)===-1)return n({error:"无权指定该生产方：请联系团队管理员在「成员管理」的「可选生产方列表」中为你授权"},403);let Se=(await B(e,v(r))).find(dt=>dt.id===k);if(!Se)return n({error:"生产方不存在，请先在「生产方管理」中添加"},400);E=Se.id,N=Se.username}let M=await R(e,r.username),O={id:_().slice(0,12),customer:o.trim(),title:s.trim(),dueDate:String(i).trim(),amount:w,currency:ht(g),orderUrl:h,purchaseUrl:y,notes:[],status:"pending",done:!1,createdAt:new Date().toISOString()};return E&&(O.producerId=E,O.producerName=N,O.producerAssignedAt=new Date().toISOString()),M.unshift(O),await Q(e,r.username,M),n({ok:!0,todo:O})}if(a.startsWith("/api/todos/")&&a.endsWith("/notes")&&d==="POST"){let r=await x(t,e);if(!r)return n({error:"未登录"},401);let s=decodeURIComponent(a.replace("/api/todos/","").replace("/notes","")),o=await I(t),i=o.text;if(!i||!i.trim())return n({error:"请输入备注内容"},400);let l=r.username;if(Ht(r)){let S=o.owner||r.username;if(S!==r.username&&!await P(e,S,v(r)))return n({error:"无权操作该用户的待办"},403);l=S}else if(o.owner&&o.owner!==r.username){if(!await qt(e,r,o.owner,s))return n({error:"无权操作该用户的待办"},403);l=o.owner}let c=await R(e,l),u=c.findIndex(S=>S.id===s);if(u===-1)return n({error:"未找到"},404);Array.isArray(c[u].notes)||(c[u].notes=[]);let g=await et(e,v(r)),b=new Set(g.map(S=>S.username)),h=new Set;if(Array.isArray(o.mentions))for(let S of o.mentions){let E=String(S||"").trim();E&&b.has(E)&&h.add(E)}for(let S of Rt(i))b.has(S)&&h.add(S);h.delete(r.username);let y=[...h],w={id:_().slice(0,12),text:i.trim(),author:r.username,createdAt:new Date().toISOString(),mentions:y};return c[u].notes.push(w),await Q(e,l,c),await _t(e,r,y,{todoId:c[u].id,todoTitle:c[u].title,owner:l,text:w.text}),n({ok:!0,note:w})}if(a.startsWith("/api/todos/")&&d==="PUT"){let r=await x(t,e);if(!r)return n({error:"未登录"},401);if(Ue(r.role))return n({error:L(r)+"无编辑权限"},403);let s=decodeURIComponent(a.replace("/api/todos/","")),o=await I(t);if((typeof o.status=="string"||typeof o.done=="boolean")&&!Jt(r))return n({error:"只有团队管理员、总经理或拥有「更新订单状态」权限的成员可以改变订单状态"},403);let l=r.username;if(we(r)&&o.owner&&o.owner!==r.username){if(!await P(e,o.owner,v(r)))return n({error:"无权操作该用户的待办"},403);l=o.owner}let c=await R(e,l),u=c.findIndex(g=>g.id===s);if(u===-1)return n({error:"未找到"},404);if(typeof o.title=="string"){let g=o.title.trim();if(!g)return n({error:"请输入 PO# / 主题"},400);c[u].title=g}if(typeof o.dueDate=="string"||o.amount!==void 0||o.orderUrl!==void 0||o.purchaseUrl!==void 0){if((c[u].status||(c[u].done?"done":"pending"))!=="pending")return n({error:"仅「待确认」的待办可修改交期/金额/文件链接"},403);if(typeof o.dueDate=="string"){let b=o.dueDate.trim();if(!/^\d{4}-\d{2}-\d{2}$/.test(b))return n({error:"请输入正确的交期"},400);c[u].dueDate=b}if(o.amount!==void 0&&o.amount!==null&&String(o.amount).trim()!==""){let b=Number(o.amount);if(Number.isNaN(b)||b<0)return n({error:"金额必须为非负数字"},400);c[u].amount=b}if(o.orderUrl!==void 0){let b=J(o.orderUrl);if(b===null)return n({error:"订单文件链接需以 http:// 或 https:// 开头"},400);c[u].orderUrl=b}if(o.purchaseUrl!==void 0){let b=J(o.purchaseUrl);if(b===null)return n({error:"采购文件链接需以 http:// 或 https:// 开头"},400);c[u].purchaseUrl=b}}if(typeof o.producerId=="string"&&o.producerId.trim()){let b=(await B(e,v(r))).find(h=>h.id===o.producerId.trim());if(!b)return n({error:"生产方不存在，请先在「生产方管理」中添加"},400);c[u].producerId=b.id,c[u].producerName=b.username,c[u].producerAssignedAt=new Date().toISOString()}if(typeof o.status=="string"){if(!["pending","doing","done"].includes(o.status))return n({error:"无效的状态"},400);c[u].status=o.status,c[u].done=o.status==="done"}else typeof o.done=="boolean"&&(c[u].done=o.done,c[u].status=o.done?"done":"doing");return c[u].status==="doing"&&!c[u].producerId&&await nt(e,r)?n({error:"请为该待办指定生产方（生产方来自「生产方管理」）"},400):(await Q(e,l,c),n({ok:!0,todo:c[u]}))}if(a.startsWith("/api/todos/")&&a.endsWith("/purchase-link")&&d==="POST"){let r=await x(t,e);if(!r)return n({error:"未登录"},401);if(!le(r))return n({error:L(r)+"无「下生产订单」权限，请联系团队管理员在「成员管理」中开通"},403);let s=decodeURIComponent(a.replace("/api/todos/","").replace("/purchase-link","")),o=await I(t),i=r.username;if(o.owner&&o.owner!==r.username){if(!await P(e,o.owner,v(r)))return n({error:"无权操作该用户的待办"},403);i=o.owner}let l=await R(e,i),c=l.findIndex(g=>g.id===s);if(c===-1)return n({error:"未找到"},404);if(l[c].purchaseUrl)return n({error:"该订单已有采购文件链接，如需修改请联系团队管理员"},403);let u=J(o.purchaseUrl);return u===null?n({error:"采购文件链接需以 http:// 或 https:// 开头"},400):u?(l[c].purchaseUrl=u,await Q(e,i,l),n({ok:!0,purchaseUrl:u})):n({error:"请输入采购文件链接"},400)}if(a.startsWith("/api/todos/")&&a.endsWith("/masked-link")&&d==="POST"){let r=await x(t,e);if(!r)return n({error:"未登录"},401);if(!ye(r))return n({error:L(r)+"无「下脱敏订单」权限，请联系团队管理员在「成员管理」中开通"},403);let s=decodeURIComponent(a.replace("/api/todos/","").replace("/masked-link","")),o=await I(t),i=r.username;if(o.owner&&o.owner!==r.username){if(!await P(e,o.owner,v(r)))return n({error:"无权操作该用户的待办"},403);i=o.owner}let l=await R(e,i),c=l.findIndex(g=>g.id===s);if(c===-1)return n({error:"未找到"},404);if(l[c].maskedUrl)return n({error:"该订单已有脱敏订单文件链接，填写后不可再修改"},403);let u=J(o.maskedUrl);return u===null?n({error:"脱敏订单文件链接需以 http:// 或 https:// 开头"},400):u?(l[c].maskedUrl=u,await Q(e,i,l),n({ok:!0,maskedUrl:u})):n({error:"请输入脱敏订单文件链接"},400)}if(a.startsWith("/api/todos/")&&a.endsWith("/ship-date")&&d==="POST"){let r=await x(t,e);if(!r)return n({error:"未登录"},401);if(!he(r))return n({error:L(r)+"无「添加出货日期」权限，请联系团队管理员在「成员管理」中开通"},403);let s=decodeURIComponent(a.replace("/api/todos/","").replace("/ship-date","")),o=await I(t),i=r.username;if(o.owner&&o.owner!==r.username){if(!await P(e,o.owner,v(r)))return n({error:"无权操作该用户的待办"},403);i=o.owner}let l=await R(e,i),c=l.findIndex(g=>g.id===s);if(c===-1)return n({error:"未找到"},404);if(l[c].shipDate)return n({error:"该订单已添加出货日期，不能重复添加"},403);let u=String(o.shipDate||"").trim();return u?/^\d{4}-\d{2}-\d{2}$/.test(u)?(l[c].shipDate=u,await Q(e,i,l),n({ok:!0,shipDate:u})):n({error:"请输入正确的出货日期"},400):n({error:"请选择出货日期"},400)}if(a.startsWith("/api/todos/")&&d==="DELETE"){let r=await x(t,e);if(!r)return n({error:"未登录"},401);if(Ue(r.role))return n({error:L(r)+"无删除权限"},403);let s=decodeURIComponent(a.replace("/api/todos/","")),o=r.username;if(we(r)){let u=new URL(t.url).searchParams.get("owner");if(u&&u!==r.username){if(!await P(e,u,v(r)))return n({error:"无权操作该用户的待办"},403);o=u}}let i=await R(e,o),l=i.find(u=>u.id===s);return l?(l.status||(l.done?"done":"pending"))!=="pending"?n({error:"该事件已进入「进行中/已完成」状态，无法删除"},403):(i=i.filter(u=>u.id!==s),await Q(e,o,i),n({ok:!0})):n({error:"未找到"},404)}return n({error:"接口不存在"},404)}var Xt={async fetch(t,e,a){let d=new URL(t.url),m=d.pathname;if(m.startsWith("/api/"))try{return await Wt(t,e,m)}catch(f){return n({error:"服务器错误: "+f.message},500)}if(m==="/"||m==="/login"){let f=await G(e);return new Response(Ke(f.siteName,f.allowRegister,f.supportEmail,f.favicon),{headers:{"Content-Type":"text/html; charset=utf-8"}})}if(m==="/todos"){let f=await G(e),p=await x(t,e),r=p?await Le(e,p):!1,s=await Et(e,p,f),i=(p?await nt(e,p):!1)?"":f.adCode;return new Response(je(f.favicon,r,s,i),{headers:{"Content-Type":"text/html; charset=utf-8"}})}if(m==="/admin"){let f=await x(t,e);if(!f)return Response.redirect(d.origin+"/",302);if(!A(f.role))return Response.redirect(d.origin+"/todos",302);let p=await G(e);return new Response(Je(p.favicon,p.siteName),{headers:{"Content-Type":"text/html; charset=utf-8"}})}return new Response("Not Found",{status:404})}};export{Xt as default};
