...
`;function de(t){return String(t??"").replace(/[&<>"']/g,e=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[e])}function he(t){let e=String(t||"").trim();if(!e)return"";let a=de(e);return`<link rel="icon" href="${a}">
...
`}var Fe=`
  ...
  </div>`;function Ne(t,e,a,d){let m=t||"待办清单",p=e!==!1,r=String(a??"").trim();return`<!DOCTYPE html>
<html lang="zh-CN">
<head>
<meta charset="UTF-8">
...
</html>`}function Be(t){return`<!DOCTYPE html>
<html lang="zh-CN">
<head>
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width, initial-scale=1.0">
...
</html>`}function Me(t){return`<!DOCTYPE html>
<html lang="zh-CN">
<head>
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width, initial-scale=1.0">
...
</html>`}import{connect as We}from"cloudflare:sockets";var W="default",Ge="__platform__",Ye=24*60*60*1e3,Ze="该功能为专业版功能：请点击顶栏「订阅」升级为专业用户后使用（试用账号仅限本人使用与添加订单）",V=/^[A-Za-z0-9._%+-]+@[A-Za-z0-9-]+(\.[A-Za-z0-9-]+)*\.[A-Za-z]{2,}$/,K=10*60,Ae=5,ue=60,Xe=5,et=1e4,G=3,tt=2e4,rt=8e3;function D(){let t=new Uint8Array(24);return crypto.getRandomValues(t),Array.from(t,e=>e.toString(16).padStart(2,"0")).join("")}async function A(t){let e=new TextEncoder().encode(t+"::cf-todolist-salt"),a=await crypto.subtle.digest("SHA-256",e);return Array.from(new Uint8Array(a),d=>d.toString(16).padStart(2,"0")).join("")}function J(t){let e=String(t??"").trim();return e?!/^https?:\/\//i.test(e)||/\s/.test(e)?null:e:""}var nt=["USD","CNY"];function at(t){let e=String(t??"").trim().toUpperCase();return nt.includes(e)?e:"USD"}function n(t,e=200){return new Response(JSON.stringify(t),{status:e,headers:{"Content-Type":"application/json; charset=utf-8"}})}async function S(t){try{return await t.json()}catch{return{}}}function ze(t){let a=(t.headers.get("Cookie")||"").match(/(?:^|;\s*)token=([^;]+)/);return a?a[1]:null}function x(t){return t?O(t.role)?Ge:t.teamId||W:W}function O(t){return t==="superadmin"}function E(t){return t==="team"}function oe(t){return!!t&&(t.plan==="pro"||t.plan==="paid")}function ne(t){if(!t)return"active";if(t.status==="disabled")return"disabled";if(oe(t)){let e=t.expiresAt||"";if(e&&new Date(e).getTime()<Date.now())return"expired"}return"active"}function P(t){return oe(t)&&ne(t)==="active"}function Ke(t){return t&&(t.subscribeRequest||t.renewRequest)||null}function B(){return n({error:Ze},403)}async function R(t,e){if(!e||e===W)return null;let a=await t.TODO_KV.get(`user:${e}`);if(!a)return null;let d=JSON.parse(a);return E(d.role)?d:null}async function st(t,e){return e?E(e.role)?P(e):P(await R(t,x(e))):!1}async function y(t,e){let a=ze(t);if(!a)return null;let d=await e.TODO_KV.get(`session:${a}`);if(!d)return null;let m=await e.TODO_KV.get(`user:${d}`);if(!m)return null;let p=JSON.parse(m);if(!O(p.role)){let r=await R(e,x(p));if(r&&r.status==="disabled")return null}return p}function ot(){let t=new Uint32Array(1);return crypto.getRandomValues(t),String(t[0]%1e6).padStart(6,"0")}function z(t){return String(t??"").replace(/[&<>"']/g,e=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[e])}function X(t){let e=String(t||"").trim(),a=e.lastIndexOf("@");if(a<=0)return e;let d=e.slice(0,a),m=d.slice(0,Math.min(2,d.length));return m+"*".repeat(Math.max(1,d.length-m.length))+e.slice(a)}function fe(t){try{let e=new URL(t.url).hostname;return e==="localhost"||e==="127.0.0.1"||e==="::1"||e==="[::1]"}catch{return!1}}function it(t){let e=new Date(t||Date.now());if(Number.isNaN(e.getTime()))return String(t||"");let a=new Date(e.getTime()+8*60*60*1e3),d=m=>String(m).padStart(2,"0");return`${a.getUTCFullYear()}-${d(a.getUTCMonth()+1)}-${d(a.getUTCDate())} ${d(a.getUTCHours())}:${d(a.getUTCMinutes())}`}function be(t){let e=Array.isArray(t)?t.slice(0,G):[],a=[];for(let d=0;d<G;d++){let m=String(e[d]||"").trim();a.push(m&&m.length<=300&&!/\s/.test(m)&&/^https?:\/\//i.test(m)?m:"")}return a}function Y(t){let e=String(t&&t.to||"").trim().toLowerCase(),a=t&&t.cc!==void 0&&t.cc!==null?t.cc:[],d=Array.isArray(a)?a:[a],m=[];for(let p of d){let r=String(p||"").trim();!r||!V.test(r)||r.toLowerCase()!==e&&(m.some(s=>s.toLowerCase()===r.toLowerCase())||m.push(r))}return m}function dt(t,e){let a=[t&&t.adminNotifyEmail,t&&t.from,e&&e.supportEmail];for(let d of a){let m=String(d||"").trim();if(m&&V.test(m))return m}return""}async function te(t){let e={};try{let g=await t.TODO_KV.get("emailSettings");g&&(e=JSON.parse(g)||{})}catch{e={}}let a=String(t.RESEND_API_KEY||e.apiKey||"").trim(),d=String(t.MAIL_FROM_NAME||e.fromName||"").trim(),m=String(t.SMTP_HOST||e.smtpHost||"smtp.qq.com").trim(),p=Number(t.SMTP_PORT||e.smtpPort||465)||465,r=String(t.SMTP_USER||t.QQ_MAIL_USER||e.smtpUser||"").trim(),s=String(t.SMTP_PASS||t.QQ_MAIL_PASS||e.smtpPass||"").trim(),i=String(t.MAIL_FROM||e.from||"").trim(),o=t.SEND_EMAIL&&typeof t.SEND_EMAIL.send=="function"?t.SEND_EMAIL:null,l=i||(r&&V.test(r)?r:""),c=a?"resend":r&&s?"smtp":o?"cloudflare":"",u=String(t.ADMIN_NOTIFY_EMAIL||e.adminNotifyEmail||"").trim(),f=be(e.subQrCodes),b=a?t.RESEND_API_KEY?"env":"kv":r&&s?t.SMTP_USER||t.SMTP_PASS||t.QQ_MAIL_USER||t.QQ_MAIL_PASS?"env":"kv":o?"binding":"none";return{provider:c,apiKey:a,from:l,fromName:d,smtpHost:m,smtpPort:p,smtpUser:r,smtpPass:s,binding:o,adminNotifyEmail:u,subExtraText:String(e.subExtraText||"").trim(),subQrCodes:f,devMode:e.devMode===!0,source:b}}function ie(t){let e=new TextEncoder().encode(String(t??"")),a="";for(let d=0;d<e.length;d++)a+=String.fromCharCode(e[d]);return btoa(a)}function Le(t){let e=String(t||""),a=[];for(let d=0;d<e.length;d+=76)a.push(e.slice(d,d+76));return a.join(`\r
`)}function Ue(t){let e=String(t??"");return/^[\x20-\x7e]*$/.test(e)?e:`=?UTF-8?B?${ie(e)}?=`}var ct="support@cloudnexus.cn";async function F(t){let e={};try{let a=await t.TODO_KV.get("settings");a&&(e=JSON.parse(a)||{})}catch{e={}}return{siteName:e.siteName||"待办清单",allowRegister:e.allowRegister!==!1,supportEmail:e.supportEmail===void 0?ct:String(e.supportEmail||"").trim(),favicon:String(e.favicon||"").trim()}}function De(t){let e=String(t||"");return e?e.slice(0,4)+"****"+e.slice(-4):""}function $e(t,e){return{provider:t.provider,source:t.source,from:t.from,fromName:t.fromName,hasApiKey:!!t.apiKey,apiKeyMasked:De(t.apiKey),smtpHost:t.smtpHost,smtpPort:t.smtpPort,smtpUser:t.smtpUser,hasSmtpPass:!!t.smtpPass,smtpPassMasked:De(t.smtpPass),devMode:t.devMode,adminNotifyEmail:t.adminNotifyEmail||"",subExtraText:t.subExtraText||"",subQrCodes:be(t.subQrCodes).concat("").slice(0,G),envApiKey:!!e.RESEND_API_KEY,envFrom:!!e.MAIL_FROM,envSmtp:!!(e.SMTP_USER||e.SMTP_PASS||e.QQ_MAIL_USER||e.QQ_MAIL_PASS)}}function lt(t,e){let a="cfmail_"+D().slice(0,16),d=String(t.from||"localhost").split("@")[1]||"localhost",m=t.fromName?`${Ue(t.fromName)} <${t.from}>`:`<${t.from}>`,p=Y(e),r=[`From: ${m}`,`To: <${e.to}>`];return p.length&&r.push(`Cc: ${p.map(s=>`<${s}>`).join(", ")}`),r.push(`Subject: ${Ue(e.subject||"")}`,`Date: ${new Date().toUTCString()}`,`Message-ID: <${D()}@${d}>`,"MIME-Version: 1.0",`Content-Type: multipart/alternative; boundary="${a}"`,""),r.concat([`--${a}`,"Content-Type: text/plain; charset=UTF-8","Content-Transfer-Encoding: base64","",Le(ie(e.text||"")),`--${a}`,"Content-Type: text/html; charset=UTF-8","Content-Transfer-Encoding: base64","",Le(ie(e.html||e.text||"")),`--${a}--`,""]).join(`\r
`)}function ut(t,e,a){let d=null;return Promise.race([t,new Promise((m,p)=>{d=setTimeout(()=>{try{a&&a()}catch{}p(new Error(`连接或响应超时（${Math.round(e/1e3)} 秒）`))},e)})]).finally(()=>clearTimeout(d))}function mt(t,e,a){let d=t&&t.message?t.message:String(t),m=(d.match(/\b(\d{3})\b/)||[])[1]||"",p="（提示：本地 `wrangler dev` 已支持 SMTP 的 TLS；如持续超时请检查网络 / 端口是否被拦截，或先用「调试模式」联调）",r="";return m==="535"||m==="534"?r="（提示：QQ 邮箱必须使用「SMTP 授权码」而不是 QQ 登录密码，并先在 QQ 邮箱「设置 → 账户」中开启 SMTP 服务）":m==="530"?r="（提示：服务器要求先建立加密连接 —— 465 用 SSL、587 用 STARTTLS）"+(a?p:""):m==="550"||m==="553"?r="（提示：QQ 邮箱要求「发件邮箱」与 SMTP 账号完全一致）":a&&/secureTransport|starttls|tls|ssl|certificate|handshake|超时|关闭/i.test(d)?r=p:/starttls must be set|secureTransport/i.test(d)?r="（提示：587 必须先以 STARTTLS 建立连接再升级；若持续失败请把端口改为 465（SSL）重试）":/certificate|tls|ssl|handshake/i.test(d)?r="（提示：端口 465 使用 SSL，端口 587 使用 STARTTLS，请检查服务器 / 端口设置）":/超时/.test(d)&&(r="（提示：连接超时通常是网络或端口被拦截所致；QQ 邮箱请用 smtp.qq.com 的 465（SSL）或 587（STARTTLS））"),`SMTP 发信失败（${e}）：${d}${r}`}async function pt(t,e,a){let d=Number(t.smtpPort)||465,m=[d];d===465?m.push(587):d===587&&m.push(465);let p=[];for(let r=0;r<m.length;r++){let s=await ft(m[r],t,e,a);if(s.ok)return s;p.push(m.length>1?`端口 ${m[r]}：${s.error}`:s.error)}return{ok:!1,error:p.join("；")}}async function ft(t,e,a,d){let m=e.smtpHost||"smtp.qq.com",p=!!(d&&d.local),r=Number(e.smtpTimeoutMs)||(p?rt:tt),s=new TextEncoder,i=new TextDecoder,o=null,l=null,c=null,u="",f=`连接 ${m}:${t}`;async function b(){for(;;){let h=u.indexOf(`
`);if(h!==-1){let w=u.slice(0,h);return u=u.slice(h+1),w.replace(/\r$/,"")}let{value:k,done:C}=await l.read();if(C)throw new Error("连接已被服务器关闭");u+=i.decode(k,{stream:!0})}}async function g(){let h=0,k="";for(;;){let C=await b();if(!(C.length<3)&&(h=parseInt(C.slice(0,3),10)||h,k+=(k?" ":"")+C.slice(4).trim(),C.charAt(3)!=="-"))break}return{code:h,text:k}}async function v(h,k){await c.write(s.encode(h+`\r
`));let C=await g();if(!k.includes(C.code))throw new Error(`服务器返回 ${C.code}${C.text?" "+C.text:""}`);return C}async function T(){f=`连接 ${m}:${t}`;try{await o.opened}catch(M){throw new Error(`建立连接失败：${M&&M.message?M.message:M}`)}let h=await g();if(h.code!==220)throw new Error(`服务器返回 ${h.code} ${h.text}`);let C=`EHLO ${e.smtpEhlo||String(e.from||"").split("@")[1]||"localhost"}`;if(await v(C,[250]),t===587){f="STARTTLS",await v("STARTTLS",[220]);try{l.releaseLock()}catch{}try{c.releaseLock()}catch{}o=o.startTls(),u="",l=o.readable.getReader(),c=o.writable.getWriter(),await v(C,[250])}f="身份认证（SMTP 授权码）",await v("AUTH LOGIN",[334]),await v(ie(e.smtpUser),[334]),await v(ie(e.smtpPass),[235]),f="发件地址",await v(`MAIL FROM:<${e.from}>`,[250]);let w=[a.to].concat(Y(a));for(let M=0;M<w.length;M++)f=M===0?"收件地址":"抄送地址",await v(`RCPT TO:<${w[M]}>`,[250,251]);f="发送邮件正文",await v("DATA",[354]);let I=lt(e,a).replace(/\r\n\./g,`\r
..`);await c.write(s.encode(`${I}\r
.\r
`));let $=await g();if($.code!==250)throw new Error(`服务器返回 ${$.code}${$.text?" "+$.text:""}`);try{await v("QUIT",[221])}catch{}return{ok:!0,id:""}}try{return o=We({hostname:m,port:t},{secureTransport:t===587?"starttls":"on"}),l=o.readable.getReader(),c=o.writable.getWriter(),await ut(T(),r,()=>{try{o.close()}catch{}})}catch(h){return{ok:!1,error:mt(h,f,p)}}finally{try{o&&o.close()}catch{}}}async function ve(t,e,a,d){if(!e.provider)return{ok:!1,error:"邮件服务未配置：请在控制台「邮件设置」中填写 QQ 邮箱 SMTP 账号与授权码（或 Resend API Key）"};if(!e.from)return{ok:!1,error:"邮件服务缺少「发件邮箱地址」：请在控制台「邮件设置」中补全后重试"};if(e.provider==="smtp")return pt(e,a,d);if(e.provider==="resend"){let m=new AbortController,p=setTimeout(()=>m.abort(),et);try{let r=await fetch("https://api.resend.com/emails",{method:"POST",headers:{Authorization:`Bearer ${e.apiKey}`,"Content-Type":"application/json"},body:JSON.stringify(Object.assign({from:e.fromName?`${e.fromName} <${e.from}>`:e.from,to:[a.to],subject:a.subject,text:a.text,html:a.html},Y(a).length?{cc:Y(a)}:{})),signal:m.signal}),s=await r.json().catch(()=>({}));return r.ok?{ok:!0,id:s.id||""}:{ok:!1,error:`邮件服务返回错误（${r.status}）：${s.message||s.error||"请检查 API Key 与发件域名"}`}}catch(r){return{ok:!1,error:"邮件服务请求失败："+(r&&r.message?r.message:String(r))}}finally{clearTimeout(p)}}try{let m=Y(a);return await e.binding.send({from:e.fromName?{name:e.fromName,email:e.from}:e.from,to:m.length?[a.to].concat(m):a.to,subject:a.subject,html:a.html,text:a.text}),{ok:!0,id:""}}catch(m){return{ok:!1,error:"邮件发送失败："+(m&&m.message?m.message:String(m))}}}async function gt(t,e,a){let d="待办清单";try{let i=await t.TODO_KV.get("settings");if(i){let o=JSON.parse(i);o&&o.siteName&&(d=o.siteName)}}catch{}let m=e.teamName||e.username,p=`【${d}】邮箱确认码 ${a}（10 分钟内有效）`,r=[`${m}，您好：`,"",`您正在注册「${d}」的团队账号（登录名：${e.username}）。`,`邮箱确认码：${a}`,"有效期：10 分钟。","","请在注册 / 登录页面输入上面的确认码完成验证，验证通过后即可正常登录。","若非本人操作，请忽略本邮件：验证通过前该账号无法登录。","",d].join(`
`),s=`<div style="font-family:-apple-system,'Microsoft YaHei',sans-serif;font-size:14px;color:#37352f;line-height:1.7"><p>${z(m)}，您好：</p><p>您正在注册「${z(d)}」的团队账号（登录名：<b>${z(e.username)}</b>）。</p><p>邮箱确认码：</p><p style="font-size:26px;font-weight:700;letter-spacing:6px;color:#2383e2;margin:8px 0">${a}</p><p>有效期 <b>10 分钟</b>。请在注册 / 登录页面输入该确认码完成验证，验证通过后即可正常登录。</p><p style="color:#9b9a97;font-size:12px">若非本人操作，请忽略本邮件：验证通过前该账号无法登录。</p></div>`;return{subject:p,text:r,html:s}}async function bt(t,e,a,d,m){let p="待办清单";try{let w=await t.TODO_KV.get("settings");if(w){let I=JSON.parse(w);I&&I.siteName&&(p=I.siteName)}}catch{}let s=m==="renew"?"续费申请":"订阅申请",i=e.teamName||e.username,o=a&&a.plan?a.plan:null,l=o?`${o.term}（￥${o.price} / ${o.days} 天）`:"未选择套餐",c=it(a&&a.at?a.at:new Date().toISOString()),u=String(d&&d.subExtraText||"").trim(),f=be(d&&d.subQrCodes).filter(Boolean),b=`【${p}】${s}已收到：${i} · ${l}`,g=[`${i}，您好：`,"",`我们已收到您的「${s}」，信息如下（本邮件已同步抄送管理员）：`,"","【申请人信息】",`团队名称：${i}`,`登录账号：${e.username}`,`联系人：${e.contact||"（未填写）"}`,`联系邮箱：${e.email||"（未填写）"}`,`提交时间：${c}`,"","【订阅套餐】",`申请类型：${s}`,`套餐：${l}`,`留言：${a&&a.note||"（无）"}`,""];u&&g.push("【说明】",u,""),f.length&&(g.push("【收款二维码】"),f.forEach((w,I)=>g.push(`二维码 ${I+1}：${w}`)),g.push("")),g.push("提交后由超级管理员为您开通；开通成功后即可使用成员 / 生产方 / 客户管理等全部功能。","如需补充信息，可在系统内重新提交申请（会覆盖上一次）。","",p);let v=g.join(`
`),T=(w,I)=>`<tr><td style="padding:4px 12px 4px 0;color:#6b6b68;white-space:nowrap">${z(w)}</td><td style="padding:4px 0;color:#37352f">${z(I)}</td></tr>`,h=u?`<div style="margin:14px 0 6px;font-weight:600">说明</div><div style="background:#f7f7f5;border-radius:8px;padding:12px;white-space:pre-wrap">${z(u)}</div>`:"",k=f.length?'<div style="margin:16px 0 6px;font-weight:600">收款二维码</div>'+f.map((w,I)=>`<div style="display:inline-block;margin:0 12px 12px 0;text-align:center;vertical-align:top"><img src="${z(w)}" alt="收款二维码 ${I+1}" style="width:180px;height:180px;object-fit:contain;border:1px solid #e9e9e7;border-radius:8px;background:#fff"><div style="font-size:12px;color:#6b6b68;margin-top:4px">二维码 ${I+1}</div></div>`).join(""):"",C=`<div style="font-family:-apple-system,'Microsoft YaHei',sans-serif;font-size:14px;color:#37352f;line-height:1.7"><p>${z(i)}，您好：</p><p>我们已收到您的「<b>${z(s)}</b>」，信息如下（本邮件已同步抄送管理员）：</p><div style="margin:10px 0 6px;font-weight:600">申请人信息</div><table style="border-collapse:collapse;font-size:14px">`+T("团队名称",i)+T("登录账号",e.username)+T("联系人",e.contact||"（未填写）")+T("联系邮箱",e.email||"（未填写）")+T("提交时间",c)+'</table><div style="margin:14px 0 6px;font-weight:600">订阅套餐</div><table style="border-collapse:collapse;font-size:14px">'+T("申请类型",s)+T("套餐",l)+T("留言",a&&a.note||"（无）")+"</table>"+h+k+'<p style="margin-top:16px;color:#6b6b68;font-size:13px">提交后由超级管理员为您开通；开通成功后即可使用成员 / 生产方 / 客户管理等全部功能。<br>如需补充信息，可在系统内重新提交申请（会覆盖上一次）。</p></div>';return{subject:b,text:v,html:C}}async function Ee(t,e,a){let d=await te(t),m=d.devMode||!d.provider&&fe(a);if(!d.provider&&!m)return{ok:!1,error:"邮件服务未配置：请联系超级管理员在控制台「邮件设置」中配置发件服务后再试"};if(d.provider&&!d.from)return{ok:!1,error:"邮件服务缺少「发件邮箱地址」：请先联系超级管理员补全配置"};if(!e.email)return{ok:!1,error:"该账号没有可用的注册邮箱，请重新注册"};let p=Date.now(),r=`mailrate:${e.username}`,s=null;try{let c=await t.TODO_KV.get(r);s=c?JSON.parse(c):null}catch{s=null}let i=!!(s&&p-Number(s.windowStart||0)<60*60*1e3);if(i){let c=ue*1e3-(p-Number(s.lastSentAt||0));if(c>0)return{ok:!1,error:`确认码刚发送过，请 ${Math.ceil(c/1e3)} 秒后再点「重新发送确认码」`};if(Number(s.count||0)>=Xe)return{ok:!1,error:"发送过于频繁（1 小时内最多 5 次），请稍后再试"}}let o=ot();if(m)console.log(`[邮箱确认码] ${e.username} <${e.email}> 确认码：${o}（调试模式，未真实发送邮件）`);else{let c=await gt(t,e,o),u=await ve(t,d,{to:e.email,...c},{local:fe(a)});if(!u.ok)return{ok:!1,error:u.error}}await t.TODO_KV.put(`emailcode:${e.username}`,JSON.stringify({codeHash:await A(o),email:e.email||"",sentAt:p,attempts:0}),{expirationTtl:K});let l=i?{windowStart:s.windowStart,count:Number(s.count||0)+1,lastSentAt:p}:{windowStart:p,count:1,lastSentAt:p};return await t.TODO_KV.put(r,JSON.stringify(l),{expirationTtl:Math.max(60,Math.ceil((60*60*1e3-(p-Number(l.windowStart)))/1e3))}),{ok:!0,sentAt:p,devCode:m?o:""}}async function wt(t,e,a){try{let m=await t.TODO_KV.get(`emailcode:${e.username}`);if(m){let p=JSON.parse(m);if(K*1e3-(Date.now()-Number(p.sentAt||0))>0)return{ok:!0,resent:!1,sentAt:p.sentAt,devCode:""}}}catch{}let d=await Ee(t,e,a);return d.ok?{ok:!0,resent:!0,sentAt:d.sentAt,devCode:d.devCode}:{ok:!1,error:d.error}}async function yt(t,e,a){let d=`emailcode:${e.username}`,m=String(a||"").trim();if(!m)return{ok:!1,error:"请输入邮箱确认码"};let p=await t.TODO_KV.get(d);if(!p)return{ok:!1,error:"确认码已失效，请点击「重新发送确认码」重新获取"};let r=null;try{r=JSON.parse(p)}catch{r=null}if(!r||!r.codeHash)return await t.TODO_KV.delete(d),{ok:!1,error:"确认码已失效，请点击「重新发送确认码」重新获取"};if(Date.now()-Number(r.sentAt||0)>K*1e3)return await t.TODO_KV.delete(d),{ok:!1,error:"确认码已过期（有效期 10 分钟），请点击「重新发送确认码」"};if(Number(r.attempts||0)>=Ae)return await t.TODO_KV.delete(d),{ok:!1,error:"确认码错误次数过多，请点击「重新发送确认码」重新获取"};if(await A(m)!==r.codeHash){r.attempts=Number(r.attempts||0)+1;let s=Ae-r.attempts;return s<=0?(await t.TODO_KV.delete(d),{ok:!1,error:"确认码错误次数过多，请点击「重新发送确认码」重新获取"}):(await t.TODO_KV.put(d,JSON.stringify(r),{expirationTtl:K}),{ok:!1,error:`确认码不正确，请检查后重试（还可尝试 ${s} 次）`})}return{ok:!0}}async function xe(t,e){let a=D();return await t.TODO_KV.put(`session:${a}`,e.username,{expirationTtl:60*60*24*7}),new Response(JSON.stringify({ok:!0,username:e.username,role:e.role}),{status:200,headers:{"Content-Type":"application/json; charset=utf-8","Set-Cookie":`token=${a}; Path=/; HttpOnly; SameSite=Lax; Max-Age=${60*60*24*7}`}})}async function ce(t){let e=await t.TODO_KV.get("user:admin");if(!e){let d={username:"admin",password:await A("admin"),role:"superadmin",createdAt:new Date().toISOString()};await t.TODO_KV.put("user:admin",JSON.stringify(d));return}let a=JSON.parse(e);a.role==="admin"&&(a.role="superadmin",await t.TODO_KV.put("user:admin",JSON.stringify(a)))}async function je(t,e){let a=await t.TODO_KV.list({prefix:"user:"}),d=[];for(let m of a.keys){let p=await t.TODO_KV.get(m.name);if(p){let r=JSON.parse(p);if(r.role==="producer"||r.role==="customer"||O(r.role)||E(r.role)||x(r)!==e)continue;let s={username:r.username,role:r.role,createdAt:r.createdAt,remark:r.remark||"",position:r.position||"",dept:r.dept||"",canPlaceOrder:we(r),canPurchase:ae(r),canViewCustomerOrder:Ie(r),canViewPurchaseOrder:Ce(r),canUpdateStatus:pe(r),canViewAllOrders:se(r)};r.role==="restricted"&&(s.watched=await re(t,r.username)),s.mentionsUnread=await Tt(t,r.username),d.push(s)}}return d}async function L(t,e,a){let d=await t.TODO_KV.get(`user:${e}`);if(!d)return null;let m=JSON.parse(d);return O(m.role)||x(m)!==a?null:m}async function ht(t){let e=await t.TODO_KV.list({prefix:"user:"}),a=[];for(let d of e.keys){let m=await t.TODO_KV.get(d.name);if(!m)continue;let p=JSON.parse(m);E(p.role)&&a.push({username:p.username,teamName:p.teamName||p.username,contact:p.contact||"",email:p.email||"",remark:p.remark||"",plan:oe(p)?"pro":"trial",pro:P(p),status:ne(p),rawStatus:p.status||"active",emailVerified:p.emailVerified!==!1,emailVerifiedAt:p.emailVerifiedAt||"",trialEndsAt:p.trialEndsAt||"",expiresAt:oe(p)&&p.expiresAt||"",createdAt:p.createdAt,renewedAt:p.renewedAt||"",subscribeRequest:Ke(p)})}return a.sort((d,m)=>{let p=d.subscribeRequest&&d.subscribeRequest.at?1:0,r=m.subscribeRequest&&m.subscribeRequest.at?1:0;return p!==r?r-p:d.createdAt<m.createdAt?1:-1}),a}async function q(t,e){let a=await t.TODO_KV.get(`todos:${e}`);return a?JSON.parse(a):[]}async function ee(t,e,a){await t.TODO_KV.put(`todos:${e}`,JSON.stringify(a))}async function H(t,e){let a=await t.TODO_KV.get(`customers:${e}`);return a?JSON.parse(a):[]}async function ke(t,e,a){await t.TODO_KV.put(`customers:${e}`,JSON.stringify(a))}async function j(t,e){let a=!e||e===W?"customerList":`customerList:${e}`,d=await t.TODO_KV.get(a);return d?JSON.parse(d):[]}async function me(t,e,a){let d=!e||e===W?"customerList":`customerList:${e}`;await t.TODO_KV.put(d,JSON.stringify(a))}async function xt(t,e,a){let d=String(a||"").trim();if(!d)return;let m=await H(t,e.username);m.some(s=>s.name===d)||(m.push({id:D().slice(0,12),name:d,createdAt:new Date().toISOString()}),await ke(t,e.username,m));let p=x(e),r=await j(t,p);r.some(s=>s.name===d)||(r.push({id:D().slice(0,12),name:d,description:"",createdAt:new Date().toISOString()}),await me(t,p,r))}var Je=["self","purchased"],vt="self";function qe(t){let e=String(t??"").trim();return Je.includes(e)?e:vt}async function U(t,e){let a=!e||e===W?"producers":`producers:${e}`,d=await t.TODO_KV.get(a);return(d?JSON.parse(d):[]).map(p=>({id:p.id,username:p.username||p.shortName||"",description:p.description||"",nature:qe(p.nature),createdAt:p.createdAt}))}async function le(t,e,a){let d=!e||e===W?"producers":`producers:${e}`;await t.TODO_KV.put(d,JSON.stringify(a))}async function re(t,e){let a=await t.TODO_KV.get(`watch:${e}`);return a?JSON.parse(a):[]}async function Re(t,e,a){await t.TODO_KV.put(`watch:${e}`,JSON.stringify(a))}var Et=50;function kt(t){let e=new Set,a=/@([A-Za-z0-9_.-]{3,20})/g,d=String(t||""),m;for(;(m=a.exec(d))!==null;)e.add(m[1]);return[...e]}async function _e(t,e){let a=[],d=await je(t,e);for(let m of d)a.push({username:m.username,role:m.role,position:m.position||"",dept:m.dept||"",isTeamAdmin:!1,label:m.username});return a}async function ge(t,e){let a=await t.TODO_KV.get(`mentions:${e}`);return a?JSON.parse(a):[]}async function Qe(t,e,a){await t.TODO_KV.put(`mentions:${e}`,JSON.stringify(a.slice(0,Et)))}async function He(t,e){let a=await ge(t,e);return{unread:a.filter(d=>!d.read).length,total:a.length}}async function Tt(t,e){return(await He(t,e)).unread}async function St(t,e,a,d){if(!a||!a.length)return;let m=new Date().toISOString();for(let p of a){let r=await ge(t,p);r.unshift({id:D().slice(0,12),from:e.username,todoId:d.todoId,todoTitle:d.todoTitle||"",todoOwner:d.owner||"",text:d.text||"",at:m,read:!1}),await Qe(t,p,r)}}var It={superadmin:"超级管理员",team:"团队管理员",editor:"成员",viewer:"业务主管",restricted:"生产部",producer:"生产方",customer:"客户",superviewer:"总经理",deptmanager:"部门主管"};function _(t){return t&&(t.position||t.dept||It[t.role]||t.role)||""}function Te(t){return t==="viewer"||t==="restricted"||t==="producer"||t==="customer"}var Ct=["品质部","财务部"];function Pe(t){return!!t&&t.role==="restricted"&&Ct.includes(t.dept||"")}function N(t){return t==="editor"||t==="member"}function Pt(t){let e=t&&t.role;return N(e)?"添加订单":"生产单下单权限"}function we(t){return t?E(t.role)||N(t.role)?!0:t.role==="superviewer"||Z(t.role)||Pe(t)?!1:typeof t.canPlaceOrder=="boolean"?t.canPlaceOrder:!1:!1}function ae(t){return t?E(t.role)?!0:Pe(t)?!1:N(t.role)?t.canUpdateStatus===!0||t.canPurchase===!0:typeof t.canPurchase=="boolean"?t.canPurchase:we(t):!1}async function Ot(t,e){let a=await H(t,e);return Array.isArray(a)&&a.length>0}async function Ve(t,e){return e?N(e.role)?Ot(t,e.username):we(e):!1}function Oe(t){return E(t)||t==="superviewer"||Z(t)}function pe(t){return t?E(t.role)||t.role==="superviewer"?!0:N(t.role)?t.canUpdateStatus===!0:!1:!1}function se(t){return t?N(t.role)?t.canViewAllOrders!==!1:Oe(t.role):!1}function Nt(t){return t?E(t.role)||t.role==="superviewer"?!0:N(t.role)&&t.canUpdateStatus===!0:!1}function Se(t){return t?Oe(t.role)?!0:N(t.role)&&t.canUpdateStatus===!0:!1}function Bt(t){return t?Te(t.role)||Se(t)?!0:N(t.role)?se(t):!1:!1}function Z(t){return t==="deptmanager"}function Ie(t){return t?E(t.role)?!0:N(t.role)?t.canUpdateStatus===!0||t.canViewCustomerOrder===!0:Z(t.role)?t.canViewCustomerOrder!==!1:t.role==="producer"||t.role==="customer":!1}function Ce(t){return t?E(t.role)?!0:N(t.role)?t.canUpdateStatus===!0||t.canViewPurchaseOrder===!0:Z(t.role)?t.canViewPurchaseOrder!==!1:t.role==="superviewer"?!0:ae(t):!1}async function Q(t,e,a=!1,d=null){let m=await t.TODO_KV.list({prefix:"todos:"}),p=m.keys.map(i=>i.name.replace("todos:","")),r=await Promise.all(p.map(async i=>{let o=await t.TODO_KV.get(`user:${i}`);return o?JSON.parse(o):null})),s=[];for(let i=0;i<p.length;i++){let o=r[i];if(!o||x(o)!==e)continue;let l=await t.TODO_KV.get(m.keys[i].name),c=l?JSON.parse(l):[];for(let u of c){let f=u.status||(u.done?"done":"pending");a&&f==="pending"||d&&!d.includes(u.producerId)||s.push({...u,status:f,owner:p[i]})}}return s.sort((i,o)=>i.createdAt<o.createdAt?1:-1),s}async function Mt(t,e){let a=await t.TODO_KV.list({prefix:"user:"}),d=[];for(let m of a.keys){let p=await t.TODO_KV.get(m.name);if(!p)continue;let r=null;try{r=JSON.parse(p)}catch{continue}!r||O(r.role)||x(r)===e&&d.push({username:r.username||m.name.replace(/^user:/,""),role:r.role})}return d}async function At(t,e){let a=0,d=await t.TODO_KV.get(`todos:${e}`);if(d)try{let m=JSON.parse(d);a=Array.isArray(m)?m.length:0}catch{a=0}return await t.TODO_KV.delete(`user:${e}`),await t.TODO_KV.delete(`todos:${e}`),await t.TODO_KV.delete(`customers:${e}`),await t.TODO_KV.delete(`watch:${e}`),await t.TODO_KV.delete(`mentions:${e}`),await t.TODO_KV.delete(`emailcode:${e}`),await t.TODO_KV.delete(`mailrate:${e}`),{todos:a}}async function Lt(t,e,a){let d=t.method;if(a==="/api/login"&&d==="POST"){await ce(e);let{username:r,password:s}=await S(t);if(!r||!s)return n({error:"请输入用户名和密码"},400);let i=await e.TODO_KV.get(`user:${r}`);if(!i)return n({error:"用户名或密码错误"},401);let o=JSON.parse(i);if(await A(s)!==o.password)return n({error:"用户名或密码错误"},401);if(!O(o.role)){let c=await R(e,x(o));if(c&&ne(c)==="disabled")return n({error:"该账户已被停用，请联系超级管理员开通"},403)}if(o.role==="team"&&o.emailVerified===!1){let c=await wt(e,o,t);return n({ok:!1,needVerify:!0,username:o.username,email:X(o.email||""),codeTtlSec:K,resendAfterSec:ue,devCode:c.devCode||void 0,sendError:c.ok?void 0:c.error,message:c.ok?`确认码${c.resent?"已重新发送":"已发送"}至注册邮箱，请在 ${Math.floor(K/60)} 分钟内输入完成确认`:"确认码发送失败，请稍后点「重新发送确认码」重试"})}return xe(e,o)}if(a==="/api/register"&&d==="POST"){if(await ce(e),!(await F(e)).allowRegister)return n({error:"系统当前已关闭新用户注册，如需开通请联系超级管理员"},403);let{teamName:s,username:i,email:o,password:l,confirmPassword:c,contact:u,remark:f}=await S(t),b=(i||"").trim(),g=(s||"").trim(),v=String(o||"").trim(),T=String(l||"");if(!g)return n({error:"请输入团队名称"},400);if(!b)return n({error:"请输入登录用户名"},400);if(!/^[A-Za-z0-9_.-]{3,20}$/.test(b))return n({error:"用户名为 3~20 位字母、数字、下划线、点或短横线"},400);if(b==="admin")return n({error:"该用户名不可用，请更换"},400);if(!v)return n({error:"请输入邮箱"},400);if(v.length>60)return n({error:"邮箱长度不能超过 60 个字符"},400);if(!V.test(v))return n({error:"邮箱格式不正确，请检查后重试"},400);if(!T)return n({error:"请输入密码"},400);if(T.length<6)return n({error:"密码至少 6 位"},400);if(c!==void 0&&T!==String(c))return n({error:"两次输入的密码不一致"},400);if(await e.TODO_KV.get(`user:${b}`))return n({error:"该用户名已被占用，请更换"},400);let k=new Date,C={username:b,password:await A(T),role:"team",teamId:b,teamName:g,contact:String(u||"").trim(),email:v,remark:String(f||"").trim(),emailVerified:!1,plan:"trial",status:"active",trialEndsAt:"",expiresAt:"",createdAt:k.toISOString()};await e.TODO_KV.put(`user:${b}`,JSON.stringify(C));let w=await Ee(e,C,t);return w.ok?n({ok:!0,needVerify:!0,username:b,role:"team",teamName:g,email:X(v),codeTtlSec:K,resendAfterSec:ue,devCode:w.devCode||void 0,unlimitedTrial:!0,message:`确认码已发送至 ${X(v)}，请在 ${Math.floor(K/60)} 分钟内输入完成邮箱确认`}):(await e.TODO_KV.delete(`user:${b}`),n({error:"注册未完成："+w.error},503))}if(a==="/api/register/verify"&&d==="POST"){await ce(e);let{username:r,code:s}=await S(t),i=String(r||"").trim();if(!i)return n({error:"请输入用户名"},400);let o=await e.TODO_KV.get(`user:${i}`);if(!o)return n({error:"用户名不存在，请先注册"},404);let l=JSON.parse(o);if(l.role!=="team")return n({error:"该账号无需邮箱确认"},400);if(l.emailVerified!==!1)return xe(e,l);let c=await yt(e,l,s);return c.ok?(l.emailVerified=!0,l.emailVerifiedAt=new Date().toISOString(),await e.TODO_KV.put(`user:${i}`,JSON.stringify(l)),await e.TODO_KV.delete(`emailcode:${i}`),xe(e,l)):n({error:c.error},400)}if(a==="/api/register/resend"&&d==="POST"){await ce(e);let{username:r,password:s}=await S(t),i=String(r||"").trim();if(!i)return n({error:"请输入用户名"},400);let o=await e.TODO_KV.get(`user:${i}`);if(!o)return n({error:"用户名不存在，请先注册"},404);let l=JSON.parse(o);if(l.role!=="team")return n({error:"该账号无需邮箱确认"},400);if(l.emailVerified!==!1)return n({error:"该账号已完成邮箱确认，可直接登录"},400);if(!s||await A(String(s))!==l.password)return n({error:"密码不正确，无法重新发送确认码"},401);let c=await Ee(e,l,t);return c.ok?n({ok:!0,email:X(l.email||""),codeTtlSec:K,resendAfterSec:ue,devCode:c.devCode||void 0,message:`确认码已重新发送至 ${X(l.email||"")}`}):n({error:c.error},429)}if(a==="/api/logout"&&d==="POST"){let r=ze(t);return r&&await e.TODO_KV.delete(`session:${r}`),new Response(JSON.stringify({ok:!0}),{status:200,headers:{"Content-Type":"application/json; charset=utf-8","Set-Cookie":"token=; Path=/; HttpOnly; SameSite=Lax; Max-Age=0"}})}if(a==="/api/me"&&d==="GET"){let r=await y(t,e);if(!r)return n({error:"未登录"},401);let s={username:r.username,role:r.role};if(s.position=r.position||"",s.canPlaceOrder=await Ve(e,r),s.canPurchase=ae(r),s.canViewCustomerOrder=Ie(r),s.canViewPurchaseOrder=Ce(r),s.canUpdateStatus=pe(r),s.canViewAllOrders=se(r),E(r.role)){s.teamId=r.teamId||r.username,s.teamName=r.teamName||r.username,s.contact=r.contact||"",s.plan=oe(r)?"pro":"trial",s.pro=P(r),s.trialEndsAt=r.trialEndsAt||"",s.expiresAt=r.expiresAt||"",s.status=ne(r);let o=Ke(r);s.subscribeRequest=o,s.renewRequest=o}else{let o=await R(e,x(r));s.teamId=x(r),s.teamName=o?o.teamName||o.username:"",s.teamPro=P(o)}let i=await He(e,r.username);return s.mentionsUnread=i.unread,s.mentionsTotal=i.total,n(s)}if(a==="/api/change-password"&&d==="POST"){let r=await y(t,e);if(!r)return n({error:"未登录"},401);let{oldPassword:s,newPassword:i}=await S(t);return!s||!i?n({error:"请填写完整"},400):await A(s)!==r.password?n({error:"原密码错误"},400):(r.password=await A(i),await e.TODO_KV.put(`user:${r.username}`,JSON.stringify(r)),n({ok:!0}))}if(a==="/api/admin/password"&&d==="POST"){let r=await y(t,e);if(!r)return n({error:"未登录"},401);if(!O(r.role))return n({error:"无权限"},403);let{newPassword:s,confirmPassword:i}=await S(t),o=String(s||"");if(!o.trim())return n({error:"请输入新密码"},400);if(o.length<6)return n({error:"密码至少 6 位"},400);if(i!==void 0&&o!==String(i))return n({error:"两次输入的密码不一致"},400);let l=await e.TODO_KV.get(`user:${r.username}`);if(!l)return n({error:"账号不存在"},404);let c=JSON.parse(l);return c.password=await A(o),c.passwordUpdatedAt=new Date().toISOString(),await e.TODO_KV.put(`user:${r.username}`,JSON.stringify(c)),n({ok:!0,username:r.username})}if(a==="/api/settings"&&d==="GET"){let r=await y(t,e);if(!r)return n({error:"未登录"},401);let s=await F(e),i=E(r.role)?r:await R(e,x(r));return n({settings:{siteName:s.siteName,allowRegister:s.allowRegister,supportEmail:s.supportEmail,favicon:s.favicon,teamName:i?i.teamName||i.username:""}})}if(a==="/api/settings"&&d==="POST"){let r=await y(t,e);if(!r)return n({error:"未登录"},401);let{siteName:s,teamName:i,allowRegister:o,supportEmail:l,favicon:c}=await S(t);if(i!==void 0){if(!E(r.role))return n({error:"无权限"},403);let g=String(i).trim();return g?(r.teamName=g,await e.TODO_KV.put(`user:${r.username}`,JSON.stringify(r)),n({ok:!0,settings:{teamName:g}})):n({error:"请输入团队名称"},400)}if(s===void 0&&o===void 0&&l===void 0&&c===void 0)return n({error:"请填写要保存的内容"},400);if(!O(r.role))return n({error:"无权限"},403);let u=await e.TODO_KV.get("settings"),f=u?JSON.parse(u):{};if(s!==void 0){if(!s||!s.trim())return n({error:"请输入网站名称"},400);f.siteName=s.trim()}if(o!==void 0&&(f.allowRegister=o===!0||o==="true"),l!==void 0){let g=String(l||"").trim();if(g.length>60)return n({error:"联系邮箱长度不能超过 60 个字符"},400);if(g&&!V.test(g))return n({error:"联系邮箱格式不正确，请检查后重试（留空则不显示该提示）"},400);f.supportEmail=g}if(c!==void 0){let g=String(c||"").trim();if(g.length>300)return n({error:"网站图标链接不能超过 300 个字符"},400);let v=J(g);if(v===null)return n({error:"网站图标需填写以 http:// 或 https:// 开头的图片链接（留空则使用默认图标）"},400);f.favicon=v}await e.TODO_KV.put("settings",JSON.stringify(f));let b=await F(e);return n({ok:!0,settings:{siteName:b.siteName,allowRegister:b.allowRegister,supportEmail:b.supportEmail,favicon:b.favicon}})}if((a==="/api/subscribe-request"||a==="/api/renew-request")&&d==="POST"){let r=await y(t,e);if(!r)return n({error:"未登录"},401);if(!E(r.role))return n({error:"无权限"},403);let{note:s,contact:i,plan:o}=await S(t);i!==void 0&&(r.contact=String(i||"").trim().slice(0,50));let l=o&&typeof o=="object"?o:null,c=l&&Number(l.days)>0?Math.floor(Number(l.days)):0,u={at:new Date().toISOString(),kind:P(r)?"renew":"subscribe",plan:l&&c?{id:String(l.id||"").slice(0,20),term:String(l.term||"").slice(0,20),price:Number(l.price)>=0?Number(l.price):0,days:c}:null,note:String(s||"").trim().slice(0,200),contact:r.contact||""};r.subscribeRequest=u,delete r.renewRequest,await e.TODO_KV.put(`user:${r.username}`,JSON.stringify(r));let f={ok:!1,to:r.email||"",cc:[],error:""};try{if(!r.email)f.error="该账号没有可用邮箱，无法发送申请邮件";else{let b=await te(e),g=await F(e),v=dt(b,g);if(f.cc=Y({to:r.email,cc:v?[v]:[]}),b.devMode)f.ok=!0,f.devMode=!0;else{let T=await bt(e,r,u,b,u.kind),h=await ve(e,b,{to:r.email,cc:f.cc,subject:T.subject,text:T.text,html:T.html},{local:fe(t)});f.ok=!!h.ok,h.ok||(f.error=h.error||"邮件发送失败")}}}catch(b){f.ok=!1,f.error="邮件发送异常："+(b&&b.message?b.message:String(b))}return console.log(`[订阅申请邮件] ${r.username} -> ${f.to||"-"}`+(f.cc&&f.cc.length?`（抄送 ${f.cc.join(", ")}）`:"")+` ${f.ok?"成功":"失败："+f.error}`),n({ok:!0,subscribeRequest:u,mail:f})}let m="/api/teams";if(a===m&&d==="GET"){let r=await y(t,e);return r?O(r.role)?n({teams:await ht(e)}):n({error:"无权限"},403):n({error:"未登录"},401)}if(a.startsWith(`${m}/`)&&a.endsWith("/status")&&d==="POST"){let r=await y(t,e);if(!r)return n({error:"未登录"},401);if(!O(r.role))return n({error:"无权限"},403);let s=decodeURIComponent(a.replace(`${m}/`,"").replace("/status","")),{status:i}=await S(t);if(!["active","disabled"].includes(i))return n({error:"无效的状态"},400);let o=await R(e,s);return o?(o.status=i,await e.TODO_KV.put(`user:${s}`,JSON.stringify(o)),n({ok:!0,status:ne(o),expiresAt:o.expiresAt||o.trialEndsAt||""})):n({error:"团队用户不存在"},404)}if(a.startsWith(`${m}/`)&&a.endsWith("/renew")&&d==="POST"){let r=await y(t,e);if(!r)return n({error:"未登录"},401);if(!O(r.role))return n({error:"无权限"},403);let s=decodeURIComponent(a.replace(`${m}/`,"").replace("/renew","")),i=await S(t),o=Number(i.days)>0?Math.floor(Number(i.days)):30;if(o>3650)return n({error:"续费天数过大"},400);let l=await R(e,s);if(!l)return n({error:"团队用户不存在"},404);let c=Date.now(),u=new Date(l.expiresAt||"").getTime(),f=Number.isNaN(u)||u<c?c:u,b=new Date(f+o*Ye).toISOString();return l.expiresAt=b,l.status="active",l.plan="pro",l.renewedAt=new Date().toISOString(),delete l.subscribeRequest,delete l.renewRequest,await e.TODO_KV.put(`user:${s}`,JSON.stringify(l)),n({ok:!0,days:o,expiresAt:b,status:"active",pro:!0})}if(a.startsWith(`${m}/`)&&a.endsWith("/reset-password")&&d==="POST"){let r=await y(t,e);if(!r)return n({error:"未登录"},401);if(!O(r.role))return n({error:"无权限"},403);let s=decodeURIComponent(a.replace(`${m}/`,"").replace("/reset-password","")),{newPassword:i}=await S(t);if(!i||!String(i).trim())return n({error:"请输入新密码"},400);let o=await R(e,s);return o?(o.password=await A(String(i).trim()),await e.TODO_KV.put(`user:${s}`,JSON.stringify(o)),n({ok:!0})):n({error:"团队用户不存在"},404)}if(a.startsWith(`${m}/`)&&a.endsWith("/remark")&&d==="POST"){let r=await y(t,e);if(!r)return n({error:"未登录"},401);if(!O(r.role))return n({error:"无权限"},403);let s=decodeURIComponent(a.replace(`${m}/`,"").replace("/remark","")),{remark:i}=await S(t),o=await R(e,s);return o?(o.remark=String(i||"").trim(),await e.TODO_KV.put(`user:${s}`,JSON.stringify(o)),n({ok:!0,remark:o.remark})):n({error:"团队用户不存在"},404)}if(a.startsWith(`${m}/`)&&d==="DELETE"){let r=await y(t,e);if(!r)return n({error:"未登录"},401);if(!O(r.role))return n({error:"无权限"},403);let s=decodeURIComponent(a.slice(m.length+1));if(!s||s.indexOf("/")!==-1)return n({error:"接口不存在"},404);let i=await R(e,s);if(!i)return n({error:"团队用户不存在"},404);if(i.status!=="disabled")return n({error:"请先「停用」该团队用户，确认无误后再删除"},400);let o=await Mt(e,s),l=o.reduce((b,g)=>(b[g.role]=(b[g.role]||0)+1,b),{}),c=0;for(let b of o){let g=await At(e,b.username);c+=g.todos}let u=await U(e,s),f=await j(e,s);return await e.TODO_KV.delete(`producers:${s}`),await e.TODO_KV.delete(`customerList:${s}`),n({ok:!0,deleted:{team:s,teamName:i.teamName||s,accounts:o.length,members:o.filter(b=>!["team","producer","customer"].includes(b.role)).length,producers:u.length,customers:f.length,todos:c,roles:l}})}if(a==="/api/admin/email-settings"&&d==="GET"){let r=await y(t,e);if(!r)return n({error:"未登录"},401);if(!O(r.role))return n({error:"无权限"},403);let s=await te(e);return n({settings:$e(s,e)})}if(a==="/api/admin/email-settings"&&d==="POST"){let r=await y(t,e);if(!r)return n({error:"未登录"},401);if(!O(r.role))return n({error:"无权限"},403);let{from:s,fromName:i,apiKey:o,devMode:l,smtpHost:c,smtpPort:u,smtpUser:f,smtpPass:b,adminNotifyEmail:g,subExtraText:v,subQrCodes:T}=await S(t),h=await e.TODO_KV.get("emailSettings"),k=h?JSON.parse(h):{};if(s!==void 0){let w=String(s||"").trim();if(w&&!V.test(w))return n({error:"发件邮箱格式不正确，请检查后重试"},400);k.from=w}if(i!==void 0&&(k.fromName=String(i||"").trim().slice(0,30)),o!==void 0){let w=String(o||"").trim();w==="-"?delete k.apiKey:w&&(k.apiKey=w)}if(c!==void 0){let w=String(c||"").trim();if(w&&!/^[A-Za-z0-9.-]{1,60}$/.test(w))return n({error:"SMTP 服务器地址格式不正确（例如 smtp.qq.com）"},400);k.smtpHost=w||"smtp.qq.com"}if(u!==void 0){let w=String(u??"").trim(),I=Number(w)||0;if(w&&(I<1||I>65535))return n({error:"SMTP 端口需为 1~65535 的数字（QQ 邮箱用 465，或 587 + STARTTLS）"},400);k.smtpPort=I||465}if(f!==void 0){let w=String(f||"").trim();if(w&&!V.test(w))return n({error:"SMTP 账号需填写完整邮箱地址（例如 xxx@qq.com）"},400);k.smtpUser=w}if(b!==void 0){let w=String(b||"").trim();w==="-"?delete k.smtpPass:w&&(k.smtpPass=w)}if(l!==void 0&&(k.devMode=!!l),g!==void 0){let w=String(g||"").trim();if(w&&!V.test(w))return n({error:"管理员提醒邮箱格式不正确（留空则默认抄送到发件邮箱）"},400);k.adminNotifyEmail=w}if(v!==void 0){let w=String(v||"").replace(/\r\n/g,`
`).trim();if(w.length>1e3)return n({error:"自定义说明不能超过 1000 个字符"},400);k.subExtraText=w}if(T!==void 0){let w=Array.isArray(T)?T.slice(0,G):[];if(w.length&&T.length>G)return n({error:`收款二维码最多 ${G} 个`},400);for(let $=0;$<w.length;$++){let M=String(w[$]||"").trim();if(M){if(M.length>300)return n({error:`收款二维码链接太长（第 ${$+1} 个，最多 300 个字符）`},400);if(J(M)===null)return n({error:`收款二维码需填写以 http:// 或 https:// 开头的图片链接（第 ${$+1} 个，留空则不显示）`},400)}}let I=be(w);for(;I.length&&!I[I.length-1];)I.pop();k.subQrCodes=I}await e.TODO_KV.put("emailSettings",JSON.stringify(k));let C=await te(e);return n({ok:!0,settings:$e(C,e)})}if(a==="/api/admin/email-test"&&d==="POST"){let r=await y(t,e);if(!r)return n({error:"未登录"},401);if(!O(r.role))return n({error:"无权限"},403);let{to:s}=await S(t),i=String(s||"").trim();if(!i)return n({error:"请输入测试收件邮箱"},400);if(!V.test(i))return n({error:"测试收件邮箱格式不正确"},400);let o=await te(e);if(o.devMode)return n({ok:!0,devMode:!0,message:"当前为调试模式（不真实发送邮件），无需测试发信"});if(!o.provider)return n({error:"邮件服务未配置：请填写 QQ 邮箱 SMTP 账号与授权码（或 Resend API Key）并保存后，再发送测试邮件"},400);let l=o.provider==="smtp"?"QQ 邮箱 SMTP":o.provider==="resend"?"Resend":"Cloudflare 邮件绑定",c=await ve(e,o,{to:i,subject:"【测试】邮件服务配置正常",text:"这是一封测试邮件：收到本邮件说明「注册邮箱确认码」的发件服务已配置成功。",html:"<p>这是一封测试邮件：收到本邮件说明「注册邮箱确认码」的发件服务已配置成功。</p>"},{local:fe(t)});return c.ok?n({ok:!0,message:`测试邮件已通过 ${l} 发送至 ${i}，请查收（若没收到，请检查垃圾邮件箱）`}):n({error:c.error},400)}if(a==="/api/users"&&d==="GET"){let r=await y(t,e);return r?E(r.role)?P(r)?n({users:await je(e,x(r))}):B():n({error:"无权限"},403):n({error:"未登录"},401)}if(a==="/api/users"&&d==="POST"){let r=await y(t,e);if(!r)return n({error:"未登录"},401);if(!E(r.role))return n({error:"无权限"},403);if(!P(r))return B();let{username:s,password:i,position:o}=await S(t);if(!s||!i)return n({error:"请填写用户名和密码"},400);if(await e.TODO_KV.get(`user:${s}`))return n({error:"用户名已存在"},400);let c={username:s,password:await A(i),role:"editor",teamId:x(r),createdAt:new Date().toISOString()},u=String(o??"").trim();return u&&(c.position=u.slice(0,20)),await e.TODO_KV.put(`user:${s}`,JSON.stringify(c)),n({ok:!0})}if(a.startsWith("/api/users/")&&d==="DELETE"){let r=await y(t,e);if(!r)return n({error:"未登录"},401);if(!E(r.role))return n({error:"无权限"},403);if(!P(r))return B();let s=decodeURIComponent(a.replace("/api/users/",""));return await L(e,s,x(r))?(await e.TODO_KV.delete(`user:${s}`),await e.TODO_KV.delete(`todos:${s}`),await e.TODO_KV.delete(`customers:${s}`),await e.TODO_KV.delete(`watch:${s}`),await e.TODO_KV.delete(`mentions:${s}`),n({ok:!0})):n({error:"成员不存在"},404)}if(a.startsWith("/api/users/")&&a.endsWith("/reset-password")&&d==="POST"){let r=await y(t,e);if(!r)return n({error:"未登录"},401);if(!E(r.role))return n({error:"无权限"},403);if(!P(r))return B();let s=x(r),i=decodeURIComponent(a.replace("/api/users/","").replace("/reset-password","")),{newPassword:o}=await S(t);if(!o||!o.trim())return n({error:"请输入新密码"},400);let l=await e.TODO_KV.get(`user:${i}`);if(!l){let f=(await U(e,s)).find(v=>v.username===i);if(f)return await e.TODO_KV.put(`user:${i}`,JSON.stringify({username:i,password:await A(o),role:"producer",producerId:f.id,teamId:s,createdAt:new Date().toISOString()})),n({ok:!0,created:!0});let g=(await j(e,s)).find(v=>v.name===i);return g?(await e.TODO_KV.put(`user:${i}`,JSON.stringify({username:i,password:await A(o),role:"customer",customerId:g.id,customerName:g.name,teamId:s,createdAt:new Date().toISOString()})),n({ok:!0,created:!0})):n({error:"成员不存在"},404)}let c=JSON.parse(l);return O(c.role)||x(c)!==s?n({error:"成员不存在"},404):(c.password=await A(o),await e.TODO_KV.put(`user:${i}`,JSON.stringify(c)),n({ok:!0}))}if(a.startsWith("/api/users/")&&a.endsWith("/remark")&&d==="POST"){let r=await y(t,e);if(!r)return n({error:"未登录"},401);if(!E(r.role))return n({error:"无权限"},403);if(!P(r))return B();let s=decodeURIComponent(a.replace("/api/users/","").replace("/remark","")),{remark:i}=await S(t),o=await L(e,s,x(r));return o?(o.remark=String(i||"").trim(),await e.TODO_KV.put(`user:${s}`,JSON.stringify(o)),n({ok:!0,remark:o.remark})):n({error:"成员不存在"},404)}if(a.startsWith("/api/users/")&&a.endsWith("/position")&&d==="POST"){let r=await y(t,e);if(!r)return n({error:"未登录"},401);if(!E(r.role))return n({error:"无权限"},403);if(!P(r))return B();let s=decodeURIComponent(a.replace("/api/users/","").replace("/position","")),{position:i}=await S(t),o=await L(e,s,x(r));return o?(o.position=String(i||"").trim().slice(0,20),await e.TODO_KV.put(`user:${s}`,JSON.stringify(o)),n({ok:!0,position:o.position})):n({error:"成员不存在"},404)}if(a.startsWith("/api/users/")&&a.endsWith("/order-permission")&&d==="POST"){let r=await y(t,e);if(!r)return n({error:"未登录"},401);if(!E(r.role))return n({error:"无权限"},403);if(!P(r))return B();let s=decodeURIComponent(a.replace("/api/users/","").replace("/order-permission","")),i=await S(t),o=typeof i.canPlaceOrder=="boolean",l=typeof i.canPurchase=="boolean",c=typeof i.canViewCustomerOrder=="boolean",u=typeof i.canViewPurchaseOrder=="boolean",f=typeof i.canUpdateStatus=="boolean",b=typeof i.canViewAllOrders=="boolean";if(!o&&!l&&!c&&!u&&!f&&!b)return n({error:"请传入 true（有）或 false（无）"},400);let g=await L(e,s,x(r));return g?o&&N(g.role)?n({error:"该成员的「添加订单」权限由「客户列表」自动决定（有客户即可添加订单），无需在此设置"},400):o&&(g.role==="superviewer"||Z(g.role))?n({error:_(g)+"成员不需要「生产单下单权限」（固定不录入订单），无需设置"},400):b&&!N(g.role)?n({error:"「是否可查看全部订单」仅适用于普通成员（原业务部）（当前成员："+_(g)+"）"},400):f&&!N(g.role)?n({error:"「是否可以更新订单状态」仅适用于普通成员（原业务部）（当前成员："+_(g)+"）"},400):(c||u)&&!N(g.role)&&!Z(g.role)?n({error:"「是否可以查看客户订单 / 查看生产订单」仅适用于普通成员（原业务部）与「部门主管」成员（当前成员："+_(g)+"）"},400):Pe(g)&&(o||l)?n({error:_(g)+"成员不需要「"+(l?"下生产订单":"生产单下单权限")+"」（该部门无录入/下单需求），无需设置"},400):(o&&(g.canPlaceOrder=i.canPlaceOrder),l&&(g.canPurchase=i.canPurchase),c&&(g.canViewCustomerOrder=i.canViewCustomerOrder),u&&(g.canViewPurchaseOrder=i.canViewPurchaseOrder),f&&(g.canUpdateStatus=i.canUpdateStatus),b&&(g.canViewAllOrders=i.canViewAllOrders),await e.TODO_KV.put(`user:${s}`,JSON.stringify(g)),n({ok:!0,canPlaceOrder:we(g),canPurchase:ae(g),canViewCustomerOrder:Ie(g),canViewPurchaseOrder:Ce(g),canUpdateStatus:pe(g),canViewAllOrders:se(g)})):n({error:"成员不存在"},404)}if(a.startsWith("/api/customers/")&&d==="GET"){let r=await y(t,e);if(!r)return n({error:"未登录"},401);let s=decodeURIComponent(a.replace("/api/customers/",""));return s!==r.username&&!(E(r.role)?await L(e,s,x(r)):null)?n({error:"无权限"},403):n({customers:await H(e,s)})}if(a.startsWith("/api/customers/")&&d==="POST"){let r=await y(t,e);if(!r)return n({error:"未登录"},401);if(!E(r.role))return n({error:"无权限"},403);if(!P(r))return B();let s=decodeURIComponent(a.replace("/api/customers/",""));if(!await L(e,s,x(r)))return n({error:"成员不存在"},404);let{name:o,globalId:l}=await S(t);if(!o||!o.trim())return n({error:"请输入客户名称"},400);let c=await H(e,s),u=o.trim(),f=l?String(l).trim():"";if(c.some(g=>g.name===u||f&&g.id===f))return n({error:"客户已存在"},400);let b={id:f||D().slice(0,12),name:u,createdAt:new Date().toISOString()};return c.push(b),await ke(e,s,c),n({ok:!0,customer:b})}if(a.startsWith("/api/customers/")&&d==="DELETE"){let r=await y(t,e);if(!r)return n({error:"未登录"},401);if(!E(r.role))return n({error:"无权限"},403);if(!P(r))return B();let s=decodeURIComponent(a.replace("/api/customers/","")),i=s.lastIndexOf("/");if(i===-1)return n({error:"参数错误"},400);let o=s.slice(0,i),l=s.slice(i+1);if(!await L(e,o,x(r)))return n({error:"成员不存在"},404);let u=await H(e,o);return u=u.filter(f=>f.id!==l),await ke(e,o,u),n({ok:!0})}if(a==="/api/customer-list"&&d==="GET"){let r=await y(t,e);return r?!E(r.role)||!P(r)?B():n({customers:await j(e,x(r))}):n({error:"未登录"},401)}if(a==="/api/customer-list"&&d==="POST"){let r=await y(t,e);if(!r)return n({error:"未登录"},401);if(!E(r.role))return n({error:"无权限"},403);let s=x(r),{name:i,password:o,description:l}=await S(t);if(!i||!i.trim())return n({error:"请输入用户名"},400);if(!o||!String(o).trim())return n({error:"请输入密码"},400);let c=await j(e,s),u=i.trim();if(c.some(g=>g.name===u))return n({error:"该用户名已存在"},400);if(await e.TODO_KV.get(`user:${u}`))return n({error:"该用户名已被占用"},400);let b={id:D().slice(0,12),name:u,description:(l||"").trim(),createdAt:new Date().toISOString()};return c.push(b),await me(e,s,c),await e.TODO_KV.put(`user:${u}`,JSON.stringify({username:u,password:await A(String(o).trim()),role:"customer",customerId:b.id,customerName:u,teamId:s,createdAt:b.createdAt})),n({ok:!0,customer:b})}if(a.startsWith("/api/customer-list/")&&d==="DELETE"){let r=await y(t,e);if(!r)return n({error:"未登录"},401);if(!E(r.role))return n({error:"无权限"},403);if(!P(r))return B();let s=x(r),i=decodeURIComponent(a.replace("/api/customer-list/","")),o=await j(e,s),l=o.find(c=>c.id===i);return o=o.filter(c=>c.id!==i),await me(e,s,o),l&&l.name&&await e.TODO_KV.delete(`user:${l.name}`),n({ok:!0})}if(a.startsWith("/api/customer-list/")&&a.endsWith("/description")&&d==="POST"){let r=await y(t,e);if(!r)return n({error:"未登录"},401);if(!E(r.role))return n({error:"无权限"},403);if(!P(r))return B();let s=x(r),i=decodeURIComponent(a.replace("/api/customer-list/","").replace("/description","")),{description:o}=await S(t),l=await j(e,s),c=l.find(u=>u.id===i);return c?(c.description=String(o||"").trim(),await me(e,s,l),n({ok:!0,description:c.description})):n({error:"客户不存在"},404)}if(a==="/api/producers"&&d==="GET"){let r=await y(t,e);return r?n({producers:await U(e,x(r))}):n({error:"未登录"},401)}if(a==="/api/producers"&&d==="POST"){let r=await y(t,e);if(!r)return n({error:"未登录"},401);if(!E(r.role))return n({error:"无权限"},403);if(!P(r))return B();let s=x(r),{username:i,password:o,description:l,nature:c}=await S(t),u=(i||"").trim();if(!u)return n({error:"请输入用户名"},400);if(!o||!String(o).trim())return n({error:"请输入密码"},400);let f=await U(e,s);if(f.some(v=>v.username===u))return n({error:"该用户名已存在"},400);if(await e.TODO_KV.get(`user:${u}`))return n({error:"该用户名已被成员占用"},400);let g={id:D().slice(0,12),username:u,description:(l||"").trim(),nature:qe(c),createdAt:new Date().toISOString()};return f.push(g),await le(e,s,f),await e.TODO_KV.put(`user:${u}`,JSON.stringify({username:u,password:await A(String(o).trim()),role:"producer",producerId:g.id,teamId:s,createdAt:g.createdAt})),n({ok:!0,producer:g})}if(a.startsWith("/api/producers/")&&d==="DELETE"){let r=await y(t,e);if(!r)return n({error:"未登录"},401);if(!E(r.role))return n({error:"无权限"},403);if(!P(r))return B();let s=x(r),i=decodeURIComponent(a.replace("/api/producers/","")),o=await U(e,s),l=o.find(c=>c.id===i);return o=o.filter(c=>c.id!==i),await le(e,s,o),l&&l.username&&await e.TODO_KV.delete(`user:${l.username}`),n({ok:!0})}if(a.startsWith("/api/producers/")&&a.endsWith("/description")&&d==="POST"){let r=await y(t,e);if(!r)return n({error:"未登录"},401);if(!E(r.role))return n({error:"无权限"},403);if(!P(r))return B();let s=x(r),i=decodeURIComponent(a.replace("/api/producers/","").replace("/description","")),{description:o}=await S(t),l=await U(e,s),c=l.find(u=>u.id===i);return c?(c.description=String(o||"").trim(),await le(e,s,l),n({ok:!0,description:c.description})):n({error:"生产方不存在"},404)}if(a.startsWith("/api/producers/")&&a.endsWith("/nature")&&d==="POST"){let r=await y(t,e);if(!r)return n({error:"未登录"},401);if(!E(r.role))return n({error:"无权限"},403);if(!P(r))return B();let s=x(r),i=decodeURIComponent(a.replace("/api/producers/","").replace("/nature","")),{nature:o}=await S(t),l=String(o??"").trim();if(!Je.includes(l))return n({error:"生产方性质只能是「自产」或「外购」"},400);let c=await U(e,s),u=c.find(f=>f.id===i);return u?(u.nature=l,await le(e,s,c),n({ok:!0,nature:u.nature})):n({error:"生产方不存在"},404)}let p="/api/watch/";if(a.startsWith(p)&&d==="GET"){let r=await y(t,e);if(!r)return n({error:"未登录"},401);let s=x(r),i=decodeURIComponent(a.replace(p,""));if(i!==r.username&&!(E(r.role)?await L(e,i,s):null))return n({error:"无权限"},403);let o=await re(e,i),l=await U(e,s),c=o.map(u=>{let f=l.find(b=>b.id===u);return f||{id:u,username:"（已删除的生产方）",description:"",deleted:!0}});return n({producers:c})}if(a.startsWith(p)&&d==="POST"){let r=await y(t,e);if(!r)return n({error:"未登录"},401);if(!E(r.role))return n({error:"无权限"},403);if(!P(r))return B();let s=x(r),i=decodeURIComponent(a.replace(p,"")),o=await L(e,i,s);if(!o)return n({error:"成员不存在"},404);let{producerId:l}=await S(t);if(!l||!String(l).trim())return n({error:"请选择生产方"},400);let c=String(l).trim();if(!(await U(e,s)).some(b=>b.id===c))return n({error:"生产方不存在，请先在「生产方管理」中添加"},400);if(o.role!=="restricted")return n({error:"该成员不是「生产部」成员，无法授权可观察的生产方"},400);let f=await re(e,i);return f.includes(c)?n({error:"已授权该生产方"},400):(f.push(c),await Re(e,i,f),n({ok:!0,producerIds:f}))}if(a.startsWith(p)&&d==="DELETE"){let r=await y(t,e);if(!r)return n({error:"未登录"},401);if(!E(r.role))return n({error:"无权限"},403);if(!P(r))return B();let s=decodeURIComponent(a.replace(p,"")),i=s.lastIndexOf("/");if(i===-1)return n({error:"参数错误"},400);let o=s.slice(0,i),l=s.slice(i+1);if(!await L(e,o,x(r)))return n({error:"成员不存在"},404);let u=await re(e,o);return u=u.filter(f=>f!==l),await Re(e,o,u),n({ok:!0,producerIds:u})}if(a==="/api/team-members"&&d==="GET"){let r=await y(t,e);return r?n({members:await _e(e,x(r))}):n({error:"未登录"},401)}if(a==="/api/mentions"&&d==="GET"){let r=await y(t,e);if(!r)return n({error:"未登录"},401);let s=await ge(e,r.username);return n({items:s,unread:s.filter(i=>!i.read).length,total:s.length})}if(a==="/api/mentions/read"&&d==="POST"){let r=await y(t,e);if(!r)return n({error:"未登录"},401);let s=await S(t),i=s&&s.id?String(s.id).trim():"",o=!(s&&s.read===!1),l=await ge(e,r.username),c=0;for(let u of l)u.read!==o&&(i&&u.id!==i||(u.read=o,c++));return c&&await Qe(e,r.username,l),n({ok:!0,changed:c,read:o,unread:l.filter(u=>!u.read).length,total:l.length})}if(a==="/api/todos"&&d==="GET"){let r=await y(t,e);if(!r)return n({error:"未登录"},401);let s=x(r);if(N(r.role)&&pe(r))return n({todos:await Q(e,s,!1),readonly:!1,allUsers:!0});if(Oe(r.role))return n({todos:await Q(e,s,!1),readonly:!1,allUsers:!0});if(r.role==="viewer")return n({todos:await Q(e,s,!0),readonly:!0,allUsers:!0});if(r.role==="restricted"){let o=await re(e,r.username);return n({todos:await Q(e,s,!0,o),readonly:!0,allUsers:!0})}if(r.role==="producer"){let l=(await U(e,s)).find(c=>c.id===r.producerId);return n({todos:await Q(e,s,!0,l?[l.id]:[]),readonly:!0,allUsers:!0})}if(r.role==="customer"){let o=r.customerName||"",l=o?await Q(e,s,!0):[];return n({todos:l.filter(c=>c.customer===o),readonly:!0,allUsers:!0})}if(N(r.role)&&!se(r)){let o=await q(e,r.username);return n({todos:o.map(l=>Object.assign({},l,{owner:r.username})),readonly:!1})}if(N(r.role))return n({todos:await Q(e,s,!1),readonly:!0,allUsers:!0});let i=await q(e,r.username);return n({todos:i.map(o=>Object.assign({},o,{owner:r.username})),readonly:!1})}if(a==="/api/todos"&&d==="POST"){let r=await y(t,e);if(!r)return n({error:"未登录"},401);if(!await Ve(e,r))return N(r.role)?n({error:"暂无客户，请联系团队管理员在「成员管理」的「客户列表」中为你添加客户（有客户即可添加订单）"},403):n({error:_(r)+"无「"+Pt(r)+"」，请联系团队管理员在「成员管理」中开通"},403);let{title:s,customer:i,dueDate:o,amount:l,orderUrl:c,purchaseUrl:u,currency:f}=await S(t);if(!i||!i.trim())return n({error:"选择客户"},400);if(!s||!s.trim())return n({error:"请输入主题"},400);if(!o||!String(o).trim())return n({error:"选择交期"},400);let b=J(c);if(b===null)return n({error:"订单文件链接需以 http:// 或 https:// 开头"},400);let g=J(u);if(g===null)return n({error:"采购文件链接需以 http:// 或 https:// 开头"},400);if(l==null||String(l).trim()==="")return n({error:"输入金额"},400);let v=Number(l);if(Number.isNaN(v)||v<0)return n({error:"金额必须为非负数字"},400);let T=i.trim();if(E(r.role)&&!P(r))await xt(e,r,T);else if(E(r.role)){let C=await j(e,x(r)),w=await H(e,r.username);if(!C.some(I=>I.name===T)&&!w.some(I=>I.name===T))return n({error:"客户不存在，请先在「客户管理」中添加"},400)}else if(!(await H(e,r.username)).some(w=>w.name===T))return n({error:"客户不存在，请联系管理员添加"},400);let h=await q(e,r.username),k={id:D().slice(0,12),customer:i.trim(),title:s.trim(),dueDate:String(o).trim(),amount:v,currency:at(f),orderUrl:b,purchaseUrl:g,notes:[],status:"pending",done:!1,createdAt:new Date().toISOString()};return h.unshift(k),await ee(e,r.username,h),n({ok:!0,todo:k})}if(a.startsWith("/api/todos/")&&a.endsWith("/notes")&&d==="POST"){let r=await y(t,e);if(!r)return n({error:"未登录"},401);let s=decodeURIComponent(a.replace("/api/todos/","").replace("/notes","")),i=await S(t),o=i.text;if(!o||!o.trim())return n({error:"请输入备注内容"},400);let l=r.username;if(Bt(r)){let h=i.owner||r.username;if(h!==r.username&&!await L(e,h,x(r)))return n({error:"无权操作该用户的待办"},403);l=h}else if(i.owner&&i.owner!==r.username)return n({error:"无权操作该用户的待办"},403);let c=await q(e,l),u=c.findIndex(h=>h.id===s);if(u===-1)return n({error:"未找到"},404);Array.isArray(c[u].notes)||(c[u].notes=[]);let f=await _e(e,x(r)),b=new Set(f.map(h=>h.username)),g=new Set;if(Array.isArray(i.mentions))for(let h of i.mentions){let k=String(h||"").trim();k&&b.has(k)&&g.add(k)}for(let h of kt(o))b.has(h)&&g.add(h);g.delete(r.username);let v=[...g],T={id:D().slice(0,12),text:o.trim(),author:r.username,createdAt:new Date().toISOString(),mentions:v};return c[u].notes.push(T),await ee(e,l,c),await St(e,r,v,{todoId:c[u].id,todoTitle:c[u].title,owner:l,text:T.text}),n({ok:!0,note:T})}if(a.startsWith("/api/todos/")&&d==="PUT"){let r=await y(t,e);if(!r)return n({error:"未登录"},401);if(Te(r.role))return n({error:_(r)+"无编辑权限"},403);let s=decodeURIComponent(a.replace("/api/todos/","")),i=await S(t);if((typeof i.status=="string"||typeof i.done=="boolean")&&!Nt(r))return n({error:"只有团队管理员、总经理或拥有「更新订单状态」权限的成员可以改变订单状态"},403);let l=r.username;if(Se(r)&&i.owner&&i.owner!==r.username){if(!await L(e,i.owner,x(r)))return n({error:"无权操作该用户的待办"},403);l=i.owner}let c=await q(e,l),u=c.findIndex(f=>f.id===s);if(u===-1)return n({error:"未找到"},404);if(typeof i.title=="string"){let f=i.title.trim();if(!f)return n({error:"请输入 PO# / 主题"},400);c[u].title=f}if(typeof i.dueDate=="string"||i.amount!==void 0||i.orderUrl!==void 0||i.purchaseUrl!==void 0){if((c[u].status||(c[u].done?"done":"pending"))!=="pending")return n({error:"仅「待确认」的待办可修改交期/金额/文件链接"},403);if(typeof i.dueDate=="string"){let b=i.dueDate.trim();if(!/^\d{4}-\d{2}-\d{2}$/.test(b))return n({error:"请输入正确的交期"},400);c[u].dueDate=b}if(i.amount!==void 0&&i.amount!==null&&String(i.amount).trim()!==""){let b=Number(i.amount);if(Number.isNaN(b)||b<0)return n({error:"金额必须为非负数字"},400);c[u].amount=b}if(i.orderUrl!==void 0){let b=J(i.orderUrl);if(b===null)return n({error:"订单文件链接需以 http:// 或 https:// 开头"},400);c[u].orderUrl=b}if(i.purchaseUrl!==void 0){let b=J(i.purchaseUrl);if(b===null)return n({error:"采购文件链接需以 http:// 或 https:// 开头"},400);c[u].purchaseUrl=b}}if(typeof i.producerId=="string"&&i.producerId.trim()){let b=(await U(e,x(r))).find(g=>g.id===i.producerId.trim());if(!b)return n({error:"生产方不存在，请先在「生产方管理」中添加"},400);c[u].producerId=b.id,c[u].producerName=b.username,c[u].producerAssignedAt=new Date().toISOString()}if(typeof i.status=="string"){if(!["pending","doing","done"].includes(i.status))return n({error:"无效的状态"},400);c[u].status=i.status,c[u].done=i.status==="done"}else typeof i.done=="boolean"&&(c[u].done=i.done,c[u].status=i.done?"done":"doing");return c[u].status==="doing"&&!c[u].producerId&&await st(e,r)?n({error:"请为该待办指定生产方（生产方来自「生产方管理」）"},400):(await ee(e,l,c),n({ok:!0,todo:c[u]}))}if(a.startsWith("/api/todos/")&&a.endsWith("/purchase-link")&&d==="POST"){let r=await y(t,e);if(!r)return n({error:"未登录"},401);if(!ae(r))return n({error:_(r)+"无「下生产订单」权限，请联系团队管理员在「成员管理」中开通"},403);let s=decodeURIComponent(a.replace("/api/todos/","").replace("/purchase-link","")),i=await S(t),o=r.username;if(i.owner&&i.owner!==r.username){if(!await L(e,i.owner,x(r)))return n({error:"无权操作该用户的待办"},403);o=i.owner}let l=await q(e,o),c=l.findIndex(f=>f.id===s);if(c===-1)return n({error:"未找到"},404);if(l[c].purchaseUrl)return n({error:"该订单已有采购文件链接，如需修改请联系团队管理员"},403);let u=J(i.purchaseUrl);return u===null?n({error:"采购文件链接需以 http:// 或 https:// 开头"},400):u?(l[c].purchaseUrl=u,await ee(e,o,l),n({ok:!0,purchaseUrl:u})):n({error:"请输入采购文件链接"},400)}if(a.startsWith("/api/todos/")&&d==="DELETE"){let r=await y(t,e);if(!r)return n({error:"未登录"},401);if(Te(r.role))return n({error:_(r)+"无删除权限"},403);let s=decodeURIComponent(a.replace("/api/todos/","")),i=r.username;if(Se(r)){let u=new URL(t.url).searchParams.get("owner");if(u&&u!==r.username){if(!await L(e,u,x(r)))return n({error:"无权操作该用户的待办"},403);i=u}}let o=await q(e,i),l=o.find(u=>u.id===s);return l?(l.status||(l.done?"done":"pending"))!=="pending"?n({error:"该事件已进入「进行中/已完成」状态，无法删除"},403):(o=o.filter(u=>u.id!==s),await ee(e,i,o),n({ok:!0})):n({error:"未找到"},404)}return n({error:"接口不存在"},404)}var Rt={async fetch(t,e,a){let d=new URL(t.url),m=d.pathname;if(m.startsWith("/api/"))try{return await Lt(t,e,m)}catch(p){return n({error:"服务器错误: "+p.message},500)}if(m==="/"||m==="/login"){let p=await F(e);return new Response(Ne(p.siteName,p.allowRegister,p.supportEmail,p.favicon),{headers:{"Content-Type":"text/html; charset=utf-8"}})}if(m==="/todos"){let p=await F(e);return new Response(Be(p.favicon),{headers:{"Content-Type":"text/html; charset=utf-8"}})}if(m==="/admin"){let p=await y(t,e);if(!p)return Response.redirect(d.origin+"/",302);if(!O(p.role))return Response.redirect(d.origin+"/todos",302);let r=await F(e);return new Response(Me(r.favicon),{headers:{"Content-Type":"text/html; charset=utf-8"}})}return new Response("Not Found",{status:404})}};export{Rt as default};
