...
</html>`}import{connect as wt}from"cloudflare:sockets";var X="default",ht="__platform__",yt=24*60*60*1e3,ke="订单管理系统",G={members:2,producers:1,customers:1},xt={members:"成员",producers:"生产方",customers:"客户"};function Pe(t){let e=G[t]||0,a=xt[t]||t;return`试用版最多可添加 ${e} 个${a}：如需添加更多，请点击顶栏「订阅」升级为专业用户（订阅后解除数量限制）`}var K=/^[A-Za-z0-9._%+-]+@[A-Za-z0-9-]+(\.[A-Za-z0-9-]+)*\.[A-Za-z]{2,}$/,J=10*60,Ge=5,ge=60,vt=5,kt=1e4,re=3,Tt=2e4,Et=8e3;function z(){let t=new Uint8Array(24);return crypto.getRandomValues(t),Array.from(t,e=>e.toString(16).padStart(2,"0")).join("")}async function U(t){let e=new TextEncoder().encode(t+"::cf-todolist-salt"),a=await crypto.subtle.digest("SHA-256",e);return Array.from(new Uint8Array(a),d=>d.toString(16).padStart(2,"0")).join("")}function F(t){let e=String(t??"").trim();return e?!/^https?:\/\//i.test(e)||/\s/.test(e)?null:e:""}var St=["USD","CNY"];function It(t){let e=String(t??"").trim().toUpperCase();return St.includes(e)?e:"USD"}function n(t,e=200){return new Response(JSON.stringify(t),{status:e,headers:{"Content-Type":"application/json; charset=utf-8"}})}async function I(t){try{return await t.json()}catch{return{}}}function st(t){let a=(t.headers.get("Cookie")||"").match(/(?:^|;\s*)token=([^;]+)/);return a?a[1]:null}function v(t){return t?A(t.role)?ht:t.teamId||X:X}function A(t){return t==="superadmin"}function T(t){return t==="team"}function ue(t){return!!t&&(t.plan==="pro"||t.plan==="paid")}function ce(t){if(!t)return"active";if(t.status==="disabled")return"disabled";if(ue(t)){let e=t.expiresAt||"";if(e&&new Date(e).getTime()<Date.now())return"expired"}return"active"}function V(t){return ue(t)&&ce(t)==="active"}function it(t){return t&&(t.subscribeRequest||t.renewRequest)||null}async function $(t,e){if(!e||e===X)return null;let a=await t.TODO_KV.get(`user:${e}`);if(!a)return null;let d=JSON.parse(a);return T(d.role)?d:null}async function dt(t,e){return e?T(e.role)?V(e):V(await $(t,v(e))):!1}async function x(t,e){let a=st(t);if(!a)return null;let d=await e.TODO_KV.get(`session:${a}`);if(!d)return null;let m=await e.TODO_KV.get(`user:${d}`);if(!m)return null;let p=JSON.parse(m);if(!A(p.role)){let f=await $(e,v(p));if(f&&f.status==="disabled")return null}return p}function Ct(){let t=new Uint32Array(1);return crypto.getRandomValues(t),String(t[0]%1e6).padStart(6,"0")}function j(t){return String(t??"").replace(/[&<>"']/g,e=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[e])}function se(t){let e=String(t||"").trim(),a=e.lastIndexOf("@");if(a<=0)return e;let d=e.slice(0,a),m=d.slice(0,Math.min(2,d.length));return m+"*".repeat(Math.max(1,d.length-m.length))+e.slice(a)}function xe(t){try{let e=new URL(t.url).hostname;return e==="localhost"||e==="127.0.0.1"||e==="::1"||e==="[::1]"}catch{return!1}}function Ot(t){let e=new Date(t||Date.now());if(Number.isNaN(e.getTime()))return String(t||"");let a=new Date(e.getTime()+8*60*60*1e3),d=m=>String(m).padStart(2,"0");return`${a.getUTCFullYear()}-${d(a.getUTCMonth()+1)}-${d(a.getUTCDate())} ${d(a.getUTCHours())}:${d(a.getUTCMinutes())}`}function Te(t){let e=Array.isArray(t)?t.slice(0,re):[],a=[];for(let d=0;d<re;d++){let m=String(e[d]||"").trim();a.push(m&&m.length<=300&&!/\s/.test(m)&&/^https?:\/\//i.test(m)?m:"")}return a}function ne(t){let e=String(t&&t.to||"").trim().toLowerCase(),a=t&&t.cc!==void 0&&t.cc!==null?t.cc:[],d=Array.isArray(a)?a:[a],m=[];for(let p of d){let f=String(p||"").trim();!f||!K.test(f)||f.toLowerCase()!==e&&(m.some(r=>r.toLowerCase()===f.toLowerCase())||m.push(f))}return m}function Pt(t,e){let a=[t&&t.adminNotifyEmail,t&&t.from,e&&e.supportEmail];for(let d of a){let m=String(d||"").trim();if(m&&K.test(m))return m}return""}async function ie(t){let e={};try{let b=await t.TODO_KV.get("emailSettings");b&&(e=JSON.parse(b)||{})}catch{e={}}let a=String(t.RESEND_API_KEY||e.apiKey||"").trim(),d=String(t.MAIL_FROM_NAME||e.fromName||"").trim(),m=String(t.SMTP_HOST||e.smtpHost||"smtp.qq.com").trim(),p=Number(t.SMTP_PORT||e.smtpPort||465)||465,f=String(t.SMTP_USER||t.QQ_MAIL_USER||e.smtpUser||"").trim(),r=String(t.SMTP_PASS||t.QQ_MAIL_PASS||e.smtpPass||"").trim(),s=String(t.MAIL_FROM||e.from||"").trim(),o=t.SEND_EMAIL&&typeof t.SEND_EMAIL.send=="function"?t.SEND_EMAIL:null,i=s||(f&&K.test(f)?f:""),l=a?"resend":f&&r?"smtp":o?"cloudflare":"",c=String(t.ADMIN_NOTIFY_EMAIL||e.adminNotifyEmail||"").trim(),u=Te(e.subQrCodes),g=a?t.RESEND_API_KEY?"env":"kv":f&&r?t.SMTP_USER||t.SMTP_PASS||t.QQ_MAIL_USER||t.QQ_MAIL_PASS?"env":"kv":o?"binding":"none";return{provider:l,apiKey:a,from:i,fromName:d,smtpHost:m,smtpPort:p,smtpUser:f,smtpPass:r,binding:o,adminNotifyEmail:c,subExtraText:String(e.subExtraText||"").trim(),subQrCodes:u,devMode:e.devMode===!0,source:g}}function me(t){let e=new TextEncoder().encode(String(t??"")),a="";for(let d=0;d<e.length;d++)a+=String.fromCharCode(e[d]);return btoa(a)}function Ye(t){let e=String(t||""),a=[];for(let d=0;d<e.length;d+=76)a.push(e.slice(d,d+76));return a.join(`\r
`)}function Xe(t){let e=String(t??"");return/^[\x20-\x7e]*$/.test(e)?e:`=?UTF-8?B?${me(e)}?=`}var Nt="support@cloudnexus.cn";async function Y(t){let e={};try{let a=await t.TODO_KV.get("settings");a&&(e=JSON.parse(a)||{})}catch{e={}}return{siteName:e.siteName||ke,allowRegister:e.allowRegister!==!1,supportEmail:e.supportEmail===void 0?Nt:String(e.supportEmail||"").trim(),favicon:String(e.favicon||"").trim(),adCode:String(e.adCode||"").trim()}}async function At(t,e,a){let d="";if(e){let p=T(e.role)?e:await $(t,v(e));d=p?p.teamName||p.username:""}let m=a?String(a.siteName||"").trim():"";return d||m||ke}function Ze(t){let e=String(t||"");return e?e.slice(0,4)+"****"+e.slice(-4):""}function et(t,e){return{provider:t.provider,source:t.source,from:t.from,fromName:t.fromName,hasApiKey:!!t.apiKey,apiKeyMasked:Ze(t.apiKey),smtpHost:t.smtpHost,smtpPort:t.smtpPort,smtpUser:t.smtpUser,hasSmtpPass:!!t.smtpPass,smtpPassMasked:Ze(t.smtpPass),devMode:t.devMode,adminNotifyEmail:t.adminNotifyEmail||"",subExtraText:t.subExtraText||"",subQrCodes:Te(t.subQrCodes).concat("").slice(0,re),envApiKey:!!e.RESEND_API_KEY,envFrom:!!e.MAIL_FROM,envSmtp:!!(e.SMTP_USER||e.SMTP_PASS||e.QQ_MAIL_USER||e.QQ_MAIL_PASS)}}function Mt(t,e){let a="cfmail_"+z().slice(0,16),d=String(t.from||"localhost").split("@")[1]||"localhost",m=t.fromName?`${Xe(t.fromName)} <${t.from}>`:`<${t.from}>`,p=ne(e),f=[`From: ${m}`,`To: <${e.to}>`];return p.length&&f.push(`Cc: ${p.map(r=>`<${r}>`).join(", ")}`),f.push(`Subject: ${Xe(e.subject||"")}`,`Date: ${new Date().toUTCString()}`,`Message-ID: <${z()}@${d}>`,"MIME-Version: 1.0",`Content-Type: multipart/alternative; boundary="${a}"`,""),f.concat([`--${a}`,"Content-Type: text/plain; charset=UTF-8","Content-Transfer-Encoding: base64","",Ye(me(e.text||"")),`--${a}`,"Content-Type: text/html; charset=UTF-8","Content-Transfer-Encoding: base64","",Ye(me(e.html||e.text||"")),`--${a}--`,""]).join(`\r
`)}function Bt(t,e,a){let d=null;return Promise.race([t,new Promise((m,p)=>{d=setTimeout(()=>{try{a&&a()}catch{}p(new Error(`连接或响应超时（${Math.round(e/1e3)} 秒）`))},e)})]).finally(()=>clearTimeout(d))}function Ut(t,e,a){let d=t&&t.message?t.message:String(t),m=(d.match(/\b(\d{3})\b/)||[])[1]||"",p="（提示：本地 `wrangler dev` 已支持 SMTP 的 TLS；如持续超时请检查网络 / 端口是否被拦截，或先用「调试模式」联调）",f="";return m==="535"||m==="534"?f="（提示：QQ 邮箱必须使用「SMTP 授权码」而不是 QQ 登录密码，并先在 QQ 邮箱「设置 → 账户」中开启 SMTP 服务）":m==="530"?f="（提示：服务器要求先建立加密连接 —— 465 用 SSL、587 用 STARTTLS）"+(a?p:""):m==="550"||m==="553"?f="（提示：QQ 邮箱要求「发件邮箱」与 SMTP 账号完全一致）":a&&/secureTransport|starttls|tls|ssl|certificate|handshake|超时|关闭/i.test(d)?f=p:/starttls must be set|secureTransport/i.test(d)?f="（提示：587 必须先以 STARTTLS 建立连接再升级；若持续失败请把端口改为 465（SSL）重试）":/certificate|tls|ssl|handshake/i.test(d)?f="（提示：端口 465 使用 SSL，端口 587 使用 STARTTLS，请检查服务器 / 端口设置）":/超时/.test(d)&&(f="（提示：连接超时通常是网络或端口被拦截所致；QQ 邮箱请用 smtp.qq.com 的 465（SSL）或 587（STARTTLS））"),`SMTP 发信失败（${e}）：${d}${f}`}async function Lt(t,e,a){let d=Number(t.smtpPort)||465,m=[d];d===465?m.push(587):d===587&&m.push(465);let p=[];for(let f=0;f<m.length;f++){let r=await Dt(m[f],t,e,a);if(r.ok)return r;p.push(m.length>1?`端口 ${m[f]}：${r.error}`:r.error)}return{ok:!1,error:p.join("；")}}async function Dt(t,e,a,d){let m=e.smtpHost||"smtp.qq.com",p=!!(d&&d.local),f=Number(e.smtpTimeoutMs)||(p?Et:Tt),r=new TextEncoder,s=new TextDecoder,o=null,i=null,l=null,c="",u=`连接 ${m}:${t}`;async function g(){for(;;){let w=c.indexOf(`
